...
class TheCheckout extends Module
{
    ...
    public function initPsKernel() {
        $initForModules = array('myparcelnl', 'buckaroo3', 'blpaczka', 'maodevfacebookcapi', 'relaiscolis');
        $doInit = false;
...
//        return $needPatch;
    ...
    private function checkPsCheckoutNeedsPatch()
    {
        $needPatch  = false;
        $moduleName = 'ps_checkout';

        if (Module::isEnabled($moduleName)) {
            $moduleClass = _PS_MODULE_DIR_ . "$moduleName/ps_checkout.php";
            if (file_exists($moduleClass)) {
                $file_content = Tools::file_get_contents($moduleClass);
                $hasFundingSourceTokensCall = preg_match('/getFundingSourceTokens/', $file_content) > 0;
                $hasPatch = preg_match('/!\$cart->id_customer/', $file_content) > 0;
                $needPatch = $hasFundingSourceTokensCall && !$hasPatch;
            }
        }
        return $needPatch;
    }
    ...
    private function checkInstallation()
    {
        ...
        if ($this->checkPsCheckoutNeedsPatch()) {
            return 'Detected '.$this->tagIt('b', 'ps_checkout').' payment module v 7.5.0+/8.5.0+ - it '.$this->tagIt('b', 'requires a patch').' to work properly with 
            TheCheckout, please update ps_checkout.php and in function hookDisplayPaymentByBinaries() below'
            .$this->tagIt('pre','
            ...
            $cart = $params[\'cart\'];
...
//                            array('id' => 'style3', 'name' => 'Style no.3'),
        ...
        $enforcedSeparatePaymentModules = array('removed_braintreeofficial');
            ...
            'form' => array(
                ...
                'input'   => array(
                    ...
                    $this->generateSwitch(
                        ...
                        $this->l('Final payment options list will be displayed on separate page. Optional for any payment method.'),
                        array(),
                        ($separate_payment_required ? $this->l('Option enforced, because [Braintree official] payment module is enabled') : ''),
