...
//        );
    ...
    private function ajaxModifyAccountAndAddress()
    {
        ...
        // 9.9.2025 - fix, when firstname/lastname is not visible in address form, and address is being saved
        // AFTER confirmAll was triggered (rare, but it can happen)
        if ($invoiceVisible) {
            // update invoice/delivery address firstname/lastname from customer name, if
            //   address' name is empty and customer's not (e.g. firstname/lastname can be hidden in address section)
            // Important: If firstname/lastname is actually set, but empty, do not update it with customer's name - probably it's really required at the checkout form
...
//        if ($result['account']['hasErrors'] === false) {
//            $businessGroupId = 4;
//
//            // check if user's invoice address's company or vat_number field is set, if yes, assign customer to business group
//            if ($this->context->customer->id > 0 && $this->context->cart->id_address_invoice > 0) {
//                $invoiceAddress = new Address($this->context->cart->id_address_invoice);
//                if (Validate::isLoadedObject($invoiceAddress)) {
//                    $customerGroups = $this->context->customer->getGroups();
//                    if ((isset($invoiceAddress->company) && '' != trim($invoiceAddress->company))
//                        || (isset($invoiceAddress->vat_number) && '' != trim($invoiceAddress->vat_number))) {
//                        if (!in_array($businessGroupId, $customerGroups)) {
//                            $this->context->customer->addGroups(array($businessGroupId));
//                        }
//                    } else {
//                        if (in_array($businessGroupId, $customerGroups)) {
//                            $customerGroups = array_diff($customerGroups, [$businessGroupId]);
...
//        }
    ...
    private function ajaxRefetchAddressBlocks() {
        ...
        $deliveryAddressBlock = $this->context->smarty->fetch(
            'module:' . $this->name . '/views/templates/front/blocks/address-delivery.tpl');
        $invoiceAddressBlock  = $this->context->smarty->fetch(
            'module:' . $this->name . '/views/templates/front/blocks/address-invoice.tpl');
        return array(
            'deliveryAddressBlock' => $deliveryAddressBlock,
            'invoiceAddressBlock'  => $invoiceAddressBlock,
            'id_address_delivery'  => $this->context->cart->id_address_delivery,
            'id_address_invoice'   => $this->context->cart->id_address_invoice,
            'deliveryAddressObject'=> $cartDeliveryAddress,
            'invoiceAddressObject' => $cartInvoiceAddress,
            'addressesAreSame'     => ($this->context->cart->id_address_invoice == $this->context->cart->id_address_delivery) ? 1 : 0
        );
    }
    ...
    private function ajaxModifyAddressSelection()
    {
        ...
        // Collapse address block is when updating delivery address, but the delivery address block is not visible anymore
        if ($addressType === 'delivery') {
            $isCollapseAddressAction = !$deliveryVisible;
        } else {
            $isCollapseAddressAction = !$invoiceVisible;
        }

        // Customer selected "New..." in combobox
        // This is called with Expand and also Collapse (of 2nd address), save new address only on Expand action
        if ((-1 == $addressId)
            ...
            $shallGenerateNewAddressBlock = true;
        } elseif (0 == $addressId && !$isCollapseAddressAction) {
            // getTransientAddressByCartId() finds only addresses that are not yet assigned to cart and have opc_CART_ID alias
            // This code is called also on second address block collapse, we need to avoid creating new address
            $existingAddressId = $this->getTransientAddressByCartId($this->context->cart->id);
            if (null == $existingAddressId) {
                $existingAddressId = 0;
            }

            if ($existingAddressId == 0 && $this->module->config->initialize_address) {
                $this->modifyAddress($addressType,
                    array(
                        'id_country' => $this->context->country->id,
                        'token'      => Tools::getValue('token')
                    ), true);
                // we want to store (new address), without waiting for customer input, as some modules require
                // address ID to be set in cart, e.g. mijoravenipak
                $shallGenerateNewAddressBlock = true;
            } else {
...
//            }
    ...
    private function addPaymentFee($cart, $paymentFee)
    {
        ...
        $cartCopy['subtotals']['payment-fee'] = array(
            'amount' => $paymentFee,
            'label'  => $fee_value > 0 ? $this->module->getTranslation('Payment fee') : $this->module->getTranslation('Payment discount'),
            'type'   => 'payment_fee',
            'value'  => $fee_value,
        );
    ...
    private function getCartSummaryBlock($paymentFee = 0)
    {
        ...
        if (is_numeric($paymentFee) && $paymentFee != 0) {
        ...
        // calculate remaining to spend and completion percentage
        $ps_freeshipping_price = Configuration::get('PS_SHIPPING_FREE_PRICE');
        if ($ps_freeshipping_price) {
            $currency = $this->context->currency;
            $ps_freeshipping_price = $currency->getConversionRate() * $ps_freeshipping_price;

            $configuration = array(
                'display_prices_tax_incl' => (bool) (new TaxConfiguration())->includeTaxes()
            );

            // Adjust for tax if needed
            if (
                (!isset($configuration['display_prices_tax_incl']) || !$configuration['display_prices_tax_incl']) &&
                $presentedCart['totals']['total_excluding_tax']['amount'] > 0
            ) {
                $a = $presentedCart['totals']['total_including_tax']['amount'];
                $b = $presentedCart['totals']['total_excluding_tax']['amount'];
                $c = $presentedCart['subtotals']['shipping']['amount'];
                $product_tax_rate = ($a - $c * ($a / $b)) / ($b - $c);
                if ($product_tax_rate > 0.1) {
                    $ps_freeshipping_price = $ps_freeshipping_price / $product_tax_rate;
                }
            }

            // Calculate totals
            $total_without_payment_fee = $presentedCart['totals']['total']['amount'] - ($presentedCart['subtotals']['payment-fee']['amount'] ?? 0);
            $total_without_shipping = $total_without_payment_fee - $presentedCart['subtotals']['shipping']['amount'];
            $remaining_to_spend = $ps_freeshipping_price - $total_without_shipping;
            $completed_percentage = ($ps_freeshipping_price > 0) ? (100 * $total_without_shipping / $ps_freeshipping_price) : 0;
            $this->context->smarty->assign(array(
                'remaining_to_spend'              => $remaining_to_spend,
                'remaining_to_spend_formatted'    => $this->_formatPrice($remaining_to_spend),
