...
/* List of possible Custom JS modifications:
const tc_initCarrierJs = false; // To disable carrier initialization and 'blink' effect, add Custom JS code
const tc_doNotPromoteBusinessFields = true;
let tc_disableAjaxErrorOverlay = true;
let tc_disableUpdateCartEmitEvent = true;
 */
...
function promoteBusinessAndPrivateFields() {
    if (typeof tc_doNotPromoteBusinessFields !== 'undefined' && tc_doNotPromoteBusinessFields === true) {
        // wrap every .form-group.business-field into own .business-fields-container
        $('.form-group.business-field').each(function() {
            if (!$(this).parent().hasClass('business-fields-container')) {
                $(this).wrap('<div class="business-fields-container"></div>');
            }
        });
        $('.business-fields-separator').remove();
...
function refetchShippingAddressBlock() {
    $.ajax({
        customPropAffectedBlocks: '#thecheckout-address-delivery',
        url: insertUrlParam('refetchAddressBlocks'), // used only to distinguish ajax call in dev tools, on server, post param 'action' is used
        type: 'POST',
        cache: false,
        dataType: "json",
        data: "&ajax_request=1&action=refetchAddressBlocks" +
            dynamicToken(),
        success: function (jsonData) {
            // if jsonData.addressesAreSame === 1, just hide delivery address block and untick 'use same address' checkbox
            if (jsonData.addressesAreSame === 1) {
                $('#thecheckout-address-delivery').hide();
                $('[data-link-action=x-ship-to-different-address]').prop('checked', false);
            } else {
                const formDeliveryAddressId = $('#thecheckout-address-delivery .inner-wrapper').attr('data-address-id');
                const jsonDeliveryAddressId = jsonData.id_address_delivery;
                // Modify the delivery address block only if the address ID has changed or the block is not visible
                const isAddressIdChanged = formDeliveryAddressId !== jsonDeliveryAddressId;
                const isDeliveryAddressBlockHidden = !$('#thecheckout-address-delivery').is(':visible');
                const addressCompanyChanged = jsonData.deliveryAddressObject?.company !== $('[data-address-type=delivery] [name=company]').val();
                if (isDeliveryAddressBlockHidden || isAddressIdChanged || addressCompanyChanged) {
                    updateAddressBlock('delivery', jsonData.deliveryAddressBlock);
                    $('#thecheckout-address-delivery').show();
                    $('[data-link-action=x-ship-to-different-address]').prop('checked', true);
                }
            }
        }
    });
...
function handleMondialRelayAddressChanges() {
    if ($('.delivery-option.mondialrelay:visible').length) {
        refetchShippingAddressBlock();
    }
}

// anytime shipping is modified (selectDeliveryOption), and mondialrelay carrier is visible, call refetchShippingAddressBlock
if (typeof window.mondialrelayWidget === 'object') {
    $(window.mondialrelayWidget).on("mondialrelay.saveSelectedRelay.success", refetchShippingAddressBlock);
    prestashop.on('thecheckout_updateDeliveryOption', handleMondialRelayAddressChanges)
}
...
function updatePaymentBlock(paymentModulesList, html, checksum, triggerElementName) {
    if ('undefined' !== typeof html && null !== html && (paymentBlockChecksum != checksum || 'thecheckout-confirm' == triggerElementName)) {
        ...
        // Special exception if 'stripejs' or 'everypay' is selected as payment method, do not update payment block on final confirmation
        const skipPaymentBlockUpdateModules = ['stripejs', 'everypay'];

        if (
            triggerElementName === 'thecheckout-confirm' &&
            payment.getSelectedOptionModuleName() &&
            skipPaymentBlockUpdateModules.some(prefix =>
                payment.getSelectedOptionModuleName().startsWith(prefix)
            )
        ) {
            console.log(payment.getSelectedOptionModuleName() + ' selected, skipping payment block update on confirmation');
            return;
        }
    ...
    if (!config_separate_payment) {
        ...
        // Hook to this:
        // prestashop.on('thecheckout_afterPaymentInit', function() { console.log('payment.init'); })
        prestashop.emit('thecheckout_afterPaymentInit', {
            reason: 'update',
        });
