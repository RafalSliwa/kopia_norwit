...
class purls extends Module
{
            ...
            if (defined('_PS_ADMIN_DIR_')) {
                ...
                if (Configuration::get('mypresta_updates') != 0 || (bool)Configuration::get('mypresta_updates') != false) {
                    ...
                    if (purlsUpdate::version($this->version) < purlsUpdate::version(Configuration::get('updatev_' . $this->name)) && Tools::getValue('ajax', 'false') == 'false') {
                        $warningMsg = '<strong>' . $this->displayName . '</strong>: ' . $this->l('New version available, check http://MyPresta.eu for more informations') . ' <a href="' . $this->mypresta_link . '">' . $this->l('More details in changelog') . '</a>';
                        if (isset($this->context->controller) && $this->context->controller !== null) {
                            $this->context->controller->warnings[] = $warningMsg;
                        }
    ...
    public static function getAnchor($id, $id_product_attribute, $with_id = false)
    {
        ...
        foreach ($attributes as &$a) {
            ...
            $anchor .= '/' . ($with_id && isset($element['id_attribute']) && $element['id_attribute'] ? $element['id_attribute_group'] . '|' . (int)$element['id_attribute'] . $sep : '') . Tools::str2url($element['group']) . $sep . Tools::str2url($element['name']);
        }
        return $anchor;
    }
