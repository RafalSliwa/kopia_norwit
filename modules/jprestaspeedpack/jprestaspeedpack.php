...
class Jprestaspeedpack extends Module
{
    ...
    public static $managed_controllers_default = array(
    ...
    public static $managed_controllers = null;
    public static $managed_object_classes = null;
     ...
     */
    protected static $cookies_to_preserve = array(
        // From Prestashop
     ...
     */
    private static $cookies_to_preserve_prefix = array(
        // From hicookie module
    ...
    public function __construct()
    {
        ...
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => '8.999.999');
        $this->bootstrap = true;
        ...
        if ((Tools::getIsset('logout') || Tools::getIsset('mylogout')) && JprestaUtils::getConfigurationOfCurrentShop('pagecache_logout_nocache')) {
    ...
    private static function getCacheInstance($id_shop, $type = null)
    {
        ...
        else if (strcmp('stdzip', $type) === 0 && PageCacheCacheZipFS::isCompatible()) {
...
'; // Let the new line here!
            ...
            if (method_exists('Cookie', 'getAll')) {
                ...
                foreach ($this->context->cookie->getAll() as $cookieName => $cookieValue) {
                    foreach (self::$cookies_to_preserve_prefix as $prefix) {
                        if (Tools::strpos($cookieName, $prefix) === 0) {
    ...
    private static function sendContextCookie() {
        ...
        if (!$cookieSent
            ...
            $cookieSent = true;
            if (self::getControllerName() == 'leoquicklogin__leocustomer') {
    ...
    private static function getCurrentObjectId() {
        ...
        if ($controllerName) {
            ...
            if ($controllerClassName && !class_exists($controllerClassName) && Tools::strpos($controllerName, '__') !== false) {
    ...
    public static function isMaintenanceEnabled() {
        ...
        if (!(int)Configuration::get('PS_SHOP_ENABLE')) {
            ...
            else if (in_array(Tools::getRemoteAddr(), $allowed_ips)) {
                $isMaintenanceEnabled = false;
            }
        }
    ...
    public static function displayCacheIfExists() {
        ...
        if (!headers_sent()) {
            if ((Tools::substr(self::getCurrentURL(), -Tools::strlen('hicookielaw/cookie')) === 'hicookielaw/cookie')
                || (Tools::substr(self::getCurrentURL(), -Tools::strlen('idxcookies/ajax')) === 'idxcookies/ajax')
            ) {
                // Delete the context
                setcookie('jpresta_cache_context', '', time() - 3600, '/');
            }
    ...
    private static function getJprestaCacheKeySpecifics() {
        ...
        if (JprestaUtils::isModuleEnabled('lgcookieslaw')) {
            if (Configuration::hasKey('PS_LGCOOKIES_COOKIE_NAME', 0, (int) Shop::getContextShopID(true), (int) Shop::getContextShopGroupID(true))) {
                ...
                $lgModule = Module::getInstanceByName('lgcookieslaw');
                if ($lgModule) {
                    if (!$lgModule->checkAccessGranted($context)) {
        ...
        if (JprestaUtils::isModuleEnabled('cookiesplus')) {
            ...
            if (Tools::version_compare($cookiesplus->version, '1.3', '>=')) {
                $cookieSource = (Tools::version_compare($cookiesplus->version, '1.5.2', '>=') || Configuration::get('C_P_COOKIE'))
                    ? 'cookies'
                    : 'psCookies';
            ...
            if (method_exists('GroupincConfiguration','isShowableBySchedule')) {
                ...
                if ($lastMd5 === false || $lastMd5 !== $md5) {
                    ...
                    foreach ($contexts as $id_ctx => $details) {
                        if (Tools::strpos($details, 'groupinc.configIds') !== false
                            && Tools::strpos($details, $md5) === false) {
        ...
        if (JprestaUtils::isModuleEnabled('iqitthemeeditor')) {
            $specifics->keepPsCookie('product_list_view');
        }
    ...
    {
        // Retrieve the Base64 encoded configuration from the database
        $encodedConf = JprestaUtils::getConfigurationByShopId('pagecache_cachekey_usergroups', $shopId, '');
        if (!$encodedConf) {
            return [];
        }

        // Decode the Base64 string to get the compressed data
        $compressedConf = @base64_decode($encodedConf, true);
        if ($compressedConf === false) {
            JprestaUtils::addLog(
                "Bad Base64 configuration value for pagecache_cachekey_usergroups for shop#$shopId: " . $encodedConf,
                2
            );
            return [];
        }
        ...
        if ($decompressedConf === false) {
            JprestaUtils::addLog(
                "Bad compressed configuration value for pagecache_cachekey_usergroups for shop#$shopId: " . $compressedConf,
                2
            );
            return [];
        }

        // Deserialize the data to get the configuration array
        $currentConf = @unserialize($decompressedConf);
        if ($currentConf === false || !is_array($currentConf)) {
            JprestaUtils::addLog(
                "Bad decompressed configuration value for pagecache_cachekey_usergroups for shop#$shopId: " . $decompressedConf,
                2
            );
            return [];
        }
    ...
    public static function saveCacheKeyForUserGroups($shopId, $conf)
    {
        ...
        $serializedConf = serialize($conf);

        // Compress the serialized data using GZIP
        $compressedConf = gzcompress($serializedConf, 9);
        if ($compressedConf === false) {
            JprestaUtils::addLog(
                "Failed to compress configuration for pagecache_cachekey_usergroups for shop#$shopId",
                2
            );
            return false;
        }
            ...
            if ($hookType === self::HOOK_TYPE_MODULE) {
                ...
                if (JprestaUtils::isModuleEnabled('cookiesplus')) {
                    ...
                    if ($cookiesPlus && method_exists($cookiesPlus, "blockModuleCode")) {
                        $cookiesPlus->blockModuleCode(array(
                            'display' => &$hookValue,
        ...
        if ($controller == null) {
            ...
            if (isset($controllers[Tools::strtolower($controller_name)])) {
                ...
                if ($context->controller) {
                    ...
                    if (JprestaUtils::isModuleEnabled('smartseourl') && in_array($controller_class, array('ProductController', 'CategoryController', 'CmsController', 'SupplierController', 'ManufacturerController'))) {
    ...
    private static $lastUpdatedProductFeatures = null;
    private static $lastUpdatedProductStockAvailable = null;
    ...
    public function installTabs()
    {
        if ($this->isSpeedPack()) {
            ...
            $this->installTab('AdminJprestaSQLProfilerConfiguration', array(
                'en' => 'SQL Profiler',
                'fr' => 'Profilage SQL',
                'es' => 'SQL Profiler'
            ), (int)Tab::getIdFromClassName('AdminParentSpeedPack'));
    ...
    public static function canBeMarkedAsDynamic($hook_name) {
        ...
        return !(
            (Tools::strpos($hookName, 'action') === 0 && strcmp($hookName, 'actionproductoutofstock') !== 0)
            || Tools::strpos($hookName, 'dashboard') === 0
            || Tools::strpos($hookName, 'displayadmin') === 0
            || Tools::strpos($hookName, 'displaybackoffice') === 0
            || in_array($hookName, [
                'header',
                'displayheader',
