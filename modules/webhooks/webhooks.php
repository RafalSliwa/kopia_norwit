...
class Webhooks extends Module
{
    ...
    public function getContent()
    {
        ...
        // Save settings
        if (((bool) Tools::isSubmit('submitWebhooksSettingsModule')) == true) {
            Configuration::updateValue('WEBHOOKS_CLEAN_EXECUTION_LOGS', (int) Tools::getValue('WEBHOOKS_CLEAN_EXECUTION_LOGS'));
        ...
        if (((bool) Tools::isSubmit('createWebhook')) == true
            ...
            $output .= $this->renderConfigFormSettings();
    ...
    protected function renderConfigFormSettings()
    {
        $helper = new HelperForm();
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'submitWebhooksSettingsModule';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false)
            . '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        ...
        return $helper->generateForm([$this->getConfigFormSettings()]);
    }

    protected function getConfigFormSettingsValues()
    {
        return [
            'WEBHOOKS_CLEAN_EXECUTION_LOGS' => (int) Tools::getValue('WEBHOOKS_CLEAN_EXECUTION_LOGS', Configuration::get('WEBHOOKS_CLEAN_EXECUTION_LOGS')),
        ];
    }
        ...
        return [
            'form' => [
                ...
                'input' => [
                    [
                        'type' => 'select',
                        'label' => $this->l('Clear execution logs older than'),
                        'name' => 'WEBHOOKS_CLEAN_EXECUTION_LOGS',
                        'desc' => $this->l('Automatically clear execution logs'),
                        'options' => [
                            'id' => 'id_option',
                            'name' => 'name',
                            'query' => $options,
    ...
    protected function createNewWebhook()
    {
        ...
        if (empty($url) || !filter_var($url, FILTER_VALIDATE_URL)) {
            ...
            !Validate::isUnsignedInt(Tools::getValue('WEBHOOKS_RETRIES'))
    ...
    {
        $webhooks = WebhookModel::getWebhooksByHook('actionPasswordRenew');
    ...
    public function hookActionCronJob()
    {
        if (Module::isInstalled($this->name)) {
            $clear_logs = Configuration::get('WEBHOOKS_CLEAN_EXECUTION_LOGS');

            if ($clear_logs > 0) {
                WebhookLogModel::clearLogs($clear_logs);
            }
