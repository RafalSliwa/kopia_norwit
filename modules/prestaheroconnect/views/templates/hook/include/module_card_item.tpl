...
>
    ...
    <div class="item-wrapper">
        ...
        <div class="module-desc">
            <span>{$item.description nofilter}</span>
        ...
        <div class="module-pos-bottom">
            <div class="module-compliancy">
                {if (isset($item.min_ps_version) && $item.min_ps_version) || (isset($item.max_ps_version) && $item.max_ps_version)}
                    ...
                    {/if}
                {else}
                    <span><i class="ets_connect_ps_icon"></i> {l s='Compatible with: 1.6.x - 8.x' mod='prestaheroconnect'}</span>
                {/if}
            </div>
                ...
                {else}
                    {if $item.to_buy}
                        {if !$item.price_number}
                            ...
                            {/if}
                        {else}
                            <a href="{$item.uri|escape:'quotes':'UTF-8'}&utm_medium=buynow&utm_campaign={str_replace([' ',',','/','\.','`','(',')','&'],'+',$item.displayName)|escape:'html':'UTF-8'}" target="_blank"
