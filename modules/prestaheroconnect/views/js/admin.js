            ...
            success: function (res) {
                Object.keys(res).forEach(function (key) {
                    ...
                    if (response.status) {
                        ...
                        else if (typeof response.action_menu_html !== 'undefined') {
                            if ($(btn).closest('.btn-group.module-actions').length)
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    if (modules.length) {
                        ...
                        $('.js-ph-con-tab-item[data-tab="purchased"]').parent('li').removeClass('hide').addClass('active');
                        $('.ph-con-list-content-modules').attr('data-active','purchased');
                        $('.js-ph-con-tab-item[data-tab="installed"]').parent('li').removeClass('active');
                    ...
                    {
                        $('.ph-con-list-content-modules').attr('data-active','installed');
                    }
                }
            },
            error: function (xhr) {
...
$(document).ready(function () {
    $(document).on('click','.btn-search',function(){
        $('#ph-con-box-search-module').show();
        $(this).parent().addClass('show-input');
        $('.ph-con-list-tabs.show_tab_hidden').removeClass('show_tab_hidden');
        return false;
    });
    ...
    $(document).on('submit', '.js-ph-con-group-btn-action form', function () {
        ...
        var actionDelete = $(this).find('button[type=submit]').hasClass('module_action_menu_delete');
        ...
        if (linkAction) {
            if (!PH_CON_IS_OLD_ACTION || $(this).hasClass('form_upgrade_from_ph'))
                phConAdmin.actionModule($(this).find('button[type=submit]')[0], linkAction,false,actionDelete);
            else
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    if(!res.isLogged)
                    {
                        phConGlobal.getModalLogin($this,res.message);
                    }
                    else
    ...
    $('#ph-con-box-search-module').keyup(phConAdmin.delay(function () {
        ...
        if (textSearch) {
            $('.ph-con-list-search-form').addClass('finding');
            ...
            $('.ph-con-list-search-form').removeClass('finding');
        }
        if($('.ph-con-list-content-modules .ph-con-list-modules-item:not(:hidden)').length==0)
            $('.ph-con-not-found-item').removeClass('hide');
        else
    ...
    $(document).on('click', '.clear_value', function () {
    ...
    $(document).on('click','.ph-con-list-search-form button',function(e){
       if ( $(this).parent().hasClass('active') ){
           $(this).parent().removeClass('active');
       }else{
           $(this).parent().addClass('active');
           $('.ph-con-list-search-form input[type="text"]').trigger("focus");

           setTimeout(function() {
               $('.ph-con-list-search-form input[type="text"]').focus();
           }, 500);
       }
    });
    $(document).mouseup(function (e)
    {
        var container_block_search = $('.ph-con-list-search-form.active');
        if (!container_block_search.is(e.target)&& container_block_search.has(e.target).length === 0)
        {
            $('.ph-con-list-search-form.active').removeClass('active');
            $('.ph-con-list-search-form').removeClass('show-input');
        }
    });
    autorespontab();
    ...
    $(document).on('click','#nav-sidebar span.menu-collapse',function(e){
        setTimeout(function(e){
            autorespontab();
        },200);

    });
    $(document).on('click','.dot_three',function(e){
        $(this).parent('.ph-con-list-tabs').toggleClass('show_tab_hidden');
    });
    $(document).on('click','.list-tabs > li:not(.hiddenTab)',function(e){
        $('.ph-con-list-tabs.show_tab_hidden').removeClass('show_tab_hidden');
    });
    $(document).mouseup(function (e)
    {
        var container_block_tab = $('.ph-con-list-tabs.show_tab_hidden');
        if (!container_block_tab.is(e.target)&& container_block_tab.has(e.target).length === 0)
        {
            $('.ph-con-list-tabs.show_tab_hidden').removeClass('show_tab_hidden');
        }
    });
});
function autorespontab(){
    $('.ph-con-list-tabs ul.list-tabs li').removeClass('hiddenTab');
    $('.ph-con-list-tabs').removeClass('has_three_dots');
    var _widthFull = $('.ph-con-list-tabs').width() - 60;
    var _widthTab = 0;
    $('.ph-con-list-tabs ul.list-tabs li').each(function(e){
        _widthTab = _widthTab + $(this).outerWidth() + 20;
        if ( _widthTab > _widthFull ){
            $(this).addClass('hiddenTab');
            $('.ph-con-list-tabs').addClass('has_three_dots');
        }
        if ( $('.ph-con-list-tabs ul.list-tabs li').hasClass('hide')){
