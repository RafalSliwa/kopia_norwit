...
/**
 * Class AdminPhConListModulesController
 * @property $module Prestaheroconnect;
 */
class AdminPhConListModulesController extends ModuleAdminController
{
    ...
    public function renderList()
    {
        ...
        foreach ($phModules as $item) {
            ...
            if ($isModule) {
                if(isset($item['price_number']) && !$item['price_number'])
                    $moduleCounter['free'] += 1;
                else
                    $moduleCounter['premium'] += 1;
            }
            if (!$isModule) {
        ...
        $desc =($description = Configuration::get('PH_CON_DESCRIPTION', $this->context->language->id)) ? $description : Configuration::get('PH_CON_DESCRIPTION', $id_lang_default);
        if(!$desc)
        {
            $desc = $this->module->getDescDefault();
        }
        $smartyParams = array(
            'phModules' => $phModules,
            ...
            'userNameLogin' => PhConApi::getApi()->isLogged() && ($userToken = PhConEmployeeToken::getToken($this->context->employee->id)) ? $userToken->firstname.' '.$userToken->lastname:false,
