...
class PhConApi
{
    ...
    public function __construct()
    {
        ...
        if (!(isset($this->context->cookie->phToken) && $this->context->cookie->phToken) || !(isset($this->context->cookie->phUserId) && $this->context->cookie->phUserId) || !isset($this->context->cookie->phUserName) || !(isset($this->context->cookie->phIdEmployee) && $this->context->cookie->phIdEmployee)) {
        ...
        if ($this->context->cookie->phIdEmployee && $this->context->employee->id == (int)$this->context->cookie->phIdEmployee && ($token = PhConEmployeeToken::getToken($this->context->employee->id))) {
            $this->userId = (int)$this->context->cookie->phUserId;
            $this->token = $this->context->cookie->phToken;
            $this->userName =trim(($token->firstname ? Tools::strtoupper(Tools::substr($token->firstname, 0, 1)) : '') . '.' . Tools::ucfirst($token->lastname));
        }
    ...
    public function login($email, $password, $rememberMe = false)
    {
        ...
        if (($res = $this->request($this->urlCheckLogin, array('email' => $email, 'password' => $password))) && ($json = json_decode($res, true))
            ...
            if ($rememberMe) {
                ...
                $token->email = isset($json['email']) ? $json['email'] : null;
