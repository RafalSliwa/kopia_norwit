        ...
        if ($modules = PhConApi::getApi()->getModulesOnMarket(false)) {
            foreach ($modules as $item) {
                if (isset($item['is_module']) && (int)$item['is_module'] > 0) {
                    ...
                    if(!Module::isInstalled($item['name']) && file_exists(_PS_MODULE_DIR_ . $item['name'] . '/' . $item['name'] . '.php'))
                    {
                        include_once _PS_MODULE_DIR_ . $item['name'] . '/' . $item['name'] . '.php';
                        $module = new $item['name']();
                        $newestVersion = $module->version;
                    }
                    else
                    ...
                    $result[$item['name']] = array(
                        ...
                        'upgrade_from_server' => self::canUpgradeFromPrestahero($item['name'], $item['version']), //(!isset($item['upgrade_from_server']) || (isset($item['upgrade_from_server']) && $item['upgrade_from_server']))
