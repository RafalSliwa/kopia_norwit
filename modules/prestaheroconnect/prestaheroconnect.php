...
$this->refs = 'https://prestahero.com/';
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        $loginApi = PhConApi::getApi();
        $token = PhConEmployeeToken::getToken($this->context->employee->id);
        $this->smarty->assign(array(
            ...
            'accountName' => $loginApi->isLogged() ? $loginApi->userName : null,
            'accountEmail' => $token ? $token->email : null,
            ...
            'isLogged' => $loginApi->isLogged() ? true : false,
        ));
    ...
    public function displayDashboardToolbarTopMenu()
    {
    ...
    public function getContent()
    {
        ...
        if(Tools::isSubmit('phConCheckAccountSocial'))
        {
            $social = Tools::getValue('social');
            $errors = array();
            if(!$social)
                $errors[] = $this->l('Social network is required');
            elseif(!in_array($social,['Google','Facebook']))
                $errors[] = $this->l('Social network is invalid');
            if(!$errors)
            {
                $token_social = Tools::passwdGen(15);
                ...
                $token->token_social = $token_social;
                $token->token_social_expire_at = date('Y-m-d H:i:s', strtotime('+30 MINUTES'));
                if($token->save(true))
                {
                    die(json_encode(array(
                        'success' =>  true ,
                        'link_login' =>_PH_CON_URL_SOCIAL_LOGIN_.'?social='.$social.'&id_employee='.(int)$this->context->employee->id.'&token='.$token_social.'&back_link='.urlencode($this->context->link->getModuleLink('prestaheroconnect','login')),
                    )));
                }
                else
                    $errors[] = $this->l('An error occurred while saving token');
        ...
        if(Tools::isSubmit('phConCheckAccountLoginSocial'))
        {
            if (($token = PhConEmployeeToken::getToken($this->context->employee->id)) && $token->token && $token->id_user && (!$token->token_expire_at || $token->token_expire_at == '0000-00-00 00:00:00' || strtotime($token->token_expire_at) >= time())) {
                ...
                die(json_encode(array(
                    ...
                    'message' =>  $this->l('Authenticated successfully'),
                )));
            }
            die(json_encode(array(
                'success' => false,
    ...
    {
        $smartyParams['version_connect'] = $this->version;
        $link_prestatahero ="https://prestahero.com/?utm_source=prestaheroconnect&utm_medium=poweredby";
        if($this->context->language->iso_code=='fr')
            $link_prestatahero="https://prestahero.com/fr/?utm_source=prestaheroconnect&utm_medium=poweredby";
        if($this->context->language->iso_code=='es')
            $link_prestatahero="https://prestahero.com/es/?utm_source=prestaheroconnect&utm_medium=poweredby";
        if($this->context->language->iso_code=='it')
            $link_prestatahero="https://prestahero.com/it/?utm_source=prestaheroconnect&utm_medium=poweredby";
        if($this->context->language->iso_code=='vn')
            $link_prestatahero="https://prestahero.com/vn/?utm_source=prestaheroconnect&utm_medium=poweredby";
        $smartyParams['link_prestahero'] = $link_prestatahero;
        $this->smarty->assign($smartyParams);
        return $this->display(__FILE__, 'list_modules.tpl');
    }
    public function getDescDefault()
    {
        return $this->display(__FILE__, 'desc_default.tpl');
    }
