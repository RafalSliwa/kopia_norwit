    ...
    $(document).ready(function () {
        $.ajax({
            ...
            success: function (json) {
                ...
                if ($('.AdminContactFormUltimateMessage a span.count_messages').length > 0)
                    $('.AdminContactFormUltimateMessage a span.count_messages').html(json.count);
                else
                    $('.AdminContactFormUltimateMessage a ').append('<span class="count_messages">' + json.count + '</span>');
                if (parseInt(json.count) < 1) {
                    $('.AdminContactFormUltimateMessage a span.count_messages').addClass('hide');
                }
            },
        });
    });
