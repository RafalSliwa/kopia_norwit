...
class ETS_CFU_Form_Tag implements ArrayAccess
{
    public $type;
    public $basetype;
    public $name = '';
    ...
    public function __construct($tag = array())
    {
        ...
        {
            static $global_capcha_count = 0;
            $global_capcha_count += 1;
            $form_unit_tag = 'form-'.(Tools::isSubmit('form_unit_tag') ? (int)Tools::getValue('form_unit_tag') : $global_capcha_count);
            if(Tools::strpos($this->name,$form_unit_tag)!==0)
                $this->name = $form_unit_tag.$this->name;
        }
    }
    ...
    public function get_default_option($default = '', $args = '')
    {
        ...
        if (isset($context->customer))
        {
            if ($this->type == 'email')
            {
                $default = str_replace('default:user_email', $context->customer->email, $default);
            }
            elseif ($this->type == 'tel')
            {
                $phone_number = '';
                if ($id_address = Address::getFirstCustomerAddressId($context->customer->id))
                {
                    $address = new Address($id_address);
                    $phone_number = $address->phone? $address->phone : $address->phone_mobile;
                }
                $default = str_replace('default:user_phone', $phone_number, $default);
            }
            else
            {
                $customer = new Customer($context->customer->id);
                $address = $customer->getAddresses($context->language->id);
                $phone_number = '';
                if ( $total_address = count($address) ){
                    for( $i = 0; $i < $total_address; $i ++){
                        if ( $address[$i]['phone'] || $address[$i]['phone_mobile'] ){
                            $phone_number = $address[$i]['phone'] ? $address[$i]['phone'] : $address[$i]['phone_mobile'];
                            break;
                        }
                    }
                }
                ...
                $default = str_replace(
                    array('default:user_email', 'default:user_first_name', 'default:user_last_name', 'default:user_full_name','default:user_phone'),
                    array($context->customer->email, $context->customer->firstname, $context->customer->lastname, $context->customer->firstname . ' ' . $context->customer->lastname,$phone_number),
                    $default
                );
            }
        }
