<?php
/**
 * Copyright ETS Software Technology Co., Ltd
 *
 * NOTICE OF LICENSE
 *
 * This file is not open source! Each license that you purchased is only available for 1 website only.
 * If you want to use this file on more websites (or projects), you need to purchase additional licenses.
 * You are not allowed to redistribute, resell, lease, license, sub-license or offer our resources to any third party.
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade PrestaShop to newer
 * versions in the future.
 *
 * @author ETS Software Technology Co., Ltd
 * @copyright  ETS Software Technology Co., Ltd
 * @license    Valid for 1 website (or project) for each purchase of license
 */

if (!defined('_PS_VERSION_')) { exit; }

class ETS_CFU_Contact extends ObjectModel
{
    public static $definition = array(
        'table' => 'ets_cfu_contact',
        'primary' => 'id_contact',
        'multilang' => true,
        'fields' => array(
            'email_to' => array('type' => self::TYPE_HTML),
            'bcc' => array('type' => self::TYPE_HTML),
            'active' => array('type' => self::TYPE_INT),
            'email_from' => array('type' => self::TYPE_HTML),
            'additional_headers' => array('type' => self::TYPE_HTML),
            'exclude_lines' => array('type' => self::TYPE_INT),
            'use_html_content' => array('type' => self::TYPE_INT),
            'save_message' => array('type' => self::TYPE_INT),
            'star_message' => array('type' => self::TYPE_INT),
            'save_attachments' => array('type' => self::TYPE_INT),
            'hook' => array('type' => self::TYPE_HTML),
            'open_form_by_button' => array('type' => self::TYPE_INT),
            'button_popup_enabled' => array('type' => self::TYPE_INT),
            'button_popup_position' => array('type' => self::TYPE_STRING),
            'button_popup_left' => array('type' => self::TYPE_INT),
            'button_popup_right' => array('type' => self::TYPE_INT),
            'button_popup_top' => array('type' => self::TYPE_INT),
            'button_popup_bottom' => array('type' => self::TYPE_INT),
            'floating_label' => array('type' => self::TYPE_HTML, 'lang' => true),
            'floating_text_color' => array('type' => self::TYPE_STRING),
            'floating_background_color' => array('type' => self::TYPE_STRING),
            'floating_hover_color' => array('type' => self::TYPE_STRING),
            'floating_background_hover_color' => array('type' => self::TYPE_STRING),
            'floating_icon_enabled' => array('type' => self::TYPE_BOOL),
            'floating_icon_custom' => array('type' => self::TYPE_STRING),
            'floating_icon_custom_file' => array('type' => self::TYPE_STRING),
            'button_label' => array('type' => self::TYPE_HTML, 'lang' => true),
            'button_text_color' => array('type' => self::TYPE_STRING),
            'button_background_color' => array('type' => self::TYPE_STRING),
            'button_hover_color' => array('type' => self::TYPE_STRING),
            'button_background_hover_color' => array('type' => self::TYPE_STRING),
            'button_icon_enabled' => array('type' => self::TYPE_BOOL),
            'button_icon_custom' => array('type' => self::TYPE_STRING),
            'button_icon_custom_file' => array('type' => self::TYPE_STRING),
            'id_employee' => array('type' => self::TYPE_INT),
            'title' => array('type' => self::TYPE_HTML, 'lang' => true),
            'short_code' => array('type' => self::TYPE_HTML, 'lang' => true),
            'render_form' => array('type' => self::TYPE_HTML),
            'condition' => array('type' => self::TYPE_HTML),
            'subject' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_body' => array('type' => self::TYPE_HTML, 'lang' => true),
            'file_attachments' => array('type' => self::TYPE_HTML),
            'use_email2' => array('type' => self::TYPE_INT),
            'email_to2' => array('type' => self::TYPE_HTML),
            'bcc2' => array('type' => self::TYPE_HTML),
            'email_from2' => array('type' => self::TYPE_HTML),
            'subject2' => array('type' => self::TYPE_HTML, 'lang' => true),
            'additional_headers2' => array('type' => self::TYPE_HTML),
            'message_body2' => array('type' => self::TYPE_HTML, 'lang' => true),
            'exclude_lines2' => array('type' => self::TYPE_INT),
            'use_html_content2' => array('type' => self::TYPE_INT),
            'file_attachments2' => array('type' => self::TYPE_HTML),
            'message_mail_sent_ok' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_mail_sent_ng' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_validation_error' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_spam' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_accept_terms' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_required' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_too_long' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_too_short' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_date_too_early' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_date_too_late' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_date' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_upload_failed' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_upload_file_type_invalid' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_upload_file_too_large' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_quiz_answer_not_correct' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_email' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_url' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_tel' => array('type' => self::TYPE_HTML, 'lang' => true),
            'additional_settings' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_upload_failed_php_error' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_invalid_number' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_number_too_small' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_number_too_large' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_captcha_not_match' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_ip_black_list' => array('type' => self::TYPE_HTML, 'lang' => true),
            'message_email_black_list' => array('type' => self::TYPE_HTML, 'lang' => true),
            'template_mail' => array('type' => self::TYPE_HTML, 'lang' => true),
            'title_alias' => array('type' => self::TYPE_HTML, 'lang' => true),
            'meta_title' => array('type' => self::TYPE_HTML, 'lang' => true),
            'meta_keyword' => array('type' => self::TYPE_HTML, 'lang' => true),
            'meta_description' => array('type' => self::TYPE_HTML, 'lang' => true),
            'enable_form_page' => array('type' => self::TYPE_INT),
            'group_access' => array('type' => self::TYPE_HTML),
            'position' => array('type' => self::TYPE_INT),

            'thank_you_active' => array('type' => self::TYPE_INT),
            'thank_you_page' => array('type' => self::TYPE_HTML),
            'thank_you_message' => array('type' => self::TYPE_HTML, 'lang' => true),
            'thank_you_url' => array('type' => self::TYPE_HTML, 'lang' => true),
            'thank_you_alias' => array('type' => self::TYPE_HTML, 'lang' => true),
            'thank_you_page_title' => array('type' => self::TYPE_HTML, 'lang' => true),

            'mailchimp_enabled' => array('type' => self::TYPE_BOOL),
            'mailchimp_api_key' => array('type' => self::TYPE_STRING, 'size' => 255),
            'mailchimp_audience' => array('type' => self::TYPE_STRING, 'size' => 128),
            'mailchimp_mapping_data' => array('type' => self::TYPE_STRING),

            'date_add' => array('type' => self::TYPE_DATE),
            'date_upd' => array('type' => self::TYPE_DATE),
        ),
    );

    public $title;
    public $short_code;
    public $render_form;
    public $condition;
    public $email_to;
    public $bcc;
    public $bcc2;
    public $email_from;
    public $subject;
    public $additional_headers;
    public $message_body;
    public $exclude_lines;
    public $use_html_content;
    public $file_attachments;
    public $hook;
    public $template_mail;
    public $use_email2;
    public $email_to2;
    public $email_from2;
    public $subject2;
    public $additional_headers2;
    public $message_body2;
    public $exclude_lines2;
    public $use_html_content2;
    public $file_attachments2;
    public $message_mail_sent_ok;
    public $message_mail_sent_ng;
    public $message_validation_error;
    public $message_spam;
    public $message_accept_terms;
    public $message_invalid_required;
    public $message_invalid_too_long;
    public $message_invalid_too_short;
    public $message_invalid_date;
    public $message_date_too_early;
    public $message_date_too_late;
    public $message_upload_failed;
    public $message_upload_file_type_invalid;
    public $message_upload_file_too_large;
    public $message_quiz_answer_not_correct;
    public $message_invalid_email;
    public $message_invalid_url;
    public $message_invalid_tel;
    public $additional_settings;
    public $save_message;
    public $star_message;
    public $save_attachments;
    public $open_form_by_button;
    public $button_popup_enabled;
    public $button_popup_position;
    public $button_popup_left;
    public $button_popup_right;
    public $button_popup_top;
    public $button_popup_bottom;
    public $floating_label;
    public $floating_text_color;
    public $floating_background_color;
    public $floating_hover_color;
    public $floating_background_hover_color;
    public $floating_icon_enabled;
    public $floating_icon_custom;
    public $floating_icon_custom_file;
    public $button_label;
    public $button_text_color;
    public $button_background_color;
    public $button_hover_color;
    public $button_background_hover_color;
    public $button_icon_enabled;
    public $button_icon_custom;
    public $button_icon_custom_file;
    public $id_employee;
    public $date_add;
    public $date_upd;
    public $message_upload_failed_php_error;
    public $message_invalid_number;
    public $message_number_too_small;
    public $message_number_too_large;
    public $message_captcha_not_match;
    public $message_ip_black_list;
    public $message_email_black_list;
    public $title_alias;
    public $meta_title;
    public $meta_keyword;
    public $meta_description;
    public $enable_form_page;
    public $position;
    public $active;
    public $group_access;
    public $thank_you_active;
    public $thank_you_page;
    public $thank_you_message;
    public $thank_you_url;
    public $thank_you_alias;
    public $thank_you_page_title;
    public $mailchimp_enabled;
    public $mailchimp_api_key;
    public $mailchimp_audience;
    public $mailchimp_mapping_data;

    public function __construct($id_item = null, $id_lang = null, $id_shop = null)
    {
        parent::__construct($id_item, $id_lang, $id_shop);
    }

    public static function getContactFloating()
    {
        $dq = new DbQuery();
        $dq
            ->select('COUNT(id_contact)')
            ->from('ets_cfu_contact')
            ->where('button_popup_enabled > 0');

        return Db::getInstance()->getValue($dq);
    }

    public static function getIdContactByAlias($alias, $id_lang = null, $thank_page = false)
    {
        $context = Context::getContext();
        if (!$id_lang) {
            $id_lang = $context->language->id;
        }
        $cacheId = 'ETS_CFU_Contact::getIdContactByAlias' . md5($alias . ($id_lang != null ? $id_lang : 0) . (int)$thank_page);
        if (!Cache::isStored($cacheId)) {
            $result = (int)Db::getInstance()->getValue("
                SELECT c.id_contact FROM `" . _DB_PREFIX_ . "ets_cfu_contact` c
                LEFT JOIN `" . _DB_PREFIX_ . "ets_cfu_contact_lang` cl ON (c.id_contact = cl.id_contact AND cl.id_lang = " . (int)$id_lang . ")
                INNER JOIN `" . _DB_PREFIX_ . "ets_cfu_contact_shop` cs ON (c.id_contact = cs.id_contact AND cs.id_shop = " . (int)$context->shop->id . ")
                WHERE " . ($thank_page ? 'cl.thank_you_alias' : 'cl.title_alias') . "  LIKE '%" . pSQL($alias) . "%'
            ");
            Cache::store($cacheId, $result);
        } else {
            $result = Cache::retrieve($cacheId);
        }
        return $result;
    }

    public function add($autodate = true, $null_values = false)
    {
        $context = Context::getContext();
        $id_shop = $context->shop->id;
        $res = parent::add($autodate, $null_values);
        $res &= Db::getInstance()->execute('
			INSERT INTO `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` (`id_shop`, `id_contact`)
			VALUES(' . (int)$id_shop . ', ' . (int)$this->id . ')'
        );
        return $res;
    }

    public static function getGroupAccessById($id_contact, $id_lang = null)
    {
        $context = Context::getContext();
        if (!$id_lang) {
            $id_lang = $context->language->id;
        }
        $sql = 'SELECT c.group_access FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` c 
                LEFT JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_lang` cl ON (c.id_contact = cl.id_contact AND cl.id_lang = ' . (int)$id_lang . ' ) 
                 INNER JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON (c.id_contact = cs.id_contact AND cs.id_shop = ' . (int)$context->shop->id . ' ) 
                  WHERE c.id_contact = ' . (int)$id_contact . ' ';
        if ($rest = DB::getInstance()->getValue($sql)) {
            return array_map('intval', explode(',', $rest));
        } else {
            return array();
        }
    }

    public static function getPosition($id_shop)
    {
        return (int)Db::getInstance()->getValue('SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` where id_shop=' . (int)$id_shop);
    }

    public static function updateSaveMessage($id_contact, $save_message)
    {
        return Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'ets_cfu_contact` SET save_message="' . (int)$save_message . '" WHERE id_contact=' . (int)$id_contact);
    }

    public static function deleteContactShop($id_contact)
    {
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` WHERE id_contact=' . (int)$id_contact);
    }

    public static function getContacts($id_shop, $select = null, $orderBy = null, $orderWay = null)
    {
        return Db::getInstance()->executeS('
            SELECT ' . ($select ? pSQL($select) : '*') . ' FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` c 
            JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON c.id_contact=cs.id_contact
            WHERE  id_shop=' . (int)$id_shop . ' 
            ' . ($orderBy ? 'ORDER BY ' . pSQL($orderBy) . ($orderWay ? pSQL($orderWay) : ' ASC') : '') . '
        ');
    }

    public static function getContactsAllShop($context = null)
    {
        if ($context == null)
            $context = Context::getContext();

        return Db::getInstance()->executeS('
            SELECT * FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` c 
            LEFT JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_lang` cl ON c.id_contact=cl.id_contact AND cl.id_lang=' . (int)$context->language->id
        );
    }

    public static function getContactsLang($id_contact)
    {
        return Db::getInstance()->executeS('
            SELECT cl.*, l.iso_code 
            FROM `' . _DB_PREFIX_ . 'ets_cfu_contact_lang` cl JOIN `' . _DB_PREFIX_ . 'lang` l ON cl.id_lang=l.id_lang
            WHERE cl.id_contact=' . (int)$id_contact
        );
    }

    public static function updatePosition($position, $id_contact)
    {
        return Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'ets_cfu_contact` set position="' . (int)$position . '" WHERE id_contact=' . (int)$id_contact);
    }

    public static function setActive($id_contact, $active)
    {
        return Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'ets_cfu_contact` SET active="' . (int)$active . '" WHERE id_contact=' . (int)$id_contact);
    }

    public static function getContactsByHook($hook, $context = null)
    {
        if ($context == null)
            $context = Context::getContext();

        return Db::getInstance()->executeS('
            SELECT c.id_contact FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` c
            INNER JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON (c.id_contact= cs.id_contact)
            LEFT JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_lang` cl on (c.id_contact= cl.id_contact AND cl.id_lang="' . (int)$context->language->id . '")
            WHERE c.active=1 
                AND c.hook LIKE "%' . pSQL($hook) . '%" 
                AND cs.id_shop="' . (int)$context->shop->id . '";
        ');
    }

    public static function getContactsInPage($id_contact, $context = null)
    {
        if ($context == null)
            $context = Context::getContext();

        return Db::getInstance()->executeS('
            SELECT c.id_contact 
            FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` c
            INNER JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON (c.id_contact= cs.id_contact)
            LEFT JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_lang` cl on (c.id_contact= cl.id_contact AND cl.id_lang="' . (int)$context->language->id . '")
            WHERE c.active=1 
                AND c.button_popup_enabled = 1 
                AND cs.id_shop="' . (int)$context->shop->id . '"
                ' . ($id_contact > 0 ? 'AND c.id_contact != ' . (int)$id_contact : '') . '
        ');
    }

    public static function getContactById($id_contact, $id_shop)
    {
        return Db::getInstance()->executeS('
            SELECT * FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` c 
            JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON c.id_contact=cs.id_contact 
            WHERE c.id_contact="' . (int)$id_contact . '" AND cs.id_shop="' . (int)$id_shop . '"
        ');
    }

    public static function deleteAll($id_shop)
    {
        Db::getInstance()->execute("DELETE FROM `" . _DB_PREFIX_ . "ets_cfu_contact` WHERE id_contact IN (SELECT id_contact FROM `" . _DB_PREFIX_ . "ets_cfu_contact_shop` WHERE id_shop=" . (int)$id_shop . ")");
        Db::getInstance()->execute("DELETE FROM `" . _DB_PREFIX_ . "ets_cfu_contact_lang` WHERE id_contact IN (SELECT id_contact FROM `" . _DB_PREFIX_ . "ets_cfu_contact_shop` WHERE id_shop=" . (int)$id_shop . ")");
        Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` WHERE id_shop="' . (int)$id_shop . '"');

        return true;
    }

    public static function getMaxId()
    {
        $req = 'SELECT max(`id_contact`) as maxid
			FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` tbl';
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($req);
        return isset($row['maxid']) ? (int)$row['maxid'] : 0;
    }

    public static function checkAliasExit($alias, $id_lang, $id_contact = false, $context = null)
    {
        if ($context == null)
            $context = Context::getContext();
        if (!$id_lang) {
            $id_lang = $context->language->id;
        }

        return Db::getInstance()->getValue("
            SELECT c.id_contact FROM `" . _DB_PREFIX_ . "ets_cfu_contact` c
            LEFT JOIN `" . _DB_PREFIX_ . "ets_cfu_contact_lang` cl ON (c.id_contact = cl.id_contact AND cl.id_lang = " . (int)$id_lang . ")
            INNER JOIN `" . _DB_PREFIX_ . "ets_cfu_contact_shop` cs ON (c.id_contact = cs.id_contact AND cs.id_shop = " . (int)$context->shop->id . ")
            WHERE cl.`thank_you_alias` LIKE '%" . pSQL($alias) . "%' " . ($id_contact ? " AND c.`id_contact` != " . (int)$id_contact . " " : "") . "
            GROUP BY c.id_contact
        ");
    }

    public static function getContactList($active = false, $filters = '', $start = 0, $limit = 0, $count = false, $order_by = 'position', $order_way = 'asc')
    {
        $context = Context::getContext();
        $sql = '
            SELECT ' . ($count ? 'COUNT(DISTINCT c.id_contact)' : 'c.*,cl.*,cs.*,e.firstname,e.lastname') . '
            FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` c
            INNER JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON (c.id_contact =cs.id_contact)
            LEFT JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_lang` cl on (c.id_contact=cl.id_contact AND cl.id_lang="' . (int)$context->language->id . '")
            LEFT JOIN `' . _DB_PREFIX_ . 'employee` e on(c.id_employee= e.id_employee)
            WHERE cs.id_shop ="' . (int)$context->shop->id . '"' . (string)$filters . ($active ? ' AND c.active=1' : '') . ' 
            ' . ($count ? '' : 'ORDER BY ' . pSQL($order_by) . ' ' . pSQL($order_way) . ' ' . ($limit ? 'LIMIT ' . (int)$start . ',' . (int)$limit : ''));
        if ($count) {
            return Db::getInstance()->getValue($sql);
        } else {
            return Db::getInstance()->executeS($sql);
        }
    }

    public static function countLog($id_contact)
    {
        return Db::getInstance()->getValue('SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'ets_cfu_log` WHERE id_contact=' . (int)$id_contact);
    }

    public static function countContactLogByIdLang($id_lang)
    {
        $sql = "
            SELECT COUNT(DISTINCT l.ip,l.id_contact,l.datetime_added) 
            FROM `" . _DB_PREFIX_ . "ets_cfu_log` l 
            INNER JOIN `" . _DB_PREFIX_ . "ets_cfu_contact` c ON (l.id_contact=c.id_contact)
            LEFT JOIN `" . _DB_PREFIX_ . "ets_cfu_contact_lang` cl ON (c.id_contact=cl.id_contact AND cl.id_lang='" . (int)$id_lang . "')
        ";
        return (int)Db::getInstance()->getValue($sql);
    }

    public static function getLogList($context = null, $limit = 6, $start = 0)
    {
        if ($context == null)
            $context = Context::getContext();
        $sql = "SELECT * 
            FROM `" . _DB_PREFIX_ . "ets_cfu_log` l 
            INNER JOIN `" . _DB_PREFIX_ . "ets_cfu_contact` c ON (l.id_contact=c.id_contact)
            LEFT JOIN `" . _DB_PREFIX_ . "ets_cfu_contact_lang` cl ON (c.id_contact=cl.id_contact AND cl.id_lang='" . (int)$context->language->id . "')
            LEFT JOIN `" . _DB_PREFIX_ . "customer` cu ON (l.id_customer=cu.id_customer)
            ORDER BY l.datetime_added DESC 
            LIMIT " . (int)$start . ", " . (int)$limit;
        return Db::getInstance()->executeS($sql);
    }

    public static function cleanLogByIdShop($id_shop)
    {
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'ets_cfu_log` WHERE id_contact IN (SELECT id_contact FROM `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` WHERE id_shop=' . (int)$id_shop . ')');
    }

    public static function giveAccessToCustomerGroup($id_contact, Context $context = null)
    {
        if (!$id_contact || !Validate::isUnsignedInt($id_contact)) {
            return false;
        }

        if ($context === null)
            $context = Context::getContext();
        $group_access = self::getGroupAccessById($id_contact, $context->language->id);
        return array_intersect(Customer::getGroupsStatic((int)$context->customer->id), $group_access);
    }

    public static function getCountView($args = array(), &$y_max_value = 0, Context $context = null)
    {
        if ($context == null)
            $context = Context::getContext();
        $sql = '
            SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'ets_cfu_log` l 
            LEFT JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON (l.id_contact = cs.id_contact)
            WHERE cs.id_contact is NOT NULL  AND cs.id_shop=' . (int)$context->shop->id
            . (isset($args['id_contact']) && $args['id_contact'] ? ' AND cs.id_contact=' . (int)$args['id_contact'] : '')
            . (isset($args['year']) && $args['year'] ? ' AND YEAR(l.datetime_added) ="' . pSQL($args['year']) . '"' : '')
            . (isset($args['month']) && $args['month'] ? ' AND MONTH(l.datetime_added) ="' . pSQL($args['month']) . '"' : '')
            . (isset($args['day']) && $args['day'] ? ' AND DAY(l.datetime_added) ="' . pSQL($args['day']) . '"' : '');
        $result = (int)Db::getInstance()->getValue($sql);
        if ($result > $y_max_value)
            $y_max_value = $result;
        return $result;
    }

    public static function getCountUsers($args = array(), &$y_max_value = 0, Context $context = null)
    {
        if ($context == null)
            $context = Context::getContext();
        $sql = '
            SELECT COUNT(DISTINCT l.id_customer) FROM `' . _DB_PREFIX_ . 'ets_cfu_log` l 
            LEFT JOIN `' . _DB_PREFIX_ . 'ets_cfu_contact_shop` cs ON (l.id_contact = cs.id_contact)
            WHERE cs.id_contact is NOT NULL AND l.id_customer != 0 AND cs.id_shop = ' . (int)$context->shop->id
            . (isset($args['id_contact']) && $args['id_contact'] ? ' AND cs.id_contact=' . (int)$args['id_contact'] : '')
            . (isset($args['year']) && $args['year'] ? ' AND YEAR(l.datetime_added) ="' . pSQL($args['year']) . '"' : '')
            . (isset($args['month']) && $args['month'] ? ' AND MONTH(l.datetime_added) ="' . pSQL($args['month']) . '"' : '')
            . (isset($args['day']) && $args['day'] ? ' AND DAY(l.datetime_added) ="' . pSQL($args['day']) . '"' : '');
        $result = (int)Db::getInstance()->getValue($sql);
        if ($result > $y_max_value)
            $y_max_value = $result;
        return $result;
    }

    public static function getMinYear($id_contact)
    {
        return Db::getInstance()->getValue('SELECT MIN(YEAR(date_add)) FROM `' . _DB_PREFIX_ . 'ets_cfu_contact` WHERE 1 ' . ((int)$id_contact ? ' AND id_contact=' . (int)$id_contact : ''));
    }

    public static function addLog($id_contact, $ip, $browser)
    {
        if (!Db::getInstance()->getRow('SELECT * FROM `' . _DB_PREFIX_ . 'ets_cfu_log` WHERE ip="' . pSQL($ip) . '" AND DAY(datetime_added) ="' . pSQL(date('d')) . '" AND MONTH(datetime_added) ="' . pSQL(date('m')) . '" AND YEAR(datetime_added) ="' . pSQL(date('Y')) . '" AND id_contact=' . (int)$id_contact)) {
            Db::getInstance()->execute('INSERT INTO `' . _DB_PREFIX_ . 'ets_cfu_log` (ip,id_contact,browser,id_customer,datetime_added) VALUES ("' . pSQL($ip) . '","' . (int)$id_contact . '","' . pSQL($browser) . '","' . (int)Context::getContext()->customer->id . '","' . pSQL(date('Y-m-d h:i:s')) . '")');
        }
    }
}