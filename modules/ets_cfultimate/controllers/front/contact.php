...
class Ets_CfUltimateContactModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if ($contact->id
            && $contact->active
            && $contact->enable_form_page
            && ETS_CFU_Contact::getContactById($id_contact, $this->context->shop->id)
            && ETS_CFU_Contact::giveAccessToCustomerGroup($contact->id, $this->context)
        ) {
            $contact_form = $this->module->ets_cfu_contact_form($contact->id);
            $this->context->smarty->assign(array(
                'form_html' => $this->module->form_html($contact_form, 'contact'),
                'contact' => $contact,
        ...
        } elseif (!(isset($this->context->customer) && $this->context->customer->id > 0 && $this->context->customer->isLogged()) && in_array((int)Configuration::get('PS_CUSTOMER_GROUP'), explode(',', $contact->group_access))) {
            Tools::redirect($this->context->link->getPageLink('authentication') . '?back=' . Ets_CfUltimate::getLinkContactForm($contact->id, $this->context->language->id));
