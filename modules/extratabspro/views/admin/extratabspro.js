...
$(document).ready(function () {
    if (typeof extratabspro_id_product !== 'undefined') {
        if (window.location.href.indexOf("extratabspro") > -1) {
            //$('*[data-target="module-extratabspro"]').click();
            if (_PS_VERSION_[0] >= 8) {
                $('#product_extra_modules-tab-nav a').click();
                function show_extratabs_Pro(){
                    $('button[data-target="module-extratabspro"]').trigger('click');
                };
                window.setTimeout(show_extratabs_Pro, 200 );
