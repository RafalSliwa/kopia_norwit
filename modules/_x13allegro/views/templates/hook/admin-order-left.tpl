                ...
                <div class="row mt-2">
                    ...
                    <div class="col-lg-7">
                        {if $allegroOrder->checkout_form_content->invoice->required}
                            ...
                            {if !empty($allegroOrder->checkout_form_content->invoice->features)}
                                {capture name='invoiceWDT'}
                                    {if in_array('VAT_EU_VERIFIED', $allegroOrder->checkout_form_content->invoice->features)}
                                        {l s='Zamówienie jest zweryfikowane pod kątem transakcji VAT UE' mod='x13allegro'}
                                    {elseif in_array('VAT_EU_UNVERIFIED', $allegroOrder->checkout_form_content->invoice->features)}
                                        {l s='Zamówienie spełnia warunki transakcji VAT UE, ale numer VAT_EU kupującego nie został zweryfikowany' mod='x13allegro'}
                                    {/if}
                                {/capture}
                                <span data-toggle="pstooltip" data-original-title="{$smarty.capture.invoiceWDT}" style="text-decoration: underline; text-decoration-style: dotted;">WDT</span>
                            {/if}
                            {if !empty($allegroOrder->checkout_form_content->invoice->address->company) && !empty($allegroOrder->checkout_form_content->invoice->address->company->vatPayerStatus)}
                                {capture name='vatPayerStatus'}
                                    {if $allegroOrder->checkout_form_content->invoice->address->company->vatPayerStatus == 'ACTIVE'}
                                        {l s='Kupujący zadeklarował, że jest aktywnym podatnikiem VAT' mod='x13allegro'}
                                    {elseif $allegroOrder->checkout_form_content->invoice->address->company->vatPayerStatus == 'NON_ACTIVE'}
                                        {l s='Kupujący zadeklarował, że nie jest aktywnym podatnikiem VAT' mod='x13allegro'}
                                    {elseif $allegroOrder->checkout_form_content->invoice->address->company->vatPayerStatus == 'NOT_APPLICABLE'}
                                        {l s='Kupujący nie zadeklarował statusu podatnika VAT lub nie jest on obowiązujący dla podanych danych' mod='x13allegro'}
                                    {/if}
                                {/capture}
                                <small data-toggle="pstooltip" data-original-title="{$smarty.capture.vatPayerStatus}" style="text-decoration: underline; text-decoration-style: dotted;">
                                    {if $allegroOrder->checkout_form_content->invoice->address->company->vatPayerStatus == 'ACTIVE'}
                                        ({l s='aktywny podatnik VAT' mod='x13allegro'})
                                    {elseif $allegroOrder->checkout_form_content->invoice->address->company->vatPayerStatus == 'NON_ACTIVE'}
                                        ({l s='nieaktywny podatnik VAT' mod='x13allegro'})
                                    {elseif $allegroOrder->checkout_form_content->invoice->address->company->vatPayerStatus == 'NOT_APPLICABLE'}
                                        ({l s='status VAT nieobowiązujący' mod='x13allegro'})
                                    {/if}
                                </small>
                            {/if}
                        {else}
