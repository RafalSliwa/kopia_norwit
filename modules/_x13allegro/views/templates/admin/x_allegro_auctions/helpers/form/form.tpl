...
{block name="input"}
	{if $input.name == 'name'}
		...
		{if $fields_value.id_product}
            ...
            {elseif version_compare($smarty.const._PS_VERSION_, '9.0.0', '>=')}
                {capture name='product_link'}{$link->getAdminLink('AdminProducts', true, ['route' => 'admin_product_form', 'id' => {$fields_value.id_product}])}{/capture}
            {else}
