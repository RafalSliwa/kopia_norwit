...
<div class="gpsr-safety-information-attachment-wrapper" data-max="{$safetyInformationAttachmentMax}" style="display: none;">
    <table class="table gpsr-safety-information-attachment-table">
        ...
        <tbody>
            {if !empty($x13gpsrAttachments)}
                {foreach $x13gpsrAttachments as $attachment}
                    <tr data-type="attachment_x13gpsr">
                        <td>
                            <input type="checkbox" name="item[{$index}][safety_information_attachment_x13gpsr][checked][]" x-name="safety_information_attachment_product" value="1" checked="checked">
                            <input type="hidden" name="item[{$index}][safety_information_attachment_x13gpsr][file][]" value="{$attachment.file}">
                            <input type="hidden" name="item[{$index}][safety_information_attachment_x13gpsr][name][]" value="{$attachment.name}">
                            <input type="hidden" name="item[{$index}][safety_information_attachment_x13gpsr][mimetype][]" value="{$attachment.mimetype}">
                        </td>
