function X13Allegro()
{
    ...
    this.productizationGPSR = 0;
    this.productizationMode = 'ASSIGN';
    this.productSelectMode = 0;
    this.titleMinWords = 3;
    this.titleMinCount = 12;
...
X13Allegro.prototype = {
    ...
    _titleWordsCount: function(title)
    {
    ...
    auctionForm: function()
    {
        ...
        $('[x-name=title]').on('input xchange', function() {
            var title = $(this).prop('value');
            var size = self._titleCount(title);
            var words = self._titleWordsCount(title);
            var tooltipErrors = [];

            var $titleBox = $(this).closest('.title-box');
            var $input = $titleBox.find('input[x-name="title"]');
            var $counter = $titleBox.find('.allegro-title-counter');
            var $counterError = $titleBox.find('.allegro-title-counter-error');

            $input.removeClass('error');
            $counter.removeClass('error');
            $counterError.attr('data-original-title', '');
            $counterError.hide();

            while (size > self.titleMaxCount) {
                $input.prop('value', $input.prop('value').slice(0, -1));
                size = self._titleCount($input.prop('value'));
            }

            if (size < self.titleMinCount) {
                $counter.addClass('error');
                tooltipErrors.push($counterError.data('error-min'));
            }

            if (words < self.titleMinWords) {
                $counter.addClass('error');
                tooltipErrors.push($counterError.data('error-words'));
            }

            if (tooltipErrors.length) {
                var errors = tooltipErrors.join('<br>');
                $counterError.attr('data-original-title', errors);
                $counterError.show();
        ...
        $('[x-name=preview]').on('click', function() {
            ...
            self.ajaxPOST({
                ...
                allegroDescription: +button.hasClass('xproductization-description-preview'),
                action: 'preview'
        ...
        $('[x-name="bulk_title_before"]').on('click', function(e) {
            ...
            $('[x-name="product_switch"]:checked').each(function(i, el) {
                ...
                var $inputTitle = $('input[name="item[' + itemId + '][title]"]');

                if ($inputTitle.attr('readonly')) {
                    return;
                }

                var titleCurrent = $inputTitle.val().trim();
                $inputTitle.val(formatTitle(itemId, titlePattern).trim() + ' ' + titleCurrent).trigger('xchange');
            });
        ...
        $('[x-name="bulk_title_after"]').on('click', function(e) {
            ...
            $('[x-name="product_switch"]:checked').each(function(i, el) {
                ...
                var titleCurrent = $inputTitle.val().trim();
                $inputTitle.val(titleCurrent + ' ' + formatTitle(itemId, titlePattern).trim()).trigger('xchange');
            });
        ...
        $('[x-name="bulk_title_change"]').on('click', function(e) {
            ...
            $('[x-name="product_switch"]:checked').each(function(i, el) {
                ...
                $inputTitle.val(formatTitle(itemId, titlePattern).trim()).trigger('xchange');
            });
    ...
    initPerformProductization: function()
    {
        ...
        function fillAllegroProductsResult($auction, data, mode, callback)
        {
            ...
            $auction.find('.xproductization-trusted-content').hide();
        ...
        function fillAllegroProductSelected($auction, data, callback)
        {
            ...
            var $auctionResponsibleProducer = $auctionDetails.find('[x-name="responsible_producer"]');
            var $auctionSafetyInformationType = $auctionDetails.find('[x-name="safety_information_type"]');
            var $auctionSafetyInformationText = $auctionDetails.find('[x-name="safety_information_text"]');
            ...
            $auctionResponsibleProducer.find('option[data-allegro]').remove();
            $auctionResponsibleProducer.val($auctionResponsibleProducer.data('default-value'));
            $auctionSafetyInformationType.val($auctionSafetyInformationType.data('default-value'));
            $auctionSafetyInformationText.val($auctionSafetyInformationText.data('default-value'));

            if (data.allegroProductResponsibleProducers.length) {
                var responsibleProducers = JSON.parse(data.allegroProductResponsibleProducers);

                responsibleProducers.forEach(function (responsibleProducer) {
                    var $responsibleProducerOption = $('<option></option>')
                        .attr('value', responsibleProducer.id)
                        .attr('data-allegro', true)
                        .text(responsibleProducer.name);

                    $responsibleProducerOption.insertAfter($auctionResponsibleProducer.find('option:first'));
                });

                if (responsibleProducers.length
                    && (self.productizationGPSR == 'allegro'
                        || self.productizationGPSR === 'allegro-x13gpsr'
                        || (self.productizationGPSR === 'x13gpsr-allegro' && !$auctionResponsibleProducer.data('default-value').length))
                ) {
                    $auctionResponsibleProducer.val(responsibleProducers[0].id);
                }
            }

            if (data.allegroProductSafetyInformation.length
                && (self.productizationGPSR == 'allegro'
                    || self.productizationGPSR === 'allegro-x13gpsr'
                    || (self.productizationGPSR === 'x13gpsr-allegro' && !$auctionSafetyInformationType.data('default-value').length))
            ) {
                var safetyInformation = JSON.parse(data.allegroProductSafetyInformation);

                $auctionSafetyInformationType.val('TEXT');
                $auctionSafetyInformationText.val(safetyInformation.description);
            }

            $auctionResponsibleProducer.trigger('chosen:updated');
            $auctionSafetyInformationType.trigger('change');
            $auctionSafetyInformationText.trigger('input');

            var trustedName = false;
            var trustedImages = false;
            var trustedDescription = false;
            var trustedCategory = false;

            if (data.allegroProductTrustedContentPaths.length) {
                var trustedContentPaths = JSON.parse(data.allegroProductTrustedContentPaths);
                var trustedContentPathTranslated = [];

                trustedContentPaths.forEach(function (path) {
                    trustedContentPathTranslated.push($auction.find('.xproductization-trusted-content').data('trusted-content-' + path));
                });

                if (trustedContentPathTranslated.length) {
                    $auction.find('.xproductization-trusted-content-paths').text(trustedContentPathTranslated.join(', '));
                    $auction.find('.xproductization-trusted-content').show();
                }

                if (trustedContentPaths.includes('name')) {
                    trustedName = true;
                }
                if (trustedContentPaths.includes('images')) {
                    trustedImages = true;
                }
                if (trustedContentPaths.includes('description')) {
                    trustedDescription = true;
                }
                if (trustedContentPaths.includes('category')) {
                    trustedCategory = true;
                }
            }

            if (self.productizationName == 'allegro' || trustedName) {
            ...
            $auctionTitleInput.prop('readonly', trustedName);

            if ($auctionDetails.find('[x-name="productization_name_copy"]').length && !trustedName) {
            ...
            if ((self.productizationDescription == 'allegro' && data.allegroProductDescription != '') || trustedDescription) {
                $auctionDetails.find('select[x-name="template"]').prop('disabled', true).trigger('chosen:updated');
                $auctionDetails.find('a[x-name="preview"]').addClass('xproductization-description-preview');
            } else {
                $auctionDetails.find('a[x-name="preview"]').removeClass('xproductization-description-preview');
            }

            if (trustedDescription) {
                $auctionDetails.find('a[x-name="description_edit"]').hide();
            } else {
                $auctionDetails.find('a[x-name="description_edit"]').show();
            }

            if (trustedImages) {
                var allegroProductImages = JSON.parse(data.allegroProductImages);
                var $allegroProductImagesList = $('<ul class="xproductization-images"></ul>');

                allegroProductImages.forEach(function (image) {
                    var $allegroProductImage = $('<li class="item_form image"></li>');
                    $allegroProductImage.append('<span class="center-helper"></span>');
                    $allegroProductImage.append('<img src="' + image.url.replace(/original/, 's128') + '">');
                    $allegroProductImagesList.append($allegroProductImage);
                });

                $auctionDetails.find('.images-sortable').closest('td').prepend($allegroProductImagesList);
                $auctionDetails.find('.images-sortable').hide();
            }
            ...
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_name"]').val(+trustedName);
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_images"]').val(+trustedImages);
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_description"]').val(+trustedDescription);
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_category"]').val(+trustedCategory);
        ...
        $(document).on('click', '[data-allegro-product]', function(e) {
            ...
            if ($auctionDetails.find('.xproductization-images').length) {
                $auctionDetails.find('.xproductization-images').remove();
            }
        ...
        $(document).on('change', '.xproductization-mode-selector select', function () {
            ...
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_name"]').val('');
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_images"]').val('');
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_description"]').val('');
            $auctionDetails.find('input[x-name="allegro_product_trusted_content_category"]').val('');
            ...
            // reset offer title
            $auctionTitleInput.val($auctionTitleInput.data('product-name')).trigger('input');
            $auctionTitleInput.prop('readonly', false);
            ...
            $auctionDetails.find('.images-sortable').show();

            $auctionResponsibleProducer.find('option[data-allegro]').remove();
            $auctionResponsibleProducer.val($auctionResponsibleProducer.data('default-value')).trigger('chosen:updated');
            $auctionSafetyInformationType.val($auctionSafetyInformationType.data('default-value')).trigger('change');
            $auctionSafetyInformationText.val($auctionSafetyInformationText.data('default-value')).trigger('input');
            ...
            if (value === 'ASSIGN') {
                ...
                if (productCount) {
                    if (productSelected) {
                        $auction.find('.xproductization-status').text('Trwa pobieranie informacji o produkcie').show();
        ...
        $(document).on(
            'click',
            'div.xproductization-describes-product:not(.xproductization-value-trusted) input.xproductization-readonly, ' +
            'div.xproductization-describes-product:not(.xproductization-value-trusted) .chosen-disabled',
            function() {
