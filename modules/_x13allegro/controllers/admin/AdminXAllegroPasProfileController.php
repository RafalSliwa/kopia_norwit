...
final class AdminXAllegroPasProfileController extends XAllegroController
{
    ...
    public function __construct()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            $this->allegroAutoLogin = false;
            $this->allegroAccountSwitch = false;
        }
    ...
    public function initPageHeaderToolbar()
    {
        if (empty($this->display) && version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            $this->page_header_toolbar_btn['allegro_delivery'] = array(
                'href' => $this->context->link->getAdminLink('AdminXAllegroPasProfile') . '&add' . $this->table,
    ...
    {
        if (version_compare(_PS_VERSION_, '1.7.0.0', '<')
            && Tools::getValue('controller') == 'AdminXAllegroPasProfile'
            && empty($this->errors)
        ) {
