...
final class AdminXAllegroAssocCategoriesController extends XAllegroController
{
    ...
    public function init()
    {
        ...
        $this->fields_options = $this->getFieldsOptions();
    }
    ...
    public function initPageHeaderToolbar()
    {
        if (empty($this->display) && version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            $this->page_header_toolbar_btn['allegro_category'] = array(
                'href' => $this->context->link->getAdminLink('AdminXAllegroAssocCategories') . '&add' . $this->table,
    ...
    {
        if (version_compare(_PS_VERSION_, '1.7.0.0', '<')
            && Tools::getValue('controller') === 'AdminXAllegroAssocCategories'
            && empty($this->errors)
            && empty($this->confirmations)
        ) {
    ...
    {
        $this->redirect_after = $this->context->link->getAdminLink('AdminXAllegroAssocCategories') . '&conf=6';
    }
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('submitCopyParameters')) {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminXAllegroAssocCategories') . '&conf=4');
        }
    ...
    public function ajaxProcessGetParameterMapForm()
    {
        ...
        try {
            if (!Validate::isLoadedObject($this->object)) {
                throw new Exception($this->l('Musisz zapisać tą ścieżkę kategorii przed mapowaniem parametrów.'));
            }
    ...
    public function ajaxProcessSubmitCategoryFieldsValues()
    {
        ...
        die(json_encode([
            'success' => true,
            'message' => $this->l('Zapisano zmiany w parametrach kategorii')
        ]));
    }
}
