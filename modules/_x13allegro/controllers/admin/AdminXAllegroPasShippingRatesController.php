...
final class AdminXAllegroPasShippingRatesController extends XAllegroController
{
    ...
    public function initPageHeaderToolbar()
    {
        if (empty($this->display) && version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            $this->page_header_toolbar_btn['allegro_delivery_rate'] = array(
                'href' => $this->context->link->getAdminLink('AdminXAllegroPasShippingRates') . '&add' . $this->table,
    ...
    {
        if (version_compare(_PS_VERSION_, '1.7.0.0', '<')
            && Tools::getValue('controller') == 'AdminXAllegroPasShippingRates'
            && empty($this->errors)
        ) {
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('submitAdd' . $this->table))
        {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminXAllegroPasShippingRates') . '&conf=4');
        }
