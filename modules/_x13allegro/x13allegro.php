...
define('X13_ALLEGRO_VERSION', '7.7.6');
...
class x13allegro extends x13allegro\XAllegroModuleCore
{
    ...
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        $this->ps_versions_compliancy = ['min' => '1.6.0.0', 'max' => '9.9.99'];
        ...
        if (strpos($_SERVER['SCRIPT_NAME'], 'sync.php') === false
            ...
            && in_array($this->context->controller->controller_type, ['admin', 'moduleadmin'])
            && isset($this->context->cookie->id_employee)
        ) {
    ...
    public function hookDisplayLeftColumnProduct($params)
    {
        ...
        return $this->displayProductAllegroAuctionLink($params);
    }
    ...
    public function displayProductAllegroAuctionLink($params)
    {
        if (!XAllegroConfiguration::get('FRONT_DISPLAY_LINK')) {
            return null;
        }

        $this->context->smarty->assign('allegro_img', $this->context->shop->getBaseURL(Configuration::get('PS_SSL_ENABLED')) . 'modules/x13allegro/img/AdminXAllegroMain.png');

        if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            $this->context->smarty->assign('allegro_href', $this->generateProductAllegroAuctionLink($params['product']['id_product'], $params['product']['id_product_attribute']));
        }

        return $this->display(__FILE__, 'product-allegro-auction-link.tpl');
    }

    public function generateProductAllegroAuctionLink($productId, $productAttributeId)
    {
        try {
            $offer = XAllegroAuction::getAuctionByProduct($productId, $productAttributeId);

            if ($offer) {
                $allegroAccount = new XAllegroAccount((int)$offer['id_xallegro_account']);
                $marketplaceProvider = new \x13allegro\Api\DataProvider\MarketplacesProvider($allegroAccount->base_marketplace);

                return $marketplaceProvider->getMarketplaceOfferUrl($offer['id_auction'], $allegroAccount->sandbox);
            }
        }
    ...
    public function hookActionObjectOrderCarrierUpdateAfter($params)
    {
        if (XAllegroConfiguration::get('ORDER_CARRIER_CHANGE_MAPPED_OPERATOR')
            ...
            $carrierPackageInfo = XAllegroCarrier::getPackageInfo($params['object']->id_order);

            if (!isset($carrierPackageInfo[$params['object']->id]) && !empty($operatorDetails)) {
                XAllegroCarrier::updatePackageInfo(
                    $params['object']->id,
                    $params['object']->id_order,
                    $operatorDetails['id_operator'],
                    $operatorDetails['operator_name']
                );
            }
            else if (isset($carrierPackageInfo[$params['object']->id])
                && !empty($operatorDetails)
                && $carrierPackageInfo[$params['object']->id]['id_operator'] !== $operatorDetails['id_operator']
            ) {
                XAllegroCarrier::updateOperator(
                    $params['object']->id_order,
                    $operatorDetails['id_operator'],
                    $operatorDetails['operator_name']
                );
    ...
    public function hookActionX13GPSRModuleRenderConfigurationForm(array &$params)
    {
        if (Tools::isSubmit('savex13gpsr')) {
            XAllegroConfiguration::updateValue('X13GPSR_SAFETY_INFORMATION_PRIORITY', Tools::getValue('X13GPSR_SAFETY_INFORMATION_PRIORITY'));
            XAllegroConfiguration::updateValue('X13GPSR_SAFETY_INFORMATION_TEXT_COMBINE', Tools::getValue('X13GPSR_SAFETY_INFORMATION_TEXT_COMBINE'));
            XAllegroConfiguration::updateValue('X13GPSR_SAFETY_INFORMATION_TEXT_GENERAL', Tools::getValue('X13GPSR_SAFETY_INFORMATION_TEXT_GENERAL'));
            XAllegroConfiguration::updateValue('X13GPSR_SAFETY_INFORMATION_TEXT_LABEL', Tools::getValue('X13GPSR_SAFETY_INFORMATION_TEXT_LABEL'));
            XAllegroConfiguration::updateValue('X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PRODUCER', Tools::getValue('X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PRODUCER'));
            XAllegroConfiguration::updateValue('X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PERSON', Tools::getValue('X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PERSON'));
        }

        $params['fields_form'][0]['form']['input'][] = [
            'name' => 'separator',
            'ignore' => true,
            'type' => 'html',
        ...
        $params['fields_form'][0]['form']['input'][] = [
            'name' => 'X13GPSR_SAFETY_INFORMATION_PRIORITY',
            'type' => 'select',
            'label' => $this->l('Priorytet pobierania informacji o bezpieczeństwie'),
            'options' => [
                'query' => [
                    ['id' => \x13allegro\Api\Model\Offers\ProductSet\SafetyInformationType::TEXT, 'name' => $this->l('tekst, załączniki')],
                    ['id' => \x13allegro\Api\Model\Offers\ProductSet\SafetyInformationType::ATTACHMENTS, 'name' => $this->l('załączniki, tekst')]
                ],
                'id' => 'id',
                'name' => 'name'
            ],
            'default' => XAllegroConfiguration::get('X13GPSR_SAFETY_INFORMATION_PRIORITY')
        ];
        $params['fields_form'][0]['form']['input'][] = [
            'name' => 'X13GPSR_SAFETY_INFORMATION_TEXT_COMBINE',
            'type' => 'select',
            'label' => $this->l('Ustawienia pobierania tekstu informacji o bezpieczeństwie'),
            'options' => [
                'query' => [
                    ['id' => 0, 'name' => $this->l('wybierz pierwszą dostępną wartość')],
                    ['id' => 1, 'name' => $this->l('połącz wszystkie wartości')]
                ],
                'id' => 'id',
                'name' => 'name'
            ],
            'default' => (int)XAllegroConfiguration::get('X13GPSR_SAFETY_INFORMATION_TEXT_COMBINE')
        ];
        $params['fields_form'][0]['form']['input'][] = [
            'name' => 'X13GPSR_SAFETY_INFORMATION_TEXT_GENERAL',
            'type' => 'switch',
            'label' => $this->l('Wyślij tekst informacji o bezpieczeństwie'),
            'values' => [
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_GENERAL_on', 'value' => 1, 'name' => $this->l('Tak')],
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_GENERAL_off', 'value' => 0, 'name' => $this->l('Nie')]
            ],
            'default' => (int)XAllegroConfiguration::get('X13GPSR_SAFETY_INFORMATION_TEXT_GENERAL')
        ];
        $params['fields_form'][0]['form']['input'][] = [
            'name' => 'X13GPSR_SAFETY_INFORMATION_TEXT_LABEL',
            'type' => 'switch',
            'label' => $this->l('Wyślij etykiety informacji o bezpieczeństwie'),
            'values' => [
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_LABEL_on', 'value' => 1, 'name' => $this->l('Tak')],
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_LABEL_off', 'value' => 0, 'name' => $this->l('Nie')]
            ],
            'default' => (int)XAllegroConfiguration::get('X13GPSR_SAFETY_INFORMATION_TEXT_LABEL')
        ];
        $params['fields_form'][0]['form']['input'][] = [
            'name' => 'X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PRODUCER',
            'type' => 'switch',
            'label' => $this->l('Wyślij dodatkowe informacje producenta'),
            'values' => [
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PRODUCER_on', 'value' => 1, 'name' => $this->l('Tak')],
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PRODUCER_off', 'value' => 0, 'name' => $this->l('Nie')]
            ],
            'default' => (int)XAllegroConfiguration::get('X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PRODUCER')
        ];
        $params['fields_form'][0]['form']['input'][] = [
            'name' => 'X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PERSON',
            'type' => 'switch',
            'label' => $this->l('Wyślij dodatkowe informacje osoby odpowiedzialnej'),
            'values' => [
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PERSON_on', 'value' => 1, 'name' => $this->l('Tak')],
                ['id' => 'X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PERSON_off', 'value' => 0, 'name' => $this->l('Nie')]
            ],
            'default' => (int)XAllegroConfiguration::get('X13GPSR_SAFETY_INFORMATION_TEXT_RESPONSIBLE_PERSON')
        ];
    }
