...
class PM_AdvancedSearch4 extends Core implements PrestaShop\PrestaShop\Core\Module\WidgetInterface
{
    ...
    private $originalPositionSortableCriterion = [
        'attribute',
        'category',
        'subcategory',
    ];
    private $criteriaGroupLabels;
    private $criterionGroupIsTemplatisable = [
        'attribute',
        'feature',
        'manufacturer',
        'supplier',
    ...
    protected $criteria_group_type_interal_name = [
        1 => 'select',
        2 => 'image',
        3 => 'link',
        4 => 'checkbox',
    ...
    private $display_vertical_search_block = [];
    public static $modulePrefix = 'as4';
    public static $renderedFacetsCache = [];
    protected $defaultConfiguration = [
        'fullTree' => true,
        'moveOosProductsAtTheEnd' => false,
        'autoReindex' => true,
        ...
        'sqlCheckCustomersGroups' => false,
        'pageBuilderCompatibilityMode' => false,
    ];
    ...
    private static $idProductToAdd = [];
    private static $idProductToUpdate = [];
    private static $idProductToDelete = [];
    ...
    public function __construct()
    {
        ...
        parent::__construct();
        $this->templatePrefix = $this->getPrestaShopTemplateVersion() . '/';
        ...
        if ($this->onBackOffice()) {
            ...
            $this->options_show_hide_crit_method = [
                1 => $this->l('On mouse over'),
                2 => $this->l('On click'),
                3 => $this->l('In an overflow block'),
            ];
            $this->options_launch_search_method = [
            ...
            $this->support_link = [
                ['link' => $forum_url, 'target' => '_blank', 'label' => $this->l('Forum topic')],
                
                ['link' => 'https://addons.prestashop.com/contact-form.php?id_product=2778', 'target' => '_blank', 'label' => $this->l('Support contact')],
            ];
            $this->display_vertical_search_block = [];
            foreach (['displayLeftColumn', 'displayRightColumn'] as $hookName) {
                if (Hook::getIdByName($hookName) !== false) {
                    $this->display_vertical_search_block[] = Hook::getIdByName($hookName);
                }
            }
            $this->criteriaGroupLabels = [
    ...
    public function customShutdownProcess()
    {
        ...
        foreach (self::$idProductToDelete as $idProduct) {
            $product = new Product((int)$idProduct);
            if (Validate::isLoadedObject($product)) {
                SearchEngineIndexation::desIndexCriterionsFromProduct((int)$product->id);
            }
        }
    ...
    public function hookModuleRoutes()
    {
        ...
        $routes = [
            'module-pm_advancedsearch4-cron' => [
                'controller' => 'cron',
                'params' => [
                    'fc' => 'module',
                    'module' => $this->name,
            ...
            'module-pm_advancedsearch4-searchresults' => [
                ...
                'keywords' => [
                    'id_search' => ['regexp' => '[0-9]+', 'param' => 'id_search'],
                    'as4_sq' => ['regexp' => $as4SqRegeXp, 'param' => 'as4_sq'],
                ],
                'params' => [
                    'fc' => 'module',
                    'module' => $this->name,
            ...
            'module-pm_advancedsearch4-searchresults-categories' => [
                'controller' => 'searchresults',
                'rule' => $searchResultsCategoryPrefix . '/s-{id_search}/{as4_sq}',
                'keywords' => [
                    'id' => ['regexp' => '[0-9]+', 'param' => 'id_category_search'],
                    'id_search' => ['regexp' => '[0-9]+', 'param' => 'id_search'],
                    'as4_sq' => ['regexp' => $as4SqRegeXp, 'param' => 'as4_sq'],
                    'rewrite' => ['regexp' => $defaultRewritePattern],
            ...
            'module-pm_advancedsearch4-searchresults-suppliers' => [
                'controller' => 'searchresults',
                'rule' => $searchResultsSupplierPrefix . '/s-{id_search}/{as4_sq}',
                'keywords' => [
                    'id' => ['regexp' => '[0-9]+', 'param' => 'id_supplier_search'],
                    'id_search' => ['regexp' => '[0-9]+', 'param' => 'id_search'],
                    'as4_sq' => ['regexp' => $as4SqRegeXp, 'param' => 'as4_sq'],
                    'rewrite' => ['regexp' => $defaultRewritePattern],
                    'meta_keywords' => ['regexp' => '[_a-zA-Z0-9-\pL]*'],
            ...
            'module-pm_advancedsearch4-searchresults-manufacturers' => [
                'controller' => 'searchresults',
                'rule' => $searchResultsManufacturerPrefix . '/s-{id_search}/{as4_sq}',
                'keywords' => [
                    'id' => ['regexp' => '[0-9]+', 'param' => 'id_manufacturer_search'],
                    'id_search' => ['regexp' => '[0-9]+', 'param' => 'id_search'],
                    'as4_sq' => ['regexp' => $as4SqRegeXp, 'param' => 'as4_sq'],
                    'rewrite' => ['regexp' => $defaultRewritePattern],
                    'meta_keywords' => ['regexp' => '[_a-zA-Z0-9-\pL]*'],
            ...
            'module-pm_advancedsearch4-searchresults-cms' => [
                'controller' => 'searchresults',
                'rule' => $searchResultsCmsPrefix . '/s-{id_search}/{as4_sq}',
                'keywords' => [
                    'id' => ['regexp' => '[0-9]+', 'param' => 'id_cms_search'],
                    'id_search' => ['regexp' => '[0-9]+', 'param' => 'id_search'],
                    'as4_sq' => ['regexp' => $as4SqRegeXp, 'param' => 'as4_sq'],
                    'rewrite' => ['regexp' => $defaultRewritePattern],
                    'meta_keywords' => ['regexp' => '[_a-zA-Z0-9-\pL]*'],
    ...
    protected function registerToAllHooks()
    {
        ...
        if (!$this->registerHook('moduleRoutes') || !$this->registerHook('displayHeader') || !$this->registerHook('actionProductUpdate') || !$this->registerHook('actionProductAdd') || !$this->registerHook('actionProductDelete')) {
            return false;
        }
    ...
    {
        if (method_exists($this, 'installControllers') && is_callable([$this, 'installControllers'])) {
            $this->installControllers();
            if ($forceSetLayout) {
                $this->updateControllersLayoutSettings(['cron', 'seositemap']);
            }
        }
    }
    ...
    public function installDB()
    {
        ...
        $sql = Tools::file_get_contents(dirname(__FILE__) . '/sql/' . self::INSTALL_SQL_BASE_FILE);
        if ($sql == false) {
            return false;
        }
        $sql = str_replace('PREFIX_', _DB_PREFIX_, (string)$sql);
        $sql = str_replace('MYSQL_ENGINE', _MYSQL_ENGINE_, (string)$sql);
        $sql = preg_split("/;\s*[\r\n]+/", (string)$sql);
        if ($sql == false) {
            return false;
        }
    ...
    public function installDBCache($id_search, $with_drop = true)
    {
        ...
        $sql = Tools::file_get_contents(dirname(__FILE__) . '/sql/' . self::INSTALL_SQL_DYN_FILE);
        if ($sql == false) {
            return false;
        }
        $sql = str_replace('ID_SEARCH', (string)$id_search, (string)$sql);
        $sql = str_replace('PREFIX_', _DB_PREFIX_, (string)$sql);
        $sql = str_replace('MYSQL_ENGINE', _MYSQL_ENGINE_, (string)$sql);
        $sql = preg_split("/;\s*[\r\n]+/", $sql);
        if ($sql == false) {
    ...
    protected function updateControllersLayoutSettings($ignoreList = [])
    {
    ...
    public function checkIfModuleIsUpdate($updateDb = false, $displayConfirm = true)
    {
        ...
        if ($updateDb) {
            ...
            if (version_compare(_PS_VERSION_, '1.7.7.0', '>=')) {
                Hook::getIdByName($newHooksList[0], true, true);
            } else {
                Hook::getIdByName($newHooksList[0]);
            }
            ...
            if (!empty($currentModuleLastVersion) && version_compare($currentModuleLastVersion, '5.0.0', '<') && version_compare($this->version, '5.0.0', '>=')) {
                if (!isset($config['maintenanceMode'])) {
                    $config['maintenanceMode'] = (bool)Configuration::get('PM_' . self::$modulePrefix . '_MAINTENANCE');
                }
            }
            if (!$firstInstall) {
                $this->defaultConfiguration['sqlCheckCustomersGroups'] = true;
            }
            if (self::hasProblematicPageBuildersInstalled()) {
                $this->defaultConfiguration['pageBuilderCompatibilityMode'] = true;
            }
            ...
            $this->generateCss();
            $this->pmClearCache();
            $this->addASToNativeSitemapExclusions();
            if ($displayConfirm) {
    ...
    protected function addASToNativeSitemapExclusions()
    {
        $updatedExclusionList = [];
        $metas = Meta::getMetasByIdLang((int)$this->context->cookie->id_lang);
        if (empty($metas) || !$metas instanceof \Traversable) {
            return;
        }
        foreach ($metas as $meta) {
            if (preg_match('/^module-pm_advancedsearch4?-.*$/', $meta['page'])) {
                $updatedExclusionList[] = (int)$meta['id_meta'];
            }
        }
        $gSitemapConfiguration = Configuration::get('GSITEMAP_DISABLE_LINKS');
        if (!empty($gSitemapConfiguration)) {
            $explodedExistingConfiguration = explode(', ', $gSitemapConfiguration);
            foreach ($explodedExistingConfiguration as $excludedMetaId) {
                if (!in_array((int)$excludedMetaId, $updatedExclusionList)) {
                    $updatedExclusionList[] = (int)$excludedMetaId;
                }
            }
        }
        $implodedUpdatedList = implode(', ', $updatedExclusionList);
        Configuration::updateValue('GSITEMAP_DISABLE_LINKS', $implodedUpdatedList);
    }
    public function updateSearchTable($oldModuleVersion = null)
    {
        ...
        if (is_array($toChange) && count($toChange) > 0) {
        ...
        if (is_array($indexToAdd) && count($indexToAdd) > 0) {
        ...
        if (is_array($primaryToAdd) && count($primaryToAdd) > 0) {
    ...
    public function displayAdvancedConfig()
    {
        ...
        if ($this->getAdvancedStylesDb() == false) {
            if (Tools::file_exists_cache($advanced_css_file) && is_readable($advanced_css_file)) {
                $advancedCssFileContent = (string)Tools::file_get_contents($advanced_css_file);
                if (Tools::strlen($advancedCssFileContent) > 0) {
                    $this->updateAdvancedStyles($advancedCssFileContent);
                }
            } else {
    ...
    {
        $advanced_searchs_id = SearchEngineUtils::getSearchsId(false, (int)$this->context->shop->id);
        $criteriaGroupToReindex = [];
        ...
        $sqlConfigVariables = [];
        foreach ($sqlConfigKeys as $sqlConfigKey) {
            $configValue = SearchEngineDb::getVariableValue($sqlConfigKey);
            if ($configValue === null) {
                continue;
            }
            $sqlConfigVariables[$sqlConfigKey] = $configValue;
        }
        $vars = [
            'groups_to_reindex' => $criteriaGroupToReindex,
            'sqlConfigVariables' => $sqlConfigVariables,
        ];
    ...
    public function getCriterionsGroupsValue()
    {
        ...
        $criterions_groups = [];
        $criterions_groups['associations'][] = [
            'id' => 0,
            'name' => $this->l('All category levels'),
            'type' => 'category',
        ];
        $criterions_groups['associations'][] = [
            'id' => 0,
            'name' => $this->l('Subcategories'),
            'type' => 'subcategory',
        ];
        ...
        foreach ($categories_level_depth as $category_level_depth) {
            $criterions_groups['associations'][] = [
                'id' => $category_level_depth['level_depth'],
                'name' => $this->l('Categories level') . ' ' . $category_level_depth['level_depth'],
                'type' => 'category',
            ];
        }
        $criterions_groups['associations'][] = [
        ...
        foreach ($attributes_groups as $row) {
            $criterions_groups['attribute'][] = [
                'id' => $row['id_attribute_group'],
                'name' => $row['public_name'],
                'internal_name' => (!empty($row['name']) ? $row['name'] : ''),
                'type' => 'attribute',
            ];
        }
        $features = SearchEngineIndexation::getAvailableFeaturesGroups($id_lang);
        foreach ($features as $row) {
            $criterions_groups['feature'][] = [
                'id' => $row['id_feature'],
                'name' => $row['name'],
                'type' => 'feature',
            ];
        }
        $criterions_groups['product_properties'][] = [
            'id' => 0,
            'name' => $this->l('Price'),
            'type' => 'price',
        ];
    ...
    protected function postProcessSearch()
    {
        ...
        if ($ObjAdvancedSearchClass->id_hook > 0) {
            $hookName = Hook::getNameById($ObjAdvancedSearchClass->id_hook);
            $currentLayoutConfig = $this->context->shop->theme->getPageLayouts();
            if (preg_match('/(leftcolumn|rightcolumn)$/i', $hookName) && !empty($currentLayoutConfig['category'])) {
                $this->updateControllersLayoutSettings(['cron', 'seositemap']);
            }
        }
        $this->errors = $this->retroValidateController($ObjAdvancedSearchClass);
        if (!count($this->errors) && !$ObjAdvancedSearchClass->save()) {
        ...
        if (!count($this->errors)) {
            if (!$id_search) {
                ...
                if (!count($this->errors) && !SearchEngineIndexation::addCacheProduct($ObjAdvancedSearchClass->id)) {
    ...
    protected function postProcessSearchDuplicate()
    {
        $idSearch = Tools::getValue('id_search', false);
        $searchEngine = new Search($idSearch);
        $targetIdShop = (int)Tools::getValue('id_shop_destination');
        $shop = new Shop($targetIdShop);
        if (!Validate::isLoadedObject($searchEngine) || !Validate::isLoadedObject($shop)) {
        ...
        $duplicatedSearchEngine = $searchEngine->duplicate((int)$shop->id);
        if (!empty($duplicatedSearchEngine) && is_object($duplicatedSearchEngine)) {
            $this->displayJsTags('open');
            $this->postDuplicateProcess([
                'obj' => $duplicatedSearchEngine,
                'class' => get_class($duplicatedSearchEngine),
            ]);
            $this->displayCloseDialogIframeJs(false);
            $this->displayJsTags('close');
        } else {
            $this->errors[] = $this->l('Error while duplicating');
            $this->displayErrorsJs(true);
        }
        $this->echoOutput(true);
    }
    ...
    protected function postProcess()
    {
        if (Tools::getValue('submitAdvancedConfig')) {
        ...
        } elseif (Tools::getIsset('submitSearchDuplicate')) {
            $this->postProcessSearchDuplicate();
            ...
            $this->cleanOutput();
            $order = Tools::getValue('order') ? explode(',', Tools::getValue('order')) : [];
            foreach ($order as $position => $searchIdentifier) {
                $idSearch = self::parseInt($searchIdentifier);
                if (!empty($idSearch)) {
                    Db::getInstance()->update('pm_advancedsearch', ['position' => (int)$position], 'id_search = ' . (int)$idSearch);
                }
            }
            ...
            foreach ($order as $position => $key_criterions_group) {
                ...
                $row = ['position' => (int)$position, 'hidden' => (int)$hidden];
            ...
            foreach ([
                'fullTree',
                'moveOosProductsAtTheEnd',
                'autoSyncActiveStatus',
                'maintenanceMode',
                'blurEffect',
                'mobileVisible',
            ] as $configKey) {
        ...
        } elseif (Tools::getIsset('submitPerformanceConfiguration') && Tools::isSubmit('submitPerformanceConfiguration')) {
            $config = $this->getModuleConfiguration();
            foreach ([
                'moduleCache',
                'autoReindex',
                'joinOptimization',
                'sqlCheckCustomersGroups',
                'pageBuilderCompatibilityMode',
            ] as $configKey) {
                if ($configKey == 'pageBuilderCompatibilityMode' && !self::hasProblematicPageBuildersInstalled()) {
                    continue;
                }
                $config[$configKey] = (bool)Tools::getValue($configKey);
            }
    ...
    protected function postDuplicateProcess($params)
    {
        ...
        if ($params['class'] == 'AdvancedSearch\Models\Search') {
            $this->generateCss();
            if (!Tools::getValue('id_shop_destination') || Tools::getValue('id_shop_destination') == (int)$this->context->shop->id) {
                $this->html .= (Tools::getValue('id_shop_destination') ? 'parent.parent.' : '') . 'addTabPanel("#wrapAsTab", ' . json_encode($params['obj']->id . ' - ' . $params['obj']->internal_name) . ',' . (int)$params['obj']->id . ', true);';
            }
        }
    }
    protected function postDeleteProcess($params)
    {
        ...
        if ($params['class'] == 'AdvancedSearch\Models\Search') {
            ...
            if (!count(SearchEngineUtils::getSearchsId(false, (int)$this->context->shop->id))) {
    ...
    protected function renderPerformanceTab()
    {
        $config = $this->getModuleConfiguration();
        $vars = [
            'config' => $config,
            'default_config' => $this->defaultConfiguration,
            'canShowPageBuilderOption' => $this->hasProblematicPageBuildersInstalled(),
        ];
        return $this->fetchTemplate('module/tabs/performance.tpl', $vars);
    }
    public function displaySearchAdminPanel()
    {
        ...
        $vars = [
            ...
            'hasMultipleShops' => count(Shop::getCompleteListOfShopsID()) > 1,
        ];
    ...
    protected function processIndexCriterionsGroup()
    {
        ...
        if (!$criterions_group_type || !(int)$id_search) {
            die;
        }
        $id_criterion_group = SearchEngineIndexation::indexCriterionsGroup((int)$id_search, $criterions_group_type, (int)$id_criterion_group_linked, false, 1, false);
        SearchEngineIndexation::optimizedSearchTables((int)$id_search);
        self::clearSmartyCache((int)$id_search);
    ...
    protected function processDesindexCriterionsGroup()
    {
        ...
        SearchEngineIndexation::desIndexCriterionsGroup((int)$id_search, $criterions_group_type, (int)$id_criterion_group_linked, (int)$id_criterion_group);
        SearchEngineIndexation::optimizedSearchTables((int)$id_search);
        self::clearSmartyCache((int)$id_search);
        $this->html .= '$("#' . $key_criterions_group . ' .loadingOnConnectList").hide().remove();';
        $criterions_groups_to_reindex = SearchEngineIndexation::getCriterionsGroupsIndexed((int)$id_search, (int)$this->context->language->id, false);
    ...
    protected function processDeleteCriterionImg()
    {
        ...
        $file_name = basename($objCriterion->icon[$id_lang]);
        $file_name_final_path = _PS_ROOT_DIR_ . '/modules/pm_advancedsearch4/search_files/criterions/' . basename($file_name);
        ...
        if ($objCriterion->save()) {
            ...
            $this->html .= 'show_error("' . addcslashes($this->l('An error occurred'), '"') . '");';
        }
    }
    protected function processSaveCriterionImg()
    {
        ...
        $file_name = basename(Tools::getValue('file_name'));
        $file_name_temp_path = _PS_ROOT_DIR_ . '/modules/pm_advancedsearch4/uploads/temp/' . basename($file_name);
        ...
        if (Core::isRealFile($file_name_temp_path)) {
            ...
            foreach (Language::getLanguages(false) as $lang) {
                if (!is_array($lang) || !isset($lang['id_lang'])) {
                    continue;
                }
    ...
    protected function processAddCustomCriterionToGroup()
    {
        ...
        if (self::isFilledArray($criterionsGroupList)) {
            foreach ($criterionsGroupList as $criterionsGroupListRow) {
                ...
                $idCriterionParentList = array_map('intval', explode(',', $criterionsGroupListRow[1]));
                if (empty($criterionsGroupListRow[1]) || empty($idCriterionParentList)) {
                    continue;
                }
                foreach ($idCriterionParentList as $idCriterionParent) {
        ...
        if (count($idCriterionParentToDelete)) {
        ...
        if (!count($permissionsErrors)) {
            if ($moduleIsUpToDate) {
                if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                    ...
                    $vars['performance_tab'] = $this->renderPerformanceTab();
    ...
    protected function displayCrontab()
    {
        ...
        if (Core::isFilledArray($searchEngines)) {
            foreach ($searchEngines as $searchEngine) {
                $cronUrls[] = $this->context->link->getModuleLink($this->name, 'cron', ['secure_key' => Configuration::getGlobalValue('PM_AS4_SECURE_KEY'), 'id_search' => (int)$searchEngine['id_search']]);
            }
        }
        $vars = [
            'searchs' => $searchEngines,
            'main_cron_url' => $this->context->link->getModuleLink($this->name, 'cron', ['secure_key' => Configuration::getGlobalValue('PM_AS4_SECURE_KEY')]),
            'cron_urls' => $cronUrls,
            'cron_cli_path' => dirname(__FILE__) . '/controllers/front/cron-cli.php',
        ];
    ...
    protected function displaySearchForm($params)
    {
        ...
        $seo_searchs = [];
        if (self::supportsSeoPages()) {
            $seo_searchs = Seo::getSeoSearchs(false, false, (int)$params['obj']->id);
        }
        $categorySelect = [0 => $this->l('-- User-context category --')] + $this->getCategoryTreeForSelect();
        $whereToSearch = [
        ...
        $vars = [
            ...
            'ukooBanner' => in_array($this->isoLang, ['fr', 'en', 'es']) ? 'banner-' . $this->isoLang . '.png' : 'banner-en.png',
            'ukooBannerUtmSource' => $this->isoLang == 'fr' ? 'AS5' : 'AS5' . $this->isoLang,
        ];
    ...
    protected function displayDuplicateSearchForm()
    {
        $idSearch = (int)Tools::getValue('id_search');
        $searchEngine = new Search($idSearch);
        ...
        $shopList = [
            $this->context->shop->id => sprintf('%s (%s)', $this->context->shop->name, $this->context->shop->getBaseUrl()),
        ];
        foreach (Shop::getShops(false) as $shop) {
            if ((int)$shop['id_shop'] == (int)$this->context->shop->id) {
                continue;
            }
            $shop = new Shop((int)$shop['id_shop']);
            if (!Validate::isLoadedObject($shop)) {
                continue;
            }
            $shopList[(int)$shop->id] = sprintf('%s (%s)', $shop->name, $shop->getBaseUrl() ? $shop->getBaseUrl() : 'N/A');
        }
        $vars = [
            'searchEngine' => $searchEngine,
            'shopsList' => $shopList,
        ];
        $this->html .= $this->fetchTemplate('module/search_engine/duplicate.tpl', $vars);
    }
    ...
    protected function displaySortCriteriaPanel($objCrit)
    {
        if (Tools::getValue('pm_load_function') == 'displaySortCriteriaPanel') {
            ...
            if (Tools::getValue('sort_way')) {
                ...
                if ($objCrit->sort_by == 'position' && $displayType != 9) {
                    $msgConfirm .= "\n" . $this->l('Now, you can sort criteria by dragging and dropping');
                } elseif ($objCrit->sort_by == 'o_position' || $displayType == 9) {
                    $msgConfirm .= "\n" . $this->l('Criteria will automaticaly inherit position');
                }
        ...
        foreach ($criterions as &$row) {
            $hasCustomCriterionsForRow = false;
            $objCritClass = new Criterion((int)$row['id_criterion'], $objCrit->id_search);
            ...
            if (!empty($row['is_custom'])) {
                $hasCustomCriterionsForRow = true;
                $hasCustomCriterions = true;
            }
            if (!$hasCustomCriterionsForRow) {
                $customCriterionsList = Criterion::getCustomCriterions((int)$objCrit->id_search, (int)$objCrit->id, (int)$this->context->language->id);
                if (is_array($customCriterionsList) && count($customCriterionsList)) {
                    $customCriterionsList = [0 => $this->l('None')] + $customCriterionsList;
                }
                $row['custom_criterions_list'] = $customCriterionsList;
                $row['custom_criterions_obj'] = Criterion::getCustomMasterIdCriterion((int)$objCrit->id_search, (int)$row['id_criterion']);
            }
        }
        $config = $this->getModuleConfiguration();
        $autoSyncActiveStatus = (!empty($config['autoSyncActiveStatus']) && in_array($objCrit->criterion_group_type, ['category', 'manufacturer', 'supplier']));
        $vars = [
    ...
    /**
     * Helper function to trick PrestaShop's translation features to detect some strings through $this->l()
     *
     * @param string $type
     * @param int|false $category_level_depth
     *
    ...
    public function getCriterionsGroupsAndCriterionsForSearch($result, $id_lang, $selected_criterion = [], $with_products = false, $id_criterion_group = 0)
    {
        ...
        $hidden_criteria_state = (isset($this->context->cookie->hidden_criteria_state) ? Core::decodeCriteria($this->context->cookie->hidden_criteria_state) : []);
        $selected_criteria_groups_type = [];
        if (empty($selected_criterion)) {
        ...
        foreach ($result as $key => $row) {
            if (!isset($result[$key]['seo_criterion_groups'])) {
                $result[$key]['seo_criterion_groups'] = [];
            }
            if (!isset($result[$key]['selected_criterion'])) {
                $result[$key]['selected_criterion'] = [];
            }
            if (self::supportsSeoPages() && $row['filter_by_emplacement'] && $seoPage !== null && !empty($seoPage->criteria)) {
                $result[$key]['id_seo'] = (int)$seoPage->id;
                if (!empty($seoCriterions) && is_array($seoCriterions)) {
            ...
            if ($this->context->controller instanceof pm_advancedsearch4searchresultsModuleFrontController && !empty($this->context->controller->getSearchEngine()) && $this->context->controller->getSearchEngine()->id != $row['id_search']) {
                $selected_criterion = [];
            }
            ...
            $result[$key]['selected_criterion_from_emplacement'] = [];
            ...
            if ($row['filter_by_emplacement'] && (empty($selected_criterion))) {
                ...
                if (!empty($seoCriterions) && is_array($seoCriterions)) {
                    foreach ($seoCriterions as $critere_seo) {
                        ...
                        if (isset($selected_criterion[$id_criterion_group_seo])) {
                            ...
                            $selected_criterion[$id_criterion_group_seo] = [];
                            $selected_criterion[$id_criterion_group_seo][] = $id_criterion_value;
                        }
                    }
                }
            }
            if (is_array($selected_criterion) && count($selected_criterion)) {
            ...
            if (self::supportsStepSearch() && $row['step_search'] && is_array($selected_criterion) && count($selected_criterion)) {
            ...
            if (!$id_criterion_group) {
                ...
                $result[$key]['criterions_groups'] = CriterionGroup::getCriterionsGroup($row['id_search'], [(int)$id_criterion_group], $id_lang);
            }
            $selectedCriterionsForSeo = [];
                    ...
                    foreach ($result[$key]['selected_criterion_from_emplacement'] as $idCriterionGroupTmp => $selectedCriterionsTmp) {
                        foreach ($result[$key]['criterions_groups'] as $criterionGroupTmp) {
                            ...
                            foreach ($selectedCriterionsTmp as $idCriterionTmp) {
                                ...
                                if (!count($currentSelectedCriterionTmp[$idCriterionGroupTmp])) {
            ...
            $result[$key]['criterionsGroupsMini'] = [];
            ...
            foreach ($result[$key]['criterions_groups'] as $groupKey => &$criterionGroup) {
                if (isset($result[$key]['criterions_groups'][$groupKey + 1])) {
                    ...
                    $nextHasSelectedCriterions = (isset($selected_criterion[$nextGroup['id_criterion_group']]) && count($selected_criterion[$nextGroup['id_criterion_group']]) > 0);
                } else {
            ...
            $result[$key]['criterions_groups_selected'] = [];
            if (is_array($current_selected_criterion) && count($current_selected_criterion)) {
                ...
                foreach ($current_selected_criterion as $id_criterion_group_selected => $selected_criteria) {
                    foreach ($selected_criteria as $criterion_value) {
                        if (!isset($result[$key]['criterions_selected'][$id_criterion_group_selected])) {
                            $result[$key]['criterions_selected'][$id_criterion_group_selected] = [];
                        }
                        if (preg_match('#~#', (string)$criterion_value)) {
                            $range = explode('~', (string)$criterion_value);
                            ...
                            $result[$key]['criterions_selected'][$id_criterion_group_selected][] = [
                                'value' => $this->getTextualRangeValue($range[0], $rangeUp, $groupInfo, $this->context->currency),
            ...
            if (!Tools::getValue('only_products')) {
                ...
                if (!count($result[$key]['criterions_groups'])) {
                    continue;
                }
                ...
                foreach ($result[$key]['criterions_groups'] as $key2 => $row2) {
                    ...
                    if ($row2['range'] == 1) {
                        ...
                        foreach ($ranges as $krange => $range) {
                            ...
                            $range2 = $this->getTextualRangeValue($range, $rangeUp, $row2, $this->context->currency);
                            if (is_array($selected_criterion)) {
                                $citeria4count = $selected_criterion;
                            }
                            $citeria4count[(int)$row2['id_criterion_group']] = [$range1];
                            ...
                            $criteria_formated[$range1] = ['id_criterion' => $range1, 'value' => $range2, 'nb_product' => $nb_products];
                        }
                        ...
                        $result[$key]['criterions'][$row2['id_criterion_group']] = SearchEngineUtils::getPriceRangeForSearchBloc($row, (int)$row2['id_criterion_group'], (int)$this->context->currency->id, (int)$this->context->country->id, (int)$this->getCurrentCustomerGroupId(), self::supportsStepSearch() && $row['step_search'] && !$id_criterion_group && $key2 == 0 ? [] : $range_selected_criterion, self::supportsStepSearch() && $row['step_search'] && !$id_criterion_group && $key2 == 0 ? [] : $selected_criteria_groups_type);
                        ...
                        if (($min_price_id_currency == 0 && $min_price_id_currency != $this->context->cookie->id_currency) || ($min_price_id_currency != 0 && $min_price_id_currency != $this->context->cookie->id_currency)) {
                            $result[$key]['criterions'][$row2['id_criterion_group']][0]['min'] = floor((float)Tools::convertPrice($result[$key]['criterions'][$row2['id_criterion_group']][0]['min_price'], null, $this->context->cookie->id_currency));
                        }
                        if (($max_price_id_currency == 0 && $max_price_id_currency != $this->context->cookie->id_currency) || ($max_price_id_currency != 0 && $max_price_id_currency != $this->context->cookie->id_currency)) {
                            $result[$key]['criterions'][$row2['id_criterion_group']][0]['max'] = ceil((float)Tools::convertPrice($result[$key]['criterions'][$row2['id_criterion_group']][0]['max_price'], null, $this->context->cookie->id_currency));
                        }
                        ...
                        $priceSpecification = $locale->getPriceSpecification($this->context->currency->iso_code);
                        ...
                        if (Core::isFilledArray($selected_criterion) && isset($selected_criterion[$row2['id_criterion_group']]) && count($selected_criterion[$row2['id_criterion_group']])) {
                            $range = explode('~', (string)$selected_criterion[$row2['id_criterion_group']][0]);
                        ...
                        if (empty($idCategoryStart) && isset($selected_criterion[$row2['id_criterion_group']]) && self::isFilledArray($selected_criterion[$row2['id_criterion_group']]) && count($selected_criterion[$row2['id_criterion_group']]) == 1) {
                        ...
                        foreach ($criteriaList as $criterion_row) {
                            if (!isset($childrensList[$criterion_row['id_parent']])) {
                                $childrensList[$criterion_row['id_parent']] = [];
                            }
                            if (count($selected_criterions) > 0 && ($selected_criterions[0] == $criterion_row['id_criterion'])) {
                        ...
                        if (!empty($selected_criterions)) {
                            $this->recursiveGetParents($selected_criterions, $linkedList, (int)$idCategoryStart, $selected_criterions[0]);
                        }
            ...
            if (isset($row['hide_criterions_group_with_no_effect']) && $row['hide_criterions_group_with_no_effect']) {
                foreach ($result[$key]['criterions_groups'] as $criterions_group_key => $criterions_group) {
                    if ($criterions_group['criterion_group_type'] != 'attribute') {
                        ...
                        if (isset($result[$key]['criterions'][$criterions_group['id_criterion_group']]) && count($result[$key]['criterions'][$criterions_group['id_criterion_group']]) == 1) {
                            foreach ($result[$key]['criterions'][$criterions_group['id_criterion_group']] as $criterion_group) {
                                ...
                                if (!$removeCriterionGroup && !isset($criterion_group['nb_product']) && $criterions_group['display_type'] == 5 && isset($criterion_group['min'], $criterion_group['max']) && $criterion_group['min'] == $criterion_group['max']) {
                                    if ($criterions_group['criterion_group_type'] == 'price') {
                                        ...
                                        $selected_criterion[(int)$criterions_group['id_criterion_group']] = [$criterion_group['min'] . '~' . $criterion_group['max']];
            ...
            foreach ($result[$key]['criterions_groups'] as $criterions_group_key => $criterions_group) {
                ...
                $result[$key]['criterions_groups'][$criterions_group_key]['prevent_reset'] = !empty($result[$key]['id_seo']) && !empty($result[$key]['seo_criterion_groups']) && in_array($idCriterionGroup, $result[$key]['seo_criterion_groups']);
                if (
            ...
            $currentSelection = (isset($result[$key]['selected_criterion']) && is_array($result[$key]['selected_criterion']) ? $result[$key]['selected_criterion'] : []);
            if (is_array($result[$key]['criterions_groups_selected'])) {
                foreach ($result[$key]['criterions_groups_selected'] as $selectedCriterionGroup) {
                    if ($result[$key]['hasOneVisibleSelectedCriterion']) {
                        break;
                    }
                    $idCriterionGroup = $selectedCriterionGroup['id_criterion_group'];
                    if (isset($result[$key]['criterions'][$idCriterionGroup])
                        && $selectedCriterionGroup['visible']
            ...
            $result[$key]['invisibleCriterions'] = [];
            foreach ($result[$key]['criterions_groups'] as $criterions_group_key => $criterions_group) {
                if (!empty($criterions_group['visible'])) {
                    continue;
                }
                $idCriterionGroup = (int)$criterions_group['id_criterion_group'];
                if (empty($result[$key]['selected_criterion'][$idCriterionGroup])) {
                    continue;
                }
                foreach ($result[$key]['selected_criterion'][$idCriterionGroup] as $selectedCriterionValue) {
                    if (!isset($result[$key]['invisibleCriterions'][$idCriterionGroup])) {
                        $result[$key]['invisibleCriterions'][$idCriterionGroup] = [];
                    }
                    $result[$key]['invisibleCriterions'][$idCriterionGroup][] = $selectedCriterionValue;
                }
            }
        }
    ...
    protected function recursiveGetParents(&$selected_criterions = [], $linkedList = [], $id_parent = 0, $current = [])
    {
    ...
    protected function getCacheId($name = null)
    {
        ...
        if (SearchEngineUtils::$productFilterListQuery && !is_array(SearchEngineUtils::$productFilterListQuery)) {
    ...
    public function putInSmartyCache($cacheId, $data)
    {
        ...
        if (!empty($config['moduleCache'])) {
            ...
            $this->context->smarty->assign('serialized_data', self::getDataSerialized(json_encode($data)));
            $templatePath = 'module:pm_advancedsearch4/views/templates/hook/' . $this->getPrestaShopTemplateVersion() . '/cache.tpl';
    ...
    public function getFromSmartyCache($cacheId)
    {
        $config = $this->getModuleConfiguration();
        if (!empty($config['moduleCache'])) {
            if (Module::isEnabled('ukooparts') && class_exists('ukooparts') && ukooparts::getFilterCacheKey()) {
                $cacheId .= 'ukooparts' . ukooparts::getFilterCacheKey();
            }
            $cacheId = $this->getCacheId($cacheId);
            $templatePath = 'module:pm_advancedsearch4/views/templates/hook/' . $this->getPrestaShopTemplateVersion() . '/cache.tpl';
            if ($this->isCached($templatePath, $cacheId)) {
                $cacheData = json_decode(self::getDataUnserialized(trim(strip_tags($this->fetch($templatePath, $cacheId)))), true);
                if ($cacheData !== false) {
                    return $cacheData;
                }
            }
    ...
    public function setSmartyVarsForTpl(Search $searchEngine, $selectedCriterions = [])
    {
        $with_product = false;
    ...
    public function displaySearchBlock($hookName, $tplName, $selected_criterion = [], $specific_id_search = false, $fromWidget = false)
    {
        if ($specific_id_search !== false && isset(self::$renderedFacetsCache[(int)$specific_id_search])) {
            return self::$renderedFacetsCache[(int)$specific_id_search];
        }
        if ($this->context->controller instanceof pm_advancedsearch4searchresultsModuleFrontController) {
            $selected_criterion = $this->context->controller->getSelectedCriterions();
        }
        $inlineJsTplName = $this->getPrestaShopTemplateVersion() . '/pm_advancedsearch_js.tpl';
        $newHookName = Hook::normalizeHookName($hookName);
        ...
        $display = '';
        foreach ($searchs as $search) {
            $display .= $this->renderSearchEngine($search, $tplName);
        }
        $this->context->smarty->assign('as_searchs', $searchs);
        if (!self::hasPageBuildersCompatibilityModeEnabled()) {
            self::$displayBeforeBodyClosingTagContent .= $this->display(__FILE__, $inlineJsTplName);
        } else {
            $display .= $this->display(__FILE__, $inlineJsTplName);
        }
        return $display;
    }
    protected function renderSearchEngine($searchEngine, $tplName)
    {
        if (!isset(self::$renderedFacetsCache[(int)$searchEngine['id_search']])) {
            $this->context->smarty->assign('as_searchs', [$searchEngine]);
            self::$renderedFacetsCache[(int)$searchEngine['id_search']] = $this->display(__FILE__, $this->templatePrefix . $tplName);
        }
        return self::$renderedFacetsCache[(int)$searchEngine['id_search']];
    }
    public function assignSearchVar()
    {
        ...
        if (self::isFilledArray($searchs)) {
            $this->includeAssets();
            foreach ($searchs as $search) {
                if ($this->context->controller instanceof pm_advancedsearch4searchresultsModuleFrontController && !empty($this->context->controller->getSearchEngine()) && $this->context->controller->getSearchEngine()->id == (int)$search['id_search']) {
                ...
                $search = $this->getCriterionsGroupsAndCriterionsForSearch([0 => $search], (int)$this->context->language->id, $selectedCriterions, false);
                if (isset($search[0])) {
                    $this->context->smarty->assign([
                        'as_searchs' => $search,
                        'hookName' => 'home',
        ...
        if (SearchEngineUtils::$productFilterListSource == 'best-sales') {
            ...
            if (SearchEngineUtils::$productFilterListSource == 'search' && SearchEngineUtils::$productFilterListData) {
                ...
                if (empty(SearchEngineUtils::$productFilterListQuery)) {
                    ...
                    if (!empty($ambJoliSearch) && Validate::isLoadedObject($ambJoliSearch) && $ambJoliSearch->active) {
                ...
                if (!empty($prestaSearchModule) && Validate::isLoadedObject($prestaSearchModule) && $prestaSearchModule->active && method_exists($prestaSearchModule, 'getFoundProductIDs')) {
    ...
    public function includeAssets()
    {
        ...
        if (!Tools::getValue('ajaxMode')) {
            ...
            $controller->registerStylesheet('modules-' . $this->name . '-css-scenes', _THEME_CSS_DIR_ . 'scenes.css', ['media' => 'all', 'priority' => 900]);
            $controller->registerStylesheet('modules-' . $this->name . '-css-category', _THEME_CSS_DIR_ . 'category.css', ['media' => 'all', 'priority' => 900]);
            $controller->registerStylesheet('modules-' . $this->name . '-css-product_list', _THEME_CSS_DIR_ . 'product_list.css', ['media' => 'all', 'priority' => 900]);
            if (Configuration::get('PS_COMPARATOR_MAX_ITEM') > 0) {
                $controller->registerJavascript('modules-' . $this->name . '-js-comparison', _THEME_JS_DIR_ . 'products-comparison.js', ['position' => 'bottom', 'priority' => 150]);
            }
            $ui_slider_path = Media::getJqueryUIPath('ui.slider', 'base', true);
            foreach ($ui_slider_path['css'] as $ui_slider_css_path => $media) {
                $controller->registerStylesheet('modules-' . $this->name . '-css-ui_slider', $ui_slider_css_path, ['media' => $media, 'priority' => 900]);
            }
            $controller->registerStylesheet('modules-' . $this->name . '-css-main', 'modules/' . $this->name . '/views/css/' . $this->name . '-17.css', ['media' => 'all', 'priority' => 900]);
            $controller->registerStylesheet('modules-' . $this->name . '-css-dyn', 'modules/' . $this->name . '/' . self::DYN_CSS_FILE, ['media' => 'all', 'priority' => 900]);
            $controller->addJqueryUI(['ui.slider', 'ui.core']);
            $controller->registerJavascript('selectize/selectize.min.js', 'modules/' . $this->name . '/views/js/selectize/selectize.min.js', ['position' => 'bottom', 'priority' => 150]);
            $controller->registerStylesheet('selectize/selectize.css', 'modules/' . $this->name . '/views/css/selectize/selectize.css', ['media' => 'all', 'priority' => 900]);
            $controller->registerJavascript('modules-' . $this->name . '-js-2', 'modules/' . $this->name . '/views/js/jquery.ui.touch-punch.min.js', ['position' => 'bottom', 'priority' => 150]);
            $controller->registerJavascript('modules-' . $this->name . '-js-3', 'modules/' . $this->name . '/views/js/jquery.form.js', ['position' => 'bottom', 'priority' => 150]);
            $controller->registerJavascript('modules-' . $this->name . '-js-4', 'modules/' . $this->name . '/views/js/as4_plugin-17.js', ['position' => 'bottom', 'priority' => 160]);
            $controller->registerJavascript('modules-' . $this->name . '-js-5', 'modules/' . $this->name . '/views/js/pm_advancedsearch.js', ['position' => 'bottom', 'priority' => 150]);
        }
        $config = $this->getModuleConfiguration();
    ...
    public function renderWidget($hookName, array $configuration)
    {
        ...
        if (isset($configuration['id_search_engine'])) {
            return $this->displaySearchBlock('-1', 'pm_advancedsearch.tpl', [], (int)$configuration['id_search_engine'], true);
        } else {
            return $this->displaySearchBlock($hookName, 'pm_advancedsearch.tpl', [], false, true);
        }
    }
    ...
    public function hookDisplayAdvancedSearch4($params)
    {
        ...
        return $this->displaySearchBlock('-1', 'pm_advancedsearch.tpl', [], (int)$params['id_search_engine']);
    }
    ...
    public function hookActionObjectUpdateAfter($params)
    {
        ...
        if (isset($params['object']) && is_object($params['object']) && $params['object'] instanceof Shop) {
            if (isset($this->context->controller) && is_object($this->context->controller) && class_exists('AdminThemesController') && $this->context->controller instanceof AdminThemesController) {
    ...
    {
        $this->hookActionAdminProductsControllerSaveAfter($params);
    }
    ...
    {
        $query = $params['query'];
        if ($query->getIdCategory() && $this->isFullTreeModeEnabled() && (!SearchEngineUtils::isSPAModuleActive() || !SearchEngineUtils::isSPAAllowedOnCategory($query->getIdCategory()))) {
    ...
    public function getCategoryProducts(
        ...
        $category = $currentController->getCategory();
        ...
        if ($getTotal) {
            $sql = 'SELECT COUNT(DISTINCT cp.`id_product`) AS total
                    ...
                    ' . (!empty($config['joinOptimization']) ? str_replace('INNER JOIN', 'STRAIGHT_JOIN', Shop::addSqlAssociation('product', 'cp')) : Shop::addSqlAssociation('product', 'cp')) . '
        ...
        $orderBy = !empty($orderBy) && Validate::isOrderBy($orderBy) ? Tools::strtolower($orderBy) : 'position';
        $orderWay = !empty($orderWay) && Validate::isOrderWay($orderWay) ? Tools::strtoupper($orderWay) : 'ASC';
        $orderByPrefix = false;
        ...
        $sql = '
            SELECT p.*, product_shop.*, image_shop.`id_image` id_image, stock.`out_of_stock`, stock.`quantity`
            ...
            LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_shop` product_attribute_shop ON (
                product_shop.`id_product` = product_attribute_shop.`id_product`
            ...
            LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (
                product_shop.`id_product` = pl.`id_product`
                AND pl.`id_lang` = ' . (int)$idLang . Shop::addSqlRestrictionOnLang('pl') . '
            )
            LEFT JOIN `' . _DB_PREFIX_ . 'image_shop` image_shop ON (
                image_shop.`id_product` = product_shop.`id_product`
            ...
            LEFT JOIN `' . _DB_PREFIX_ . 'product_sale` p_sale ON (
                p_sale.`id_product` = product_shop.`id_product`
            ) ' : '') . '
            WHERE c.`id_category` > 0'
            . ($active ? ' AND product_shop.`active` = 1' : '')
            . ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '')
        ...
        ';
        if ($random === true) {
            $sql .= ' ORDER BY ' . SearchEngineUtils::getSortOrderByOosInstruction() . ' RAND() LIMIT ' . (int)$randomNumberProducts;
        } else {
            $sql .= ' ORDER BY ' . SearchEngineUtils::getSortOrderByOosInstruction() . (!empty($orderByPrefix) ? $orderByPrefix . '.' : '') . '`' . bqSQL($orderBy) . '` ' . pSQL($orderWay);
            if ($orderBy == 'orderprice') {
                $sql .= ' LIMIT ' . (max(0, (int)$pageNumber - 3) * (int)$productPerPage) . ',' . ((int)$productPerPage * 5 + (max(0, (int)$pageNumber - 3) * (int)$productPerPage));
            } else {
        ...
        if ($orderBy == 'orderprice') {
            SearchEngineUtils::sortProductsByPrice($result, $orderWay);
            $offset = 0;
            if ($pageNumber <= 3) {
                $offset = ($pageNumber - 1) * $productPerPage;
            } else {
                $offset = (int)$productPerPage * 2;
            }
            $result = array_slice($result, $offset, $productPerPage, true);
        }
    ...
    public function cronTask($idSearch = false)
    {
        ...
        if (!empty($idSearch)) {
            SearchEngineIndexation::reindexSpecificSearch((int)$idSearch);
        } else {
    ...
    public static function clearSmartyCache($idSearch, $idCriterionGroup = null)
    {
        ...
        $templatePath = null;
        $cacheIdToClear = [];
        $forceFileSystemClear = ($smarty instanceof SmartyCustom && method_exists($smarty, 'check_template_invalidation') && is_callable([$smarty, 'check_template_invalidation']));
    ...
    protected function facetedSearchIsEnabled()
    {
        ...
        return is_object($ps_facetedsearch_module) && !empty($ps_facetedsearch_module->active);
    }
    ...
    {
        $value = SearchEngineDb::getVariableValue('max_allowed_packet');
    ...
    public static function hasPageBuildersCompatibilityModeEnabled()
    {
        static $pageBuilderCompatibilityModeEnabled = null;
        if ($pageBuilderCompatibilityModeEnabled === null) {
            $config = self::getModuleConfigurationStatic();
            if (isset($config['pageBuilderCompatibilityMode'])) {
                $pageBuilderCompatibilityModeEnabled = (bool)$config['pageBuilderCompatibilityMode'];
            } else {
                $pageBuilderCompatibilityModeEnabled = false;
            }
        }
        return $pageBuilderCompatibilityModeEnabled;
    }
    public function hasProblematicPageBuildersInstalled()
    {
        return Module::isInstalled('appagebuilder') || Module::isInstalled('iqitelementor');
    }
}
