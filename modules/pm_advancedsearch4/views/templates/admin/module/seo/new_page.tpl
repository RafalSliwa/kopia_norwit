{if is_array($criterions_groups_indexed) && count($criterions_groups_indexed)}
    ...
    <div id="seoSearchPanelCriteriaTabs">
        <div id="seoSearchPanelFilter">
            <input id="seoSearchPanelFilterInput" class="form-control" type="text" value="" placeholder="{l s='Search for your criterion using its name' mod='pm_advancedsearch4'}" />
        </div>
        ...
        {foreach from=$criterions_groups_indexed key=criterion_group_key item=criterions_group_indexed}
            <div id="seoSearchPanelCriteriaTabs-{$criterion_group_key|escape:'htmlall':'UTF-8'}" class="seoSearchPanelCriteriaTabsContent seoSearchPanelCriteriaTabsContent-{$criterions_group_indexed.criterion_group_type|escape:'htmlall':'UTF-8'}">
                ...
                {else}
                    {foreach from=$criterions_group_indexed.criterions item=criterion}
                    <li class="ui-state-default seoSearchCriterionSortable" id="criterion_{$criterions_group_indexed.id_criterion_group|intval}_{$criterion.id_criterion|as4_nofilter}" title="{$criterion.value|escape:'htmlall':'UTF-8'}">
                        {$criterion.value|escape:'htmlall':'UTF-8'}{if !empty($criterion.parent_name)}&nbsp;<span class="seoSearchCriterionCategoryParentName">({l s='parent:' mod='pm_advancedsearch4'} {$criterion.parent_name|escape:'htmlall':'UTF-8'})</span>{/if}
                    </li>
                    {/foreach}
                {/if}
                </ul>
    ...
    <div id="seoSearchPanelCriteriaSelected">
        <div class="ui-widget-content" style="padding:10px;">
            <ul class="ui-helper-reset ui-sortable seoSearchPanelDropZone">
                {if is_array($criteria) && count($criteria)}
                    {foreach from=$criteria item=criterion}
                        <li class="ui-state-default seoSearchCriterionSortable" id="biscriterion_{$criterion|as4_nofilter}" rel="criterion_{$criterion|as4_nofilter}">
                            <span class="ui-icon ui-icon-close" style="float: left; margin-right: .3em;cursor:pointer;" onclick="removeSelectedSeoCriterion(this);"></span> {$criteria_values[$criterion].value|escape:'htmlall':'UTF-8'} {if !empty($criteria_values[$criterion].parent_name)}&nbsp;<span class="seoSearchCriterionCategoryParentName">({l s='parent:' mod='pm_advancedsearch4'} {$criteria_values[$criterion].parent_name|escape:'htmlall':'UTF-8'})</span>{/if}
                            <script type="text/javascript">
                    ...
                    {/foreach}
                {else}
                    <li class="placeholder" style="width: 100%">{l s='Drop your criteria here' mod='pm_advancedsearch4'}</li>
                {/if}
            </ul>
        ...
        $(document).ready(function() {
            $.expr[':'].noticontains = function(a, i, m) {
                return jQuery(a).text().toUpperCase()
                    .indexOf(m[3].toUpperCase()) == -1;
            };
            $(document).on('input', '#seoSearchPanelFilterInput', function() {
                let toSearch = $(this).val().replaceAll('"', '').trim();
                $('.seoSearchPanelCriteriaTabsContent .seoSearchCriterionSortable.seoSearchCriterionHidden').removeClass('seoSearchCriterionHidden');
                if (toSearch.length) {
                    $('.seoSearchPanelCriteriaTabsContent .seoSearchCriterionSortable:noticontains("' + toSearch + '")').addClass('seoSearchCriterionHidden');
                }
            });
            ...
            $("#seoSearchPanelCriteriaSelected ul").droppable({
                ...
                drop: function( event, ui) {
                    ...
                    if (ui.draggable.hasClass("seoSearchCriterionPriceSortable")) {
                        $('<li class="ui-state-default seoSearchCriterionSortable" id="bis' + ui.draggable.attr("id") + '" rel="' + ui.draggable.attr("id") + '"></li>').html('<span class="ui-icon ui-icon-close" style="float: left; margin-right: .3em;cursor:pointer;" onclick="removeSelectedSeoCriterion(this);"></span> ' + ui.draggable.html()).appendTo(this);
                    } else {
                        $('<li class="ui-state-default seoSearchCriterionSortable" id="bis' + ui.draggable.attr("id") + '" rel="' + ui.draggable.attr("id") + '"></li>').html('<span class="ui-icon ui-icon-close" style="float: left; margin-right: .3em;cursor:pointer;" onclick="removeSelectedSeoCriterion(this);"></span> ' + ui.draggable.html()).appendTo(this);
                    }
    ...
    {include file='../../core/clear.tpl'}
    {as4_select obj=$params.obj options=$eligibleMainCriterionsList  label={l s='Main criterion' mod='pm_advancedsearch4'} key='id_criterion_master' defaultvalue=false}
