{if is_array($criterions_groups_indexed) && count($criterions_groups_indexed)}
    {module->displayTitle text="{l s='Add facets (SEO pages) in bulk' mod='pm_advancedsearch4'}"}
    {module->showInfo text="{l s='Select the criteria from the groups below and organize them in a way that is understandable to a human.' mod='pm_advancedsearch4'}"}
    ...
    <div id="seoSearchPanelCriteriaTabs" class="massSeo">
        ...
        {foreach from=$criterions_groups_indexed key=criterion_group_key item=criterions_group_indexed}
            <div id="seoSearchPanelCriteriaTabs-{$criterion_group_key|escape:'htmlall':'UTF-8'}" class="seoSearchPanelCriteriaTabsContent">
                ...
                {as4_button text={l s='Check/uncheck all' mod='pm_advancedsearch4'} onclick="enableAllCriterion4MassSeo(this);" icon_class='done'}
                <br /><br />
                ...
                {elseif $criterions_group_indexed.criterion_group_type == 'price'}
                    ...
                    >
                        ...
                        <div class="pmAsSeoPriceRangeContainer">
                ...
                {else}
                    {foreach from=$criterions_group_indexed.criterions item=criterion}
                        <li class="ui-state-default massSeoSearchCriterion" id="criterion_{$criterion.id_criterion|as4_nofilter}" title="{$criterion.value|escape:'htmlall':'UTF-8'}" onclick="enableCriterion4MassSeo(this);">
                            ...
                            {if !empty($criterion.parent_name)}
                                <span class="seoSearchCriterionCategoryParentName">({l s='parent:' mod='pm_advancedsearch4'} {$criterion.parent_name|escape:'htmlall':'UTF-8'})</span>
                            {/if}
                        </li>
                    {/foreach}
                {/if}
    ...
    <script type="text/javascript">
        var msgMaxCriteriaGroupCountForMass = {$maxCriterionsGroupForMass|json_encode};
        var msgMaxCriteriaForMass = {{l s='You can not select more than %d groups of criteria' mod='pm_advancedsearch4' sprintf=$maxCriterionsGroupForMass}|json_encode};
    ...
    {module->displaySubmit text="{l s='Generate facets' mod='pm_advancedsearch4'}" name="submitMassSeoSearchForm"}

    {include file='../../core/clear.tpl'}

    {as4_endForm id="seoMassSearchForm"}
{else}
    {module->showInfo text="{l s='Before adding a new page, please add criteria groups to your search engine' mod='pm_advancedsearch4'}"}
{/if}
