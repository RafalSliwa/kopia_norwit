{if isset($as_searchs)}
	...
	{foreach from=$as_searchs item=as_search name=as_searchs}
		{assign var='hidden_criteria_group_open' value=false}
		...
		{foreach from=$as_search.criterions_groups item=criterions_group name=criterions_groups}
			{if !empty($criterions_group.display_group)}
				{if $criterions_group.hidden eq '1' && empty($hidden_criteria_group_open)}
		...
		{/if}
		<input type="hidden" name="orderby"{if Tools::getIsset('order') && Tools::getValue('order')} value="{Tools::getValue('order')}"{else} disabled="disabled"{/if} />
