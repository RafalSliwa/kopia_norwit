{if empty($selectionFromGroups)}<section id="js-active-search-filters">{/if}
    ...
    <form action="{$ASSearchUrlForm}" method="POST" class="PM_ASSelectionsBlock PM_ASSelections active_filters" data-id-search="{$as_search.id_search|intval}">
                ...
                {foreach from=$as_search.criterions_groups_selected item=criterions_group name=criterions_groups}
                    {if isset($as_search.criterions[$criterions_group.id_criterion_group]) && $criterions_group.visible && isset($current_selection[$criterions_group.id_criterion_group]) && is_array($current_selection[$criterions_group.id_criterion_group]) && sizeof($current_selection[$criterions_group.id_criterion_group])}
                        {foreach from=$as_search.criterions_selected[$criterions_group.id_criterion_group] key=criterion_key item=criterion name=criterions}
                            {if !empty($criterion.visible) && (isset($criterion.id_criterion) AND isset($as_search.selected_criterion[$criterions_group.id_criterion_group]) AND is_array($as_search.selected_criterion[$criterions_group.id_criterion_group]) AND in_array($criterion.id_criterion, $as_search.selected_criterion[$criterions_group.id_criterion_group])) || isset($criterion.min)}
                                <li>
        ...
        {* Hidden criterions *}
        {foreach from=$as_search.invisibleCriterions item=invisibleIdCriterions key=invisibleIdCriterionGroup}
            {foreach from=$invisibleIdCriterions item=invisibleIdCriterion}
                <input type="hidden" name="as4c[{$invisibleIdCriterionGroup|intval}][]" value="{$invisibleIdCriterion nofilter}" />
                <input type="hidden" name="as4c_hidden[{$invisibleIdCriterionGroup|intval}][]" value="{$invisibleIdCriterion nofilter}" />
            {/foreach}
        {/foreach}
        ...
        {if Tools::getValue('id_seo')}
            <input type="hidden" name="id_seo" value="{Tools::getValue('id_seo')|intval}" />
        {/if}
