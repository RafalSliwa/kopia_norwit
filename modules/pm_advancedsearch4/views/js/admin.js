...
function toggleSearchEngineSettings(realChange) {
    ...
    if (searchType == 0) {
        ...
        $('.enabled-option-step-search').hide(transition);
        ...
        $('.enabled-option-step-search').show(transition);
        // Only apply presets if value is changed from the select
        if (realChange == true) {
    ...
    if (displayEmptyCriterion) {
        $('.hide-empty-criterion-group').hide(transition);
    } else {
        $('.hide-empty-criterion-group').show(transition);
    }
}
