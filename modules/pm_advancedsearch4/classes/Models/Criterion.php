...
{
    const UPLOAD_PATH = '/search_files/criterions/';
    public $id;
    public $id_search;
    ...
    public $position;
    public $is_custom;
    private static $getCriterionsListByIdCriterionGroupCache = [];
    private static $getCriterionsValueListByIdCriterionGroupCache = [];
    private static $getCriterionsStaticIdCache = [];
    private static $getCustomCriterionsCache = [];
    private static $getCriterionValueByIdCache = [];
    private static $getIdCriterionByTypeAndIdLinkedCache = [];
    private static $getIdCriterionByTypeAndValueCache = [];
    private static $getIdCriteriongByURLIdentifierCache = [];
    private static $getIdCriterionGroupByIdCriterionCache = [];
    private static $getCustomCriterionsLinkIdsByGroupCache = [];
    private static $getCustomMasterIdCriterionCache = [];
    private static $getIdCriterionLinkByIdCriterionCache = [];
    protected $tables = [
    ...
    public static $definition = [
        ...
        'multilang' => true,
        'fields' => [
            'id_criterion_group' => ['type' => self::TYPE_INT, 'required' => true, 'validate' => 'isInt'],
            'level_depth' => ['type' => self::TYPE_INT, 'validate' => 'isInt'],
            'color' => ['type' => self::TYPE_STRING, 'size' => 255],
            'visible' => ['type' => self::TYPE_INT, 'validate' => 'isInt'],
            'id_parent' => ['type' => self::TYPE_INT, 'validate' => 'isInt'],
            'position' => ['type' => self::TYPE_INT, 'validate' => 'isInt'],
            'is_custom' => ['type' => self::TYPE_INT, 'validate' => 'isInt'],
            'value' => ['type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isString', 'size' => 255],
            'decimal_value' => ['type' => self::TYPE_STRING, 'lang' => true],
            'url_identifier' => ['type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isString', 'size' => 255],
            'url_identifier_original' => ['type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isString', 'size' => 255],
    ...
    public function __construct($idCriterion = null, $idSearch = null, $idLang = null, $idShop = null)
    {
        ...
        if ($this->id && empty($this->id_criterion_linked)) {
    ...
    public function save($nullValues = false, $autoDate = true)
    {
        ...
        if ($saveResult) {
            self::populateCriterionsLink((int)$this->id_search, (int)$this->id, $this->id_criterion_linked);
            self::addCriterionToList((int)$this->id_search, (int)$this->id, (int)$this->id);
        }
        return $saveResult;
    }
    ...
    public static function getCriterionsListByIdCriterionGroup(int $idSearch, int $idCriterionGroup)
    {
        ...
        self::$getCriterionsListByIdCriterionGroupCache[$cacheKey] = [];
        if (is_array($results)) {
    ...
    public static function getCriterionsValueListByIdCriterionGroup(int $idSearch, int $idCriterionGroup)
    {
        ...
        self::$getCriterionsValueListByIdCriterionGroupCache[$cacheKey] = [];
        if (is_array($results)) {
            foreach ($results as $row) {
    ...
    public static function getCustomCriterions(int $idSearch, int $idCriterionGroup, int $idLang = null)
    {
        ...
        self::$getCustomCriterionsCache[$cacheKey] = [];
        if (is_array($result) && count($result)) {
            foreach ($result as $row) {
                self::$getCustomCriterionsCache[$cacheKey][(int)$row['id_criterion']] = $row['value'];
            }
        }
        return self::$getCustomCriterionsCache[$cacheKey];
    }
    ...
    {
        static $getCustomCriterionsLinkIdsCache = [];
        if (!isset($getCustomCriterionsLinkIdsCache[$idSearch])) {
            ...
            if (is_array($result) && count($result)) {
                ...
                $getCustomCriterionsLinkIdsCache[$idSearch] = [];
            }
        }
        $listToReturn = [];
        $uniqueListToReturn = [];
        foreach ($criterions as $idCriterion) {
            ...
            if (isset($getCustomCriterionsLinkIdsCache[$idSearch][$idCriterion])) {
                ...
                $listToReturn[$idCriterion] += [$idCriterion];
                $uniqueListToReturn = array_merge($uniqueListToReturn, [$idCriterion]);
            }
        }
        if ($uniqueValues) {
    ...
    public static function getCustomCriterionsLinkIdsByGroup(int $idSearch, int $idCriterionGroup)
    {
        ...
        self::$getCustomCriterionsLinkIdsByGroupCache[$cacheKey] = [];
        if (is_array($result) && count($result)) {
            foreach ($result as $row) {
    ...
    public static function getCustomMasterIdCriterion(int $idSearch, int $idCriterion)
    {
        ...
        $result = SearchEngineDb::value('SELECT GROUP_CONCAT(aclist.`id_criterion_parent`)
        FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_criterion_' . (int)$idSearch . '` ac
        JOIN `' . _DB_PREFIX_ . 'pm_advancedsearch_criterion_' . (int)$idSearch . '_list` aclist ON (ac.`id_criterion` = aclist.`id_criterion_parent`)
        WHERE ac.`is_custom`=1 AND aclist.`id_criterion`=' . (int)$idCriterion);
        if (!empty($result)) {
            self::$getCustomMasterIdCriterionCache[$cacheKey] = array_map('intval', explode(',', $result));
        } else {
            self::$getCustomMasterIdCriterionCache[$cacheKey] = [];
        }
        return self::$getCustomMasterIdCriterionCache[$cacheKey];
    }
    public static function getIdCriterionLinkByIdCriterion(int $idSearch, $criterionsList)
    {
        if (!is_array($criterionsList)) {
            $criterionsList = [(int)$criterionsList];
        }
        $cacheKey = sha1(json_encode(func_get_args()));
        ...
        self::$getIdCriterionLinkByIdCriterionCache[$cacheKey] = isset($row['id_criterion_linked']) ? array_map('intval', explode(',', $row['id_criterion_linked'])) : [];
        return self::$getIdCriterionLinkByIdCriterionCache[$cacheKey];
    }
    ...
    public static function populateCriterionsLink(int $idSearch, int $idCriterion, $idCriterionLinked = false, array $criterionsGroupList = [])
    {
        ...
        if (!$idCriterionLinked && is_array($criterionsGroupList) && count($criterionsGroupList)) {
        ...
        } elseif ($idCriterionLinked || is_array($idCriterionLinked) && count($idCriterionLinked)) {
            if (!is_array($idCriterionLinked)) {
                $idCriterionLinked = [$idCriterionLinked];
            }
    ...
    {
        self::$getCriterionsListByIdCriterionGroupCache = [];
        self::$getCriterionsValueListByIdCriterionGroupCache = [];
        self::$getCriterionsStaticIdCache = [];
        self::$getCustomCriterionsCache = [];
        self::$getCriterionValueByIdCache = [];
        self::$getIdCriterionByTypeAndIdLinkedCache = [];
        self::$getIdCriterionByTypeAndValueCache = [];
        self::$getIdCriteriongByURLIdentifierCache = [];
        self::$getIdCriterionGroupByIdCriterionCache = [];
        self::$getCustomCriterionsLinkIdsByGroupCache = [];
        self::$getCustomMasterIdCriterionCache = [];
        self::$getIdCriterionLinkByIdCriterionCache = [];
        parent::clearCache($all);
    }
    public function getHashIdentifier()
    {
        $idCriterionLinked = (is_array($this->id_criterion_linked) ? array_map('intval', $this->id_criterion_linked) : [(int)$this->id_criterion_linked]);
