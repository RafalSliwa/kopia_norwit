...
class Facets implements ProductSearchProviderInterface
{
    ...
    public function __construct(PM_AdvancedSearch4 $module, $translator, $searchInstance, $criterionsList)
    {
        $this->module = $module;
        $this->translator = $translator;
        if (!empty($searchInstance) && is_object($searchInstance)) {
            $this->searchInstance = $searchInstance;
            $this->idSearch = $this->searchInstance->id;
        } else {
            $this->idSearch = null;
        }
    ...
    public function getSortOrders($includeAll = false, $includeDefaultSortOrders = true)
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7.8.0', '<') && ($includeAll || !empty($config['sortOrders']['product.position.asc']))) {
            $sortOrders[] = (new SortOrder('product', 'position', 'asc'))->setLabel($this->module->l('Relevance (reverse)', 'facets'));
        } elseif (version_compare(_PS_VERSION_, '1.7.8.0', '>=') && ($includeAll || !empty($config['sortOrders']['product.position.desc']))) {
        ...
        if ($includeAll || !empty($config['sortOrders']['product.reference.asc'])) {
            $sortOrders[] = (new SortOrder('product', 'reference', 'asc'))->setLabel($this->module->l('Reference, A to Z', 'facets'));
        }
        if ($includeAll || !empty($config['sortOrders']['product.reference.desc'])) {
            $sortOrders[] = (new SortOrder('product', 'reference', 'desc'))->setLabel($this->module->l('Reference, Z to A', 'facets'));
        }
    ...
    public function runQuery(
        ...
        $query->setResultsPerPage((int)Tools::getValue('resultsPerPage', (int)$this->searchInstance->products_per_page));
        ...
        $nbProducts = SearchEngineUtils::getProductsSearched(
            (int)$this->idSearch,
            $this->criterionsList,
            SearchEngineUtils::getCriterionGroupsTypeAndDisplay((int)$this->idSearch, array_keys($this->criterionsList)),
            null,
            null,
            true,
            $query
        );
        $result->setTotalProductsCount((int)$nbProducts);
