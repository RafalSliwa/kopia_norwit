...
abstract class SearchEngineSeo
{
    public static function getCriterionsGroupsIndexedForSEO($id_search, $id_lang)
    {
        $search = current(SearchEngineUtils::getSearch($id_search, $id_lang, false));
    ...
    {
        if (Core::isFilledArray($criterionsList)) {
            $seoKeysList = [];
            foreach ($criterionsList as &$criterion) {
                $criterion['seo_key'] = self::getSeoKeyFromCriteria($idSearch, array_merge($selectedCriterionsForSeo, [(int)$criterion['id_criterion_group'] . '_' . (int)$criterion['id_criterion']]), Context::getContext()->cookie->id_currency);
                $seoKeysList[] = $criterion['seo_key'];
            }
            if (count($seoKeysList)) {
                $seoPageUrlByKeys = Seo::getSeoPageUrlByKeys($seoKeysList, (int)Context::getContext()->language->id);
                foreach ($criterionsList as &$criterion) {
