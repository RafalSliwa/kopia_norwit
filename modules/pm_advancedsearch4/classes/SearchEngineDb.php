...
abstract class SearchEngineDb
{
    ...
    public static function query($query, $type = 1, $useArray = true, $useCache = true)
    {
        ...
        if (!$groupConcatLimitDone) {
            self::setGroupConcatMaxLength($instanceMaster);
            $groupConcatLimitDone = true;
        }
        if (self::$as4SqlLog) {
            $log = preg_replace('/^\s+$/m', ' ', trim($query));
            $log = preg_replace('/^\s+/m', '', trim((string)$log));
            $finalOrigin = [
                'line' => 'UNKNOWN',
                'class' => 'UNKNOWN',
                'function' => 'UNKNOWN',
    ...
    public static function valueList($query, $castFunction = null, $useCache = true)
    {
        $list = [];
    ...
    public static function setGroupConcatMaxLength($instanceMaster)
    {
        return Db::getInstance($instanceMaster)->Execute('SET group_concat_max_len = 33554432');
    }
    public static function getVariableValue($sqlConfigKey)
    {
        $result = self::query('SHOW VARIABLES LIKE \'' . pSQL($sqlConfigKey) . '\'');
        if (empty($result) || !is_array($result) || !isset($result[0]) || !is_array($result[0]) || !isset($result[0]['Value'])) {
            return null;
        }
