...
trait SeoTrait
{
    ...
    {
        return [
            'module-' . _PM_AS_MODULE_NAME_ . '-seo' => [
                'controller' => 'seo',
            ...
            'module-' . _PM_AS_MODULE_NAME_ . '-seositemap' => [
    ...
    protected function postProcessSeoSearch()
    {
        ...
        $this->html = '';
        if (!$id_search) {
            $this->errors[] = $this->l('An error occurred', 'SeoTrait');
        } else {
    ...
    protected function postProcessSeoRegenerate()
    {
        ...
        if (!$fields_to_regenerate || !count($fields_to_regenerate)) {
    ...
    protected function postProcessMassSeoSearch()
    {
        ...
        if (!$id_search) {
            ...
            $criteria_groups = [];
            $rawCriteriaGroups = Tools::getValue('criteria_groups', '');
            if (!empty($rawCriteriaGroups)) {
                $criteria_groups = explode(',', $rawCriteriaGroups);
            }
            $criteria = Tools::getValue('criteria', false);
            $seoIds = [];
            if (empty($criteria_groups) || empty($criteria)) {
                ...
                foreach ($criteria_groups as $key_criterion_group) {
                    ...
                    if (isset($criteria[$id_criterion_group]) && count($criteria[$id_criterion_group])) {
                ...
                foreach ($criteria_cartesian as $criteria_final_str) {
                    ...
                    $objAdvancedSearchSeoClass = new Seo((int)$cur_id_seo);
                    $objAdvancedSearchSeoClass->id_search = $id_search;
                ...
                if (Tools::getValue('massSeoSearchCrossLinks', true) && count($seoIds)) {
    ...
    protected function updateSeoPagesCriteria($oldModuleVersion)
    {
        ...
        $seoPages = Seo::getSeoSearchs(false, true);
    ...
    public function getSeoStrings($criteria, $id_search, $id_currency, $fields_to_get = false)
    {
        ...
        foreach ($this->languages as $language) {
            foreach ($criteria as $k => $criterion) {
                ...
                $objAdvancedSearchCriterionGroupClass = new CriterionGroup((int)$id_criterion_group, $id_search);
                if (preg_match('#~#', $id_criterion)) {
                    ...
                    $objAdvancedSearchCriterionClass = new Criterion((int)$id_criterion, $id_search, $language['id_lang']);
                    $criterion_value = trim($objAdvancedSearchCriterionClass->value);
                }
                foreach ($fieldToFill as $field) {
                    if (!empty($fields_to_get) && is_array($fields_to_get) && !in_array($field, $fields_to_get)) {
                        continue;
                    }
                    if (!isset($defaultReturnSeoStr[$language['id_lang']])) {
                        $defaultReturnSeoStr[$language['id_lang']] = [];
                    }
    ...
    {
        $criteria = Tools::getValue('criteria', false);
        $id_search = (int)Tools::getValue('id_search', false);
        ...
        $seoCriterions = json_encode(explode(',', str_replace('biscriterion_', '', Tools::getValue('criteria'))));
        $eligibleCriterions = $this->getEligibleMainCriterionsList(null, $id_search, $seoCriterions);
        $this->html .= 'var currentSelectedValue = $("select#id_criterion_master").val();';
        $this->html .= '$("select#id_criterion_master option[value!=\"0\"]").remove();';
        $this->html .= '$.each(' . json_encode($eligibleCriterions) . ', function(index, value) { $("<option/>", { value: index, text: value }).appendTo("select#id_criterion_master"); });';
        $this->html .= '$("select#id_criterion_master").val(currentSelectedValue);';
        $vars = [
            'resultTotalProducts' => $resultTotalProducts,
        ];
        $this->html .= $this->fetchTemplate('module/seo/check_seo_criteria.tpl', $vars);
    }
    ...
    public static function getArrayCriteriaFromSeoArrayCriteria($criteria)
    {
        ...
        foreach ($criteria as $value) {
            ...
            $newCriteria[$id_criterion_group][] = (int)$id_criterion;
        }
        return $newCriteria;
    }
    ...
    protected function displaySeoSearchPanelList()
    {
        ...
        $vars = [
            ...
            'sitemap_url' => $this->context->link->getModuleLink(_PM_AS_MODULE_NAME_, 'seositemap', [
    ...
    protected function displaySeoUrl($id_search)
    {
        ...
        $seo_url_by_lang = [];
        foreach ($this->languages as $language) {
            $seo_url_by_lang[(int)$language['id_lang']] = $this->context->link->getModuleLink(_PM_AS_MODULE_NAME_, 'seo', [
                'id_seo' => (int)$ObjAdvancedSearchSeoClass->id,
                'seo_url' => $ObjAdvancedSearchSeoClass->seo_url[$language['id_lang']],
    ...
    protected function displaySeoSearchForm($params)
    {
        ...
        if (Validate::isLoadedObject($params['obj']) && !empty($params['obj']->id_currency)) {
            $default_currency = new Currency((int)$params['obj']->id_currency);
        }
        ...
        foreach ($criterions_groups_indexed as &$criterions_group_indexed) {
            if ($criterions_group_indexed['range'] == 1) {
                ...
                $price_range = SearchEngineUtils::getPriceRangeForSearchBloc($search[0], (int)$criterions_group_indexed['id_criterion_group_linked'], (int)$default_currency->id, (int)$this->getCurrentCustomerGroupId(), (int)$this->context->country->id, [], []);
                ...
                $range = SearchEngineUtils::getCriterionsRange($search[0], (int)$criterions_group_indexed['id_criterion_group'], (int)$this->context->language->id, [], [], false, false, $criterions_group_indexed);
                ...
                if (in_array($criterions_group_indexed['criterion_group_type'], ['category', 'subcategory'])) {
                    foreach ($criterions as $k => $criterion) {
                        $criterions[$k]['parent_name'] = SearchEngineUtils::getCategoryName((int)$criterion['id_parent'], (int)$this->context->language->id);
                    }
                }
                $criterions_group_indexed['criterions'] = $criterions;
            }
        ...
        if (self::isFilledArray($criteria)) {
            foreach ($criteria as &$criterion) {
                ...
                if (preg_match('#~#', $id_criterion)) {
                    ...
                    $objAdvancedSearchCriterionClass = new Criterion((int)$id_criterion, $id_search, (int)$this->context->language->id);
                    $criterion_value = trim($objAdvancedSearchCriterionClass->value);
                    if (in_array($objAdvancedSearchCriterionGroupClass->criterion_group_type, ['category', 'subcategory'])) {
                        $parentName = SearchEngineUtils::getCategoryName((int)$objAdvancedSearchCriterionClass->id_parent, (int)$this->context->language->id);
                    }
                }
        ...
        $eligibleMainCriterionsList = $this->getEligibleMainCriterionsList($params['obj']);
        $vars = [
            'params' => $params,
            ...
            'default_currency_id' => (int)$default_currency->id,
            'default_currency_sign_left' => '',
            'default_currency_sign_right' => $default_currency->getSign('right'),
            'currencies' => Currency::getCurrencies(),
            'eligibleMainCriterionsList' => [0 => $this->l('-- Select criterion to be used as parent for breadcrumb --', 'SeoTrait')] + $eligibleMainCriterionsList,
            'currentSeoPagesCount' => count(Seo::getSeoSearchs(false, false, (int)$id_search)),
        ];
        $this->html .= $this->fetchTemplate('module/seo/new_page.tpl', $vars);
    }
    protected function getEligibleMainCriterionsList($seoPageObj = null, $idSearch = null, $criterions = null)
    {
        $eligibleMainCriterionsList = [];
        if (!empty($seoPageObj) && $idSearch == null) {
            $idSearch = $seoPageObj->id_search;
        }
        if (!empty($seoPageObj) && $criterions == null) {
            $criterions = $seoPageObj->criteria;
        }
        foreach (self::getArrayCriteriaFromSeoArrayCriteria(Core::decodeCriteria($criterions)) as $idCriterionGroup => $idCriterionList) {
            foreach ($idCriterionList as $idCriterion) {
                $criterionGroupObj = new CriterionGroup($idCriterionGroup, $idSearch, (int)$this->context->language->id);
                if (!in_array($criterionGroupObj->criterion_group_type, ['category', 'supplier', 'manufacturer'])) {
                    continue;
                }
                if ($criterionGroupObj->criterion_group_type == 'manufacturer' && version_compare(_PS_VERSION_, '1.7.7.0', '>=') && !Configuration::get('PS_DISPLAY_MANUFACTURERS')) {
                    continue;
                }
                if ($criterionGroupObj->criterion_group_type == 'supplier' && !Configuration::get('PS_DISPLAY_SUPPLIERS')) {
                    continue;
                }
                $criterionObj = new Criterion((int)$idCriterion, $idSearch, (int)$this->context->language->id);
                $eligibleMainCriterionsList[(int)$idCriterion] = sprintf('%s (%s)', $criterionObj->value, $this->criteriaGroupLabels[$criterionGroupObj->criterion_group_type]);
            }
        }
        return $eligibleMainCriterionsList;
    }
    ...
    protected function displaySeoPriceSlider()
    {
        ...
        $price_range = SearchEngineUtils::getPriceRangeForSearchBloc($search[0], (int)$id_criterion_group_linked, (int)$id_currency, (int)$this->getCurrentCustomerGroupId(), (int)$this->context->country->id, [], []);
