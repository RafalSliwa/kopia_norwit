...
abstract class SearchEngineIndexation
{
    use SupportsStepSearch;
    use \AdvancedSearch\Traits\StepSearchTrait;
    private static $booleanCriterionsGroupType = [
        'on_sale',
        'online_only',
        'available_for_order',
        'stock',
    ...
    private static $_getAllProductsIdNotCachedCache = [];
    private static $_getParentsCategoriesIdCache = [];
    private static $_getCategoriesFromProductCache = [];
    private static $_getDefaultAttributePriceCache = [];
    private static $_getDefaultAttributeCache = [];
    private static $_productHasAttributesCache = [];
    public static $defaultSpecificPriceIDPList = [];
    public static $processingObjectIndexation = false;
    private static $_sqlBulkInsertCount = [];
    private static $_getIdCacheProductFromIdProductCache = [];
    private static $_isColorAttributesGroupCache = [];
    ...
    public static function updateCacheProduct($idSearch)
    {
        ...
        if (Core::isFilledArray($getAllProductsIdNotCached_result)) {
    ...
    {
        return SearchEngineDb::query('SELECT p.`id_product`
    ...
    public static function getAllProductsIdNotCached($id_search)
    {
        ...
        $result = SearchEngineDb::query('SELECT p.`id_product`
        FROM `' . _DB_PREFIX_ . 'product` p
        ' . Shop::addSqlAssociation('product', 'p') . '
    ...
    {
        $result = SearchEngineDb::query('SELECT *
        ...
        LIMIT 1');
        if (!$id_lang && $result) {
            $result_lang = SearchEngineDb::query('
                SELECT agl.*
    ...
    {
        $attributeList = [];
        $result = SearchEngineDb::query('SELECT a.id_attribute FROM `' . _DB_PREFIX_ . 'attribute` a WHERE a.`id_attribute_group` = ' . (int)$id_attribute_group);
    ...
    {
        $featureList = [];
        $result = SearchEngineDb::query('SELECT fv.id_feature_value FROM `' . _DB_PREFIX_ . 'feature_value` fv WHERE fv.`id_feature` = ' . (int)$id_feature);
        foreach ($result as $row) {
    ...
    {
        $categoryList = [];
        $result = SearchEngineDb::query('SELECT c.id_category FROM `' . _DB_PREFIX_ . 'category` c
        INNER JOIN `' . _DB_PREFIX_ . 'category_shop` c_shop ON (c_shop.id_category = c.id_category AND c_shop.`id_shop` IN (' . (int)SearchEngineUtils::getShopBySearch($idSearch) . '))');
        foreach ($result as $row) {
    ...
    {
        $supplierList = [];
        $result = SearchEngineDb::query('SELECT s.id_supplier FROM `' . _DB_PREFIX_ . 'supplier` s
        INNER JOIN `' . _DB_PREFIX_ . 'supplier_shop` s_shop ON (s_shop.id_supplier = s.id_supplier AND s_shop.`id_shop` IN (' . (int)SearchEngineUtils::getShopBySearch($idSearch) . '))');
        foreach ($result as $row) {
    ...
    {
        $manufacturerList = [];
        $result = SearchEngineDb::query('SELECT m.id_manufacturer FROM `' . _DB_PREFIX_ . 'manufacturer` m
        INNER JOIN `' . _DB_PREFIX_ . 'manufacturer_shop` m_shop ON (m_shop.id_manufacturer = m.id_manufacturer AND m_shop.`id_shop` IN (' . (int)SearchEngineUtils::getShopBySearch($idSearch) . '))');
        foreach ($result as $row) {
    ...
    {
        $result = SearchEngineDb::query('SELECT acp.`id_cache_product`, pa.`id_product_attribute`, pa.`id_product`
    ...
    public static function getFeatureValuesFromValue($id_feature, $feature_value = false, $id_criterion_linked = false)
    {
        ...
        while ($row = Db::getInstance(_PS_USE_SQL_SLAVE_)->nextRow($resource)) {
            ...
            if (Tools::strlen($featureIdentifier)) {
                if (!isset($featureIdentifierList[$featureIdentifier])) {
                    $featureIdentifierList[$featureIdentifier] = [
                        'name' => [
                            (int)$row['id_lang'] => trim($row['value']),
                        ],
                        'id_feature' => (int)$row['id_feature'],
                        'id_feature_value_list' => [],
                    ];
                }
        ...
        if ($feature_value !== false) {
            $featureIdentifier = Tools::strtolower(trim((string)$feature_value));
            if (isset($featureIdentifierList[$featureIdentifier])) {
                return [
                    $featureIdentifierList[$featureIdentifier],
                ];
    ...
    protected static function getProductWeight(Product $product)
    {
        $idProductAttribute = Product::getDefaultAttribute((int)$product->id);
        if (empty($idProductAttribute)) {
            return (float)$product->weight;
        }
        $combination = new Combination($idProductAttribute);
        if (Validate::isLoadedObject($combination)) {
            return (float)$product->weight + (float)$combination->weight;
        }
        return (float)$product->weight;
    }
    public static function getProductsFieldValues($field, $value = false)
    {
        if ($value) {
            if ($field == 'weight') {
                $where_clause = 'ROUND(p.`' . pSQL($field) . '` + IFNULL(product_attribute_shop.`' . pSQL($field) . '`, 0), 6) = ROUND(' . pSQL($value) . ', 6)';
            } elseif ($field == 'width' || $field == 'height' || $field == 'depth') {
        ...
        if ($field == 'weight') {
            ...
            SELECT DISTINCT (p.`' . pSQL($field) . '` + IFNULL(product_attribute_shop.`' . pSQL($field) . '`, 0)) AS `fieldValue`
            FROM `' . _DB_PREFIX_ . 'product` p
            ' . Shop::addSqlAssociation('product', 'p') . '
            LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute` pa ON (pa.`id_product` = p.`id_product`)
            ' . Shop::addSqlAssociation('product_attribute', 'pa', false, 'product_attribute_shop.`default_on` = 1 AND product_attribute_shop.`weight` <> 0') . '
            WHERE product_shop.`active` = 1' . ($value ? ' AND ' . $where_clause : '') . '
            ORDER BY `fieldValue`');
        } else {
            $results = SearchEngineDb::query('
            SELECT DISTINCT p.`' . pSQL($field) . '` AS `fieldValue`
            FROM `' . _DB_PREFIX_ . 'product` p
            ' . Shop::addSqlAssociation('product', 'p') . '
            WHERE product_shop.`active` = 1' . ($value ? ' AND ' . $where_clause : '') . '
            ORDER BY `fieldValue`');
        }
        if (Core::isFilledArray($results)) {
            foreach ($results as $row) {
                $langList = Language::getLanguages(false);
                $valueRow = [];
                foreach ($langList as $langRow) {
                    $valueRow['name'][$langRow['id_lang']] = $row['fieldValue'];
                }
                $valuesList[] = $valueRow;
            }
        }
    ...
    public static function getCategoriesP($id_search, $idCategory = false, $levelDepth = false)
    {
        ...
        INNER JOIN `' . _DB_PREFIX_ . 'category_shop` c_shop ON (c_shop.id_category = c.id_category AND c_shop.`id_shop` IN (' . SearchEngineUtils::getShopBySearch($id_search) . '))
    ...
    public static function getBooleanCriteria()
    {
        ...
        return [
            [
                'name' => $moduleInstance->translateMultiple('yes'),
                'value' => 1,
            ],
            [
                'name' => $moduleInstance->translateMultiple('no'),
                'value' => 0,
            ],
        ];
    }
    public static function getConditionCriteria($value = false)
    {
        ...
        $return = [
            'new' => [
                'name' => $moduleInstance->translateMultiple('new'),
                'value' => 'new',
            ],
            'used' => [
                'name' => $moduleInstance->translateMultiple('used'),
                'value' => 'used',
            ],
            'refurbished' => [
                'name' => $moduleInstance->translateMultiple('refurbished'),
                'value' => 'refurbished',
            ],
        ];
        if ($value) {
    ...
    public static function getBooleanTrueCriteria($criterionValue = null)
    {
        if ($criterionValue === 0) {
            return [];
        }
    ...
    public static function getProductsIdFromCategory($id_search, $id_category, $recursing_indexing)
    {
        if ($recursing_indexing) {
            ...
            $all_childs_categories = [$id_category];
        }
        return SearchEngineDb::query('
    ...
    public static function getProductsIdFromProductField($id_search, $field_value, $field)
    {
        ...
        if ($field == 'stock') {
            ...
            $where_clause = 'ROUND(p.`' . pSQL($field) . '`, 6) = ROUND(' . pSQL($field_value) . ', 6)';
        } elseif ($field == 'weight') {
            $where_clause = 'ROUND(p.`' . pSQL($field) . '` + IFNULL(product_attribute_shop.`' . pSQL($field) . '`, 0), 6) = ROUND(' . pSQL($field_value) . ', 6)';
        } elseif ($field == 'pack') {
        ...
        $idShop = SearchEngineUtils::getShopBySearch($id_search);
        return SearchEngineDb::query('
                SELECT DISTINCT acp.`id_cache_product`
                FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_' . (int) $id_search . '` acp
                LEFT JOIN `' . _DB_PREFIX_ . 'product` p ON (p.`id_product` = acp.`id_product`)
                ...
                LEFT JOIN `' . _DB_PREFIX_ . 'specific_price` sp ON (
                    ...
                    AND sp.`id_shop` IN (0, ' . (int)$idShop . ')
                )
                ' : '') . '
    ...
    {
        if ($id_shop == false) {
            $id_shop = SearchEngineUtils::getShopBySearch($id_search);
        }
        return SearchEngineDb::query('SELECT sp.*, product_shop.`cache_default_attribute`, product_shop.`price` as default_price,  product_shop.`ecotax`, acp.`id_cache_product`, ' . SearchEngineUtils::getScoreQuery($id_shop, 0, 0, 0, true, true, true) . '
        ...
        AND sp.`id_shop` IN (0, ' . $id_shop . ')
        ' . ($id_product ? ' AND p.`id_product` = ' . (int) $id_product : '') . '
        ' . (class_exists('AdvancedPack') && Core::isFilledArray(\AdvancedPack::getIdsPacks(true)) ? ' AND p.`id_product` NOT IN (' . implode(',', array_map('intval', \AdvancedPack::getIdsPacks(true))) . ')' : '') . '
    ...
    public static function getProductsPriceFromProductTable($id_search, $id_product = false, $id_shop = false)
    {
        ...
        return SearchEngineDb::query('SELECT product_shop.`id_shop`, product_shop.`cache_default_attribute`, p.`id_product`, product_shop.`price`, product_shop.`ecotax`, product_shop.`id_tax_rules_group`, acp.`id_cache_product`
        FROM `' . _DB_PREFIX_ . 'product` p
        INNER JOIN `' . _DB_PREFIX_ . 'product_shop` product_shop ON (product_shop.`id_product` = p.`id_product` AND product_shop.`id_shop` = ' . (int)$id_shop . ')
        LEFT JOIN `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_' . (int) $id_search . '` acp ON (p.`id_product` = acp.`id_product`)
    ...
    public static function getDefaultAttributePrice($id_product_attribute, $id_shop)
    {
        ...
        $row = SearchEngineDb::row('SELECT product_attribute_shop.`price`
    ...
    public static function productHasAttributes($id_product)
    {
        ...
        self::$_productHasAttributesCache[$id_product] = (bool)SearchEngineDb::value('
    ...
    {
        $specific_prices_cache = SearchEngineDb::query('
            ...
            AND app.`id_shop`=' . (int)SearchEngineUtils::getShopBySearch($id_search) . '
            GROUP BY acp.`id_product`, app.`id_shop`, app.`id_currency`, app.`id_country`, app.`id_group`');
        if ($id_product == false) {
            SearchEngineDb::execute('UPDATE `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` SET `valid_id_specific_price`=0');
        } else {
            SearchEngineDb::execute('UPDATE `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` app, `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_' . (int)$id_search . '` acp
            SET app.`valid_id_specific_price`=0
            WHERE app.`id_cache_product`=acp.`id_cache_product` AND acp.`id_product`=' . (int)$id_product);
        }
        if ($specific_prices_cache && Core::isFilledArray($specific_prices_cache)) {
            foreach ($specific_prices_cache as $row) {
                ...
                if ($specific_price && Core::isFilledArray($specific_price)) {
                    SearchEngineDb::execute('UPDATE `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` SET `valid_id_specific_price`=1 WHERE `id_group`=' . (int)$row['id_group'] . ' AND `id_cache_product`=' . (int)$row['id_cache_product'] . ' AND `id_specific_price`=' . (int)$specific_price['id_specific_price']);
                } else {
                    SearchEngineDb::execute('UPDATE `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` SET `has_no_specific`=1 WHERE `id_group`=' . (int)$row['id_group'] . ' AND `id_cache_product`=' . (int)$row['id_cache_product'] . ' AND `id_shop`=' . (int)$row['id_shop']);
                    SearchEngineDb::execute('DELETE FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` WHERE `id_group`=' . (int)$row['id_group'] . ' AND `is_specific`=1 AND `id_cache_product`=' . (int)$row['id_cache_product'] . ' AND `id_shop`=' . (int)$row['id_shop']);
                }
            }
        }
    ...
    public static function setProductsPrices($id_search, $id_product = false)
    {
        $sql_insert_multiple = [];
        $sql_insert_multiple_header = 'INSERT IGNORE INTO `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` (`id_shop`, `id_country`, `id_currency`, `id_group`, `price_wt`, `reduction_amount`, `reduction_type`, `reduction_tax`, `from`, `to`, `is_specific`, `has_no_specific`, `id_specific_price`, `valid_id_specific_price`, `id_cache_product`) VALUES ';
        $select_limit = 10000;
        $select_iteration = 0;
        $products_price = self::getProductsSpecificPrices($id_search, $id_product, false, $select_iteration, $select_limit);
        $nb_products_specific_price = count($products_price);
        $specificPriceDefaultGroup = [];
        $defaultGroupId = SearchEngineUtils::getDefaultGroupId();
        ...
        while ($nb_products_specific_price2 > 0) {
            ...
            if ($nb_products_specific_price2 >= $select_limit2) {
                ...
                $select_iteration2++;
                $nb_products_specific_price2 = count($products_price2);
        ...
        $has_specific_price = [];
        $ecoTaxRate = Configuration::get('PS_ECOTAX_TAX_RULES_GROUP_ID') ? (1 + Tax::getProductEcotaxRate() / 100) : 1;
        while ($nb_products_specific_price > 0) {
            foreach ($products_price as $row) {
                ...
                if ($id_shop == 0) {
                    $liste_id_shop = [SearchEngineUtils::getShopBySearch($id_search)];
                } else {
                    $liste_id_shop = [$id_shop];
                }
                foreach ($liste_id_shop as $id_shop) {
                    ...
                    if (!empty($row['ecotax']) && $row['ecotax'] > 0) {
                        $price += $row['ecotax'] / $ecoTaxRate;
                    }
                    $reduc = 0;
                    ...
                    if (!isset($has_specific_price[$id_shop])) {
                        $has_specific_price[$id_shop] = [];
                    }
                    $has_specific_price[$id_shop][] = (int)$row['id_cache_product'];
                    if (!isset($row['reduction_tax'])) {
                        $row['reduction_tax'] = 1;
                    }
                    $sql_insert_multiple[] = '(' . (int)$id_shop . ', ' . (int)$row['id_country'] . ', ' . (int)$row['id_currency'] . ', ' . (int)$row['id_group'] . ', ' . (float)$price . ', ' . (float)$reduc . ', "' . pSQL($row['reduction_type']) . '", ' . (int)$row['reduction_tax'] . ', "' . $row['from'] . '", "' . $row['to'] . '", 1, 0, ' . (int)$row['id_specific_price'] . ', 0, ' . (int)$row['id_cache_product'] . ')';
                }
        ...
        foreach ($products as $row) {
            ...
            $price = $row['price'];
            if (isset($packList) && in_array($id_product, $packList) && class_exists('AdvancedPack')) {
            ...
            $sql_insert_multiple[] = '(' . (int)$id_shop . ', 0, 0, 0, ' . (float) $price . ', 0, NULL, 1, "0000-00-00 00:00:00", "0000-00-00 00:00:00", 0, ' . (int)$has_no_specific_price_bool . ', NULL, 0, ' . (int) $row['id_cache_product'] . ')';
            self::sqlBulkInsert('pm_advancedsearch_product_price_' . $id_search, $sql_insert_multiple_header, $sql_insert_multiple, 200);
        }
        self::sqlBulkInsert('pm_advancedsearch_product_price_' . $id_search, $sql_insert_multiple_header, $sql_insert_multiple, 1);
    }
    public static function criterionsGroupIsIndexed($criterion_group_type, $id_criterion_group_linked, $id_search, $invisible = false)
    {
        ...
        return isset($row) ? (int)$row['id_criterion_group'] : false;
    }
    ...
    public static function deleteCacheFromIdProduct($id_search, $id_product)
    {
        ...
        if ($idCacheProduct) {
            SearchEngineDb::execute('DELETE FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_criterion_' . (int)$id_search . '` WHERE `id_cache_product` = ' . (int)$idCacheProduct);
            SearchEngineDb::execute('DELETE FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` WHERE `id_cache_product` = ' . (int)$idCacheProduct);
        } else {
            SearchEngineDb::execute('DELETE acpc.* FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_criterion_' . (int)$id_search . '` acpc LEFT JOIN `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_' . (int) $id_search . '` acp ON (acp.`id_cache_product` = acpc.`id_cache_product`) WHERE acp.`id_product` = ' . (int)$id_product);
            SearchEngineDb::execute('DELETE app.* FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$id_search . '` app LEFT JOIN `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_' . (int) $id_search . '` acp ON (acp.`id_cache_product` = app.`id_cache_product`) WHERE acp.`id_product` = ' . (int)$id_product);
        }
    }
    public static function deleteCacheCriterionGroup($id_search, $id_criterion_group)
    {
        SearchEngineDb::execute('DELETE acpc.* FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_criterion_' . (int) $id_search . '` acpc LEFT JOIN `' . _DB_PREFIX_ . 'pm_advancedsearch_criterion_' . (int) $id_search . '` ac ON (ac.`id_criterion` = acpc.`id_criterion`) WHERE ac.`id_criterion_group` = ' . (int)$id_criterion_group);
    }
    ...
    public static function deleteCachePriceGroup($idSearch, $idProduct = null)
    {
        $idCacheProduct = null;
        if ($idProduct) {
            $idCacheProduct = self::getIdCacheProductFromIdProduct($idSearch, $idProduct);
        }
        return SearchEngineDb::execute('
            DELETE app.*
            FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_product_price_' . (int)$idSearch . '` app'
            . ($idCacheProduct ? ' WHERE app.`id_cache_product`=' . (int)$idCacheProduct : '')
        );
    }
    public static function addProductToCache($id_search, $id_product)
    {
        Db::getInstance()->insert('pm_advancedsearch_cache_product_' . (int)$id_search, ['id_product' => (int)$id_product]);
    }
    ...
    public static function indexCriterionsFromProduct($product, $add = false)
    {
        ...
        if (SearchEngineIndexation::$processingAutoReindex && Shop::getContext() != Shop::CONTEXT_SHOP) {
            $advanced_searchs_id = SearchEngineUtils::getSearchsIdWithAutoReindex(false);
        } else {
            $advanced_searchs_id = SearchEngineUtils::getSearchsId(false);
        }
        foreach ($advanced_searchs_id as $id_search) {
            if (!self::getIdCacheProductFromIdProduct($id_search, (int)$product->id)) {
                self::addProductToCache($id_search, (int)$product->id);
            } elseif (!$add) {
                self::deleteCacheFromIdProduct($id_search, (int)$product->id);
            }
            $criterions_groups_indexed = self::getCriterionsGroupsIndexed($id_search, false, false);
            foreach ($criterions_groups_indexed as $row2) {
                if ($row2['criterion_group_type'] == 'manufacturer' && (int)$product->id_manufacturer) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, (int)$product->id_manufacturer);
                } elseif ($row2['criterion_group_type'] == 'supplier' && (int)$product->id_supplier) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, (int)$product->id_supplier);
                } elseif ($row2['criterion_group_type'] == 'feature') {
                    $features = self::getFeaturesFromProduct((int)$product->id, (int)Context::getContext()->language->id, $row2['id_criterion_group_linked']);
                    foreach ($features as $feature) {
                        self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], $feature['id_feature'], $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, $feature['value']);
                    }
                } elseif ($row2['criterion_group_type'] == 'attribute') {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], $row2['id_criterion_group_linked'], $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id);
                } elseif ($row2['criterion_group_type'] == 'category' || $row2['criterion_group_type'] == 'subcategory') {
                    ...
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id);
                } elseif ($row2['criterion_group_type'] == 'prices_drop') {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id);
                } elseif ($row2['criterion_group_type'] == 'on_sale' && $product->on_sale) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, (int)$product->on_sale);
                } elseif ($row2['criterion_group_type'] == 'available_for_order' && $product->available_for_order) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, (int)$product->available_for_order);
                } elseif ($row2['criterion_group_type'] == 'online_only' && $product->online_only) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, (int)$product->online_only);
                } elseif ($row2['criterion_group_type'] == 'stock') {
                    $globalStockAvailable = StockAvailable::getQuantityAvailableByProduct($product->id, 0, SearchEngineUtils::getShopBySearch($id_search));
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, $globalStockAvailable > 0);
                } elseif ($row2['criterion_group_type'] == 'weight') {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, self::getProductWeight($product));
                } elseif ($row2['criterion_group_type'] == 'width' && $product->width) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, $product->width);
                } elseif ($row2['criterion_group_type'] == 'height' && $product->height) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, $product->height);
                } elseif ($row2['criterion_group_type'] == 'depth' && $product->depth) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, $product->depth);
                } elseif ($row2['criterion_group_type'] == 'condition' && $product->condition) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, $product->condition);
                } elseif ($row2['criterion_group_type'] == 'pack' && class_exists('AdvancedPack')) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, (int)\AdvancedPack::isValidPack((int)$product->id));
                } elseif ($row2['criterion_group_type'] == 'new_products' && $product->new) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, (int)$product->id, false, (int)$product->new);
                }
            }
        }
        return true;
    ...
    public static function desIndexCriterionsGroup($id_search, $criterions_group_type, $id_criterion_group_linked, $id_criterion_group = false, $force_delete = false, $desindexGroup = true)
    {
        ...
        if (!$id_criterion_group) {
            $id_criterion_group = CriterionGroup::getIdCriterionGroupByTypeAndIdLinked($id_search, $criterions_group_type, $id_criterion_group_linked);
        }
        ...
        $criterions = Criterion::getCriterionsStatic($id_search, $objAdvancedSearchCriterionGroupClass->id);
        foreach ($criterions as $row) {
            $objAdvancedSearchCriterionClassD = new Criterion($row['id_criterion'], $id_search);
    ...
    public static function removeOldCriterions($idSearch, $idCriterionGroup, $criterionsGroupType, $criterionsGroup = [])
    {
        $idList = [];
        if ($criterionsGroupType == 'attribute') {
            ...
            $valuesListFromCriterions = Criterion::getCriterionsValueListByIdCriterionGroup($idSearch, $idCriterionGroup);
                ...
                if ($duplicateValueCount > 1) {
                    ...
                    while ($arraySearchResult = array_search($duplicateValue, $valuesListFromCriterions) and $arraySearchResult !== false) {
                        if (!isset($idCriterionToMerge[$duplicateValue])) {
                            $idCriterionToMerge[$duplicateValue] = [];
                        }
            ...
            foreach ($idCriterionToMerge as $duplicateValue => $idCriterionList) {
                ...
                foreach ($idCriterionList as $idCriterionToRemove) {
                    $objAdvancedSearchCriterionToRemove = new Criterion((int)$idCriterionToRemove, $idSearch);
                    $objAdvancedSearchCriterionToRemove->delete();
                    $objAdvancedSearchCriterionToRemove->as4ForceClearCache(false);
                }
            }
            $idList = self::getFeaturesIdList($criterionsGroup['id_feature']);
        } elseif ($criterionsGroupType == 'category' || $criterionsGroupType == 'subcategory') {
        ...
        if (Core::isFilledArray($idList)) {
            $currentCriterionsLinkedList = Criterion::getCriterionsListByIdCriterionGroup($idSearch, $idCriterionGroup);
            if (Core::isFilledArray($currentCriterionsLinkedList)) {
                $criterionsToRemove = array_diff($currentCriterionsLinkedList, $idList);
                if (Core::isFilledArray($criterionsToRemove)) {
                    if ($criterionsGroupType == 'feature') {
                        foreach (array_keys($criterionsToRemove) as $idCriterionToRemove) {
                            $objAdvancedSearchCriterionToRemove = new Criterion($idCriterionToRemove, $idSearch);
                            ...
                            if (is_array($objAdvancedSearchCriterionToRemove->id_criterion_linked) && count($objAdvancedSearchCriterionToRemove->id_criterion_linked) > 1) {
                                continue;
                            } else {
                                $objAdvancedSearchCriterionToRemove = new Criterion($idCriterionToRemove, $idSearch);
                                $objAdvancedSearchCriterionToRemove->delete();
    ...
    public static function indexCriterionsGroupFromObject($object, $onlyRemoveOldCriterions = false)
    {
        ...
        $removeCriterionsInformations = [];
        $objectClass = get_class($object);
        if (in_array($objectClass, [
            'Supplier',
            'Manufacturer',
            'Category',
            'FeatureValue',
            ...
            if ($groupType == 'featurevalue') {
                ...
                $removeCriterionsInformations = ['id_feature' => $idCriterionGroupLinked];
            } elseif ($groupType == 'attribute' || $groupType == 'productattribute') {
                $groupType = 'attribute';
                $idObject = false;
                $idCriterionGroupLinked = (int)$object->id_attribute_group;
                $removeCriterionsInformations = ['id_attribute_group' => $idCriterionGroupLinked];
        ...
        if (!empty($groupType)) {
            if (SearchEngineIndexation::$processingAutoReindex && Shop::getContext() != Shop::CONTEXT_SHOP) {
                $searchIds = SearchEngineUtils::getSearchsIdWithAutoReindex(false);
            } else {
                $searchIds = SearchEngineUtils::getSearchsId(false);
            }
            foreach ($searchIds as $idSearch) {
                $criterionsGroups = CriterionGroup::getCriterionsGroupByType($idSearch, $groupType);
                foreach ($criterionsGroups as $groupInfos) {
                    ...
                    if ($onlyRemoveOldCriterions) {
                        ...
                        } elseif ($groupInfos['criterion_group_type'] == 'category' || $groupInfos['criterion_group_type'] == 'subcategory' || $groupInfos['criterion_group_type'] == 'supplier' || $groupInfos['criterion_group_type'] == 'manufacturer') {
    ...
    public static function indexCriterionsGroup($id_search, $criterions_group_type, $id_criterion_group_linked, $id_criterion_group = false, $visible = 1, $checkIfIsIndexed = true, $update = false, $id_product = false, $id_criterion_linked = false, $criterion_value = false)
    {
        ...
        $objSearch = new Search($id_search, (int)$context->language->id);
        $allLangIds = SearchEngineIndexation::getAllIdLang();
        ...
        if ($update) {
            if (!$id_criterion_group) {
                $id_criterion_group = CriterionGroup::getIdCriterionGroupByTypeAndIdLinked($objSearch->id, $criterions_group_type, $id_criterion_group_linked);
            }
            if (!$id_criterion_group) {
                return;
            }
            if ($criterions_group_type == 'price') {
                self::deleteCachePriceGroup($objSearch->id, (int)$id_product);
            }
            if (!$id_product && !$id_criterion_linked) {
                self::deleteCacheCriterionGroup($objSearch->id, (int)$id_criterion_group);
            }
        }
        if ($criterions_group_type == 'attribute') {
            ...
            $moduleInstance = Module::getInstanceByName(_PM_AS_MODULE_NAME_);
            $criterions_group = [
                'name' => $moduleInstance->translateMultiple('categories', $id_criterion_group_linked),
            ];
        } else {
            $moduleInstance = Module::getInstanceByName(_PM_AS_MODULE_NAME_);
            $criterions_group = [
                'name' => $moduleInstance->translateMultiple($criterions_group_type),
            ];
        }
        ...
        $objAdvancedSearchCriterionGroupClass = new CriterionGroup((int)$id_criterion_group, $objSearch->id);
        if (!$update) {
        ...
        if (in_array($criterions_group_type, self::$booleanCriterionsGroupType)) {
            if (!Validate::isLoadedObject($objAdvancedSearchCriterionGroupClass)) {
                $objAdvancedSearchCriterionGroupClass->is_multicriteria = 1;
                $objAdvancedSearchCriterionGroupClass->display_type = 4;
            }
        }
        if (self::supportsStepSearch() && !empty($objSearch->step_search)) {
            self::prepareNewCriterionGroupForStepSearchIndexation($objAdvancedSearchCriterionGroupClass);
        }
        if ($objAdvancedSearchCriterionGroupClass->save()) {
            ...
            if ($update && !$id_product) {
                ...
                } elseif ($criterions_group_type == 'category' || $criterions_group_type == 'subcategory' || $criterions_group_type == 'supplier' || $criterions_group_type == 'manufacturer') {
            ...
            if ($criterions_group_type != 'price') {
                ...
                } elseif (in_array($criterions_group_type, self::$booleanCriterionsGroupType)) {
                    $criterions = self::getBooleanTrueCriteria($criterion_value);
                ...
                $conf = Core::getModuleConfigurationStatic();
                while (count($criterions)) {
                    $position++;
                    $current_id_criterion_linked = null;
                    ...
                    } elseif (in_array($criterions_group_type, [
                        'width',
                        'weight',
                        'depth',
                        'height',
                    ...
                    if ($update) {
                        if ($id_criterion_linked) {
                            $id_criterion = Criterion::getIdCriterionByTypeAndIdLinked($objSearch->id, $objAdvancedSearchCriterionGroupClass->id, $id_criterion_linked);
                        }
                        if (!$id_criterion && $criterion_value) {
                            $id_criterion = Criterion::getIdCriterionByTypeAndValue($objSearch->id, $objAdvancedSearchCriterionGroupClass->id, (int)$context->language->id, $criterion_value);
                        }
                        if (!$id_criterion && $current_id_criterion_linked) {
                            if (is_array($current_id_criterion_linked)) {
                                foreach ($current_id_criterion_linked as $current_id_criterion_linked_value) {
                                    $id_criterion = Criterion::getIdCriterionByTypeAndIdLinked($objSearch->id, $objAdvancedSearchCriterionGroupClass->id, $current_id_criterion_linked_value);
                                    if ($id_criterion) {
                                ...
                                $id_criterion = Criterion::getIdCriterionByTypeAndIdLinked($objSearch->id, $objAdvancedSearchCriterionGroupClass->id, $current_id_criterion_linked);
                            }
                        }
                    }
                    $objAdvancedSearchCriterionClassOld = false;
                    $objAdvancedSearchCriterionClass = new Criterion($id_criterion, $objSearch->id);
                    if ($id_criterion) {
                    ...
                    if (!$id_product) {
                        ...
                        if ($criterions_group_type == 'attribute') {
                            ...
                            $productsIdCache = self::getProductsIdFromCategory($objSearch->id, $row['id_category'], (bool)$objSearch->recursing_indexing);
                        } elseif (in_array($criterions_group_type, self::$booleanCriterionsGroupType) || $criterions_group_type == 'condition') {
                        ...
                        $productsIdCache = [
                            [
                                'id_cache_product' => $idCacheProduct,
                            ],
                        ];
                    }
                    ...
                    if ($criterions_group_type == 'feature' && $objAdvancedSearchCriterionClassOld !== false && !$criterionHasNotChanged) {
                        if (is_array($objAdvancedSearchCriterionClassOld->id_criterion_linked) && count($objAdvancedSearchCriterionClassOld->id_criterion_linked) > 1 && $objAdvancedSearchCriterionClassOld->value != $objAdvancedSearchCriterionClass->value) {
                    ...
                    if ($criterionHasNotChanged || $objAdvancedSearchCriterionClass->save()) {
                        if (!$criterionHasNotChanged) {
                            self::deleteCacheCriterion($objSearch->id, (int)$objAdvancedSearchCriterionClass->id, $idCacheProduct);
                        }
                ...
                self::createVirtualCriterionRelation($objSearch->id, $objAdvancedSearchCriterionGroupClass->id);
            } elseif ($criterions_group_type == 'price') {
                self::setProductsPrices($objSearch->id, $id_product);
                self::setProductsSpecificPrices($objSearch->id, $id_product);
            }
        }
        self::$processingIndexation = false;
        $objAdvancedSearchCriterionClass = new Criterion(null, $objSearch->id);
        $objAdvancedSearchCriterionClass->as4ForceClearCache(true);
        $objAdvancedSearchCriterionGroupClass->as4ForceClearCache(false);
        Criterion::forceUniqueUrlIdentifier($objSearch->id, $objAdvancedSearchCriterionGroupClass->id);
        CriterionGroup::forceUniqueUrlIdentifier($objSearch->id);
    ...
    public static function sqlBulkInsert($tableName, $sql_insert_multiple_header, &$sql_insert_multiple, $size)
    {
        ...
        if (self::$_sqlBulkInsertCount[$tableName] >= $size && count($sql_insert_multiple)) {
            SearchEngineDb::execute($sql_insert_multiple_header . implode(',', $sql_insert_multiple));
            $sql_insert_multiple = [];
            unset(self::$_sqlBulkInsertCount[$tableName]);
        }
    }
    public static function createVirtualCriterionRelation($idSearch, $idCriterionGroup)
    {
        $customCriterions = SearchEngineDb::query('SELECT ac.`id_criterion`, GROUP_CONCAT(aclist.`id_criterion`) as linked_criterions
        FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_criterion_' . (int)$idSearch . '` ac
        LEFT JOIN `' . _DB_PREFIX_ . 'pm_advancedsearch_criterion_' . (int)$idSearch . '_list` aclist ON ac.`id_criterion` = aclist.`id_criterion_parent`
        WHERE ac.`is_custom` = 1
        AND ac.`id_criterion_group` = ' . (int)$idCriterionGroup . '
        GROUP BY ac.`id_criterion`');
        foreach ($customCriterions as $customCriterion) {
            SearchEngineDb::execute('
            DELETE FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_criterion_' . (int)$idSearch . '`
            WHERE `id_criterion` = ' . (int)$customCriterion['id_criterion']);
            if (!empty($customCriterion['linked_criterions'])) {
                SearchEngineDb::execute('INSERT IGNORE INTO `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_criterion_' . (int)$idSearch . '` (
                    SELECT ' . (int)$customCriterion['id_criterion'] . ' as `id_criterion`, `id_cache_product`
                    FROM `' . _DB_PREFIX_ . 'pm_advancedsearch_cache_product_criterion_' . (int)$idSearch . '`
                    WHERE `id_criterion` IN (' . pSQL($customCriterion['linked_criterions']) . ')
                )');
            }
        }
    }
    public static function optimizedSearchTables($idSearch, $optimizeAll = false)
    {
        ...
        foreach ($tableToOptimize as $tableName) {
            SearchEngineDb::execute('ANALYZE TABLE `' . _DB_PREFIX_ . $tableName . '`');
        }
    }
    public static function getIdCacheProductFromIdProduct($id_search, $id_product)
    {
        ...
        if (isset($row['id_cache_product'])) {
            self::$_getIdCacheProductFromIdProductCache[$cacheKey] = (int)$row['id_cache_product'];
            return self::$_getIdCacheProductFromIdProductCache[$cacheKey];
        }
        return false;
    }
    public static function isColorAttributesGroup($id_attribute_group)
    {
        ...
        if ($colorGroupList === null) {
            $result = SearchEngineDb::query('SELECT `id_attribute_group` FROM `' . _DB_PREFIX_ . 'attribute_group` WHERE `is_color_group`=1');
            $colorGroupList = [];
            if (Core::isFilledArray($result)) {
                foreach ($result as $row) {
    ...
    public static function getCategoriesCriteriaGroup($id_search)
    {
        ...
        WHERE `criterion_group_type` IN ("category", "subcategory")');
        return $results;
    }
    public static function getAvailableAttributesGroups($id_lang)
    {
        ...
        if (class_exists('AdvancedPack') && Validate::isLoadedObject($ap5ModuleInstance) && \AdvancedPack::getPackAttributeGroupId() !== false) {
    ...
    {
        PM_AdvancedSearch4::changeTimeLimit(0);
        if ($fromCron) {
            $advanced_searchs_id = SearchEngineUtils::getSearchsId(false, (int)Context::getContext()->shop->id);
        } else {
            $advanced_searchs_id = SearchEngineUtils::getSearchsId(false);
        }
        foreach ($advanced_searchs_id as $idSearch) {
    ...
    public static function reindexSpecificSearch($idSearch)
    {
        ...
        if (Core::isFilledArray($criterions_groups_indexed)) {
            foreach ($criterions_groups_indexed as $row2) {
    ...
    {
        PM_AdvancedSearch4::changeTimeLimit(0);
        $objCritGroup = new CriterionGroup($idCriterionGroup, $idSearch);
