...
<script type="text/javascript">
    const ETS_TRANS_CONTROLLER = "{if isset($controller) && $controller}{$controller|escape:'html':'UTF-8'}{else}{/if}" ;
    const ETS_TRANS_tool_all_value = "{if isset($tool_all_value) && $tool_all_value}{$tool_all_value|escape:'html':'UTF-8'}{else}{/if}" ;
    const ETS_TRANS_ID_COMBINATION = "{if isset($id_combination) && $id_combination}{$id_combination}{else}{/if}" ;
...
{/if}
{if isset($linkScriptBo) && $linkScriptBo}
    <script src="{$linkScriptBo|escape:'html':'UTF-8'}"></script>
{/if}