 ...
 * support@mypresta.eu
 */
class importvoucherVoucherEngine extends importvoucher
{
    public $languages;
    ...
    public function generateReductionType($name)
    {
        ...
        if (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8) {
        ...
        ' . $this->l('Amount') . ' ' . $this->generateInput('reduction_amount') . ' ' . $this->generateCurrencySelect('reduction_currency') . ' ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? $this->generateTaxIncExc('reduction_tax') : '') . '
        </div>';
        return $input;
    }
    ...
    public function generateAttributesSelect($name)
    {
        ...
        $id_lang = Configuration::get('PS_LANG_DEFAULT');
        $result = self::getAttributes($id_lang);
        $cat_tree .= '<div id="' . $this->prefix . $name . '_mr" style="display:block; clear:both; ' . (Configuration::get($this->prefix . $name) == 1 ? '' : 'display:none;') . '"><select name="' . $this->prefix . $name . '_mr[]" style="display:block; clear:both; overflow:auto; border:1px solid #AAAAAA; width:400px; height:160px; " multiple="">';
        foreach ($result as $value) {
    ...
    public function getVoucherEngineSettings($prefix)
    {
        ...
        $return['voucher'] = Configuration::getMultiple(array(
            ...
            $prefix . 'product_rules_dont_combine'
        ));
    ...
    {
        if (parent::psversion() == 4 || parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8) {
            if (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8) {
            ...
            if (parent::psversion() == 7 || parent::psversion(0) >= 8) {
            ...
            Configuration::updateValue($prefix . "product_rules_dont_combine", $post[$prefix . 'product_rules_dont_combine']);
    ...
    public static function AddVoucherCode($prefix, $id_customer = null, $code = null, $name = null, $description = null, $value = null, $quantity = null, $quantity_per_user = null, $cumulable = null, $date_from = null, $date_to = null, $minimal_basket = null, $minimal_basket_currency = null, $currency_id = null, $partial_use = null, $free_shipping = null, $highlight = null, $id_cart_rule = false, $rule_products = false, $rule_categories = false, $product_rules_dont_combine = false)
    {
        if (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8) {
            $context = Context::getContext();
            if (parent::psversion() == 7 || parent::psversion() == 6 || parent::psversion(0) >= 8) {
            ...
            if (Configuration::get($prefix . "excludeSpecials") == 1 && self::psversion() == 7 || parent::psversion(0) >= 8) {
            ...
            if (Configuration::get($prefix . "product_rules_dont_combine") == 1) {
                $product_rules_dont_combine = true;
            }

            if ($voucher->save() == true) {
                self::afterAdd($voucher, $prefix, $rule_products, $rule_categories, $product_rules_dont_combine);
            }
    ...
    public static function afterAdd($currentObject, $prefix, $rule_products = false, $rule_categories = false, $product_rules_dont_combine = false)
    {
        ...
        if (Configuration::get($prefix . 'restriction_products') == 1 || $rule_products != false) {
            ...
            if (count($restrictions_array) > 0) {
                if (!isset($id_product_rule_group) || $product_rules_dont_combine == true) {
    ...
    public function generateForm()
    {
        ...
        ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? $this->context->controller->addJqueryPlugin('datepicker') : '') . '
        ...
        ' . (parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
        ...
        $form .= (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
            ...
            ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
            <div style="display:block; clear:both;">
        ...
        $form .= '
            <label>' . $this->l('Minimum amount') . '</label>
                <div class="margin-form" style="position:relative;">
                    ' . $this->generateInput("minimum_amount", 1) . ' ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? $this->generateCurrencySelect("minimum_amount_currency") : '') . ' ' . $this->generateTaxIncExc("minimum_amount_tax") . ' ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? $this->generateShippingIncExc("minimum_amount_shipping") : '') . '
        ...
        ' . $this->generateInput("quantity_per_user", 1) . '
            ...
            ' . (parent::psversion() == 4 || parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
            <label>[' . $this->l('Product rule') . '] ' . $this->l('Add rule concerning categories') . '</label>
                <div class="margin-form" style="position:relative;">
                    ' . $this->generateCategoriesSelect("restriction_categories") . '
    			</div>' : '') . '
            ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
            <label>[' . $this->l('Product rule') . '] ' . $this->l('Add rule concerning products') . '</label>
                <div class="margin-form" style="position:relative;">
                    ' . $this->generateProductsSelect("restriction_products") . '
    			</div>' : '') . '
            ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
            <label>[' . $this->l('Product rule') . '] ' . $this->l('Add rule concerning attributes') . '</label>
                <div class="margin-form" style="position:relative;">
                    ' . $this->generateAttributesSelect("restriction_attributes") . '
            </div>' : '') . '
            ' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
            <label>[' . $this->l('Product rule') . '] ' . $this->l('Add rule concerning manufacturers') . '</label>
                <div class="margin-form" style="position:relative;">
                    ' . $this->generateManufacturersSelect("res_manufacturers") . '
    			</div>' : '') . '
    		' . (parent::psversion() == 5 || parent::psversion() == 6 || parent::psversion() == 7 || parent::psversion(0) >= 8 ? '
            <label>[' . $this->l('Product rule') . '] ' . $this->l('Add rule concerning suppliers') . '</label>
                <div class="margin-form" style="position:relative;">
                    ' . $this->generateSuppliersSelect("res_suppliers") . '
    			</div>' : '') . '
    		</div>
    		<div style="display:block; clear:both;">
                <label>[' . $this->l('Product rule') . '] ' . $this->l('Do not combine product rules into one') . '</label>
                <div class="margin-form" style="position:relative;">
                    ' . $this->generateYesOrNo("product_rules_dont_combine") . '
    				<p class="preference_description">' . $this->l('By default all rules are combined into one (product that will be a subject of voucher code discount must meet all defined conditions). When you will activate this option discount code will be applied to products that meet at least one defined condition.') . '</p>
    			</div>
            </div>
