...
namespace FacebookProductAd\Common;
...
{
    /**
     * @var dirReader : store the current object
     */
    public static $obj;

    /**
     * @var string : current buffer
     */
    public $sBuffer;
     ...
     */
    private static $_AVAILABLE_CONF = [
        'verbose' => ['value' => false, 'condition' => 'is_bool'],
        'add' => ['value' => false, 'condition' => 'is_bool'],
        'buffer' => ['value' => false, 'condition' => 'is_bool'],
        'error' => ['value' => false, 'condition' => 'is_bool'],
        'strip_tags' => ['value' => false, 'condition' => 'is_bool'],
        'rename' => ['value' => false, 'condition' => 'is_bool'],
        'delete' => ['value' => false, 'condition' => 'is_bool'],
        'lock' => ['value' => null, 'condition' => ['LOCK_SH', 'LOCK_EX', 'LOCK_UN', 'LOCK_NB']],
    ];

    /**
     * @static array $_PROJECT_NAME
     */
    private static $_PROJECT_NAME;
    ...
    public function write($sFileName, $sContent, $bVerbose = false, $bAdd = false, $bStripTag = false)
    {
        ...
        if (is_writable($dirName)) {
            ...
            if ($bStripTag === true || self::$_AVAILABLE_CONF['strip_tags']['value'] === true) {
                $sContent = strip_tags(str_replace(['<br/>', '<BR/>', '<br>', '<BR>'], "\n", $sContent));
            }
            ...
            if ($rHandle === false) {
                ...
                if (self::$_AVAILABLE_CONF['lock']['value'] !== null) {
                    if (flock($rHandle, $this->_sLock)) {
                        ...
                        // throw exception
                        throw new \Exception($this->sDate . " $dirName : " . self::ERR_LOCK_FILE_M, self::ERR_LOCK_FILE_N);
                    }
                } else {
                    $nWrittenData = fwrite($rHandle, $sContent, $nDataLength);
                }
                ...
                if ($nDataLength != $nWrittenData) {
                    throw new \Exception($this->sDate . " $sFileName : " . self::ERR_LENGTH_DATA_WRITE_M, self::ERR_LENGTH_DATA_WRITE_N);
        ...
        if (is_writable($sDirDest)) {
            ...
            if ($bRename === true || self::$_AVAILABLE_CONF['rename']['value'] === true) {
                if (file_exists($sDest) === false) {
                    if (!copy($sSource, $sDest)) {
                        // throw exception
                        throw new \Exception($this->sDate . $sSource . ' in to ' . $sDest . ' : ' . self::ERR_COPY_FILE_M, self::ERR_COPY_FILE_N);
                    } elseif ($bVerbose === true || self::$_AVAILABLE_CONF['verbose']['value'] === true) {
                        self::verbose(' --- File ' . $sSource . ' has been copied ' . $sDest . ' in to directory ' . $sDirDest, 'WRITING');
                    }
                } elseif (!rename($sSource, $sDest)) {
                    // throw exception
                    throw new \Exception($this->sDate . $sSource . ' in to ' . $sDest . ' : ' . self::ERR_RENAME_FILE_M, self::ERR_RENAME_FILE_N);
                } elseif ($bVerbose === true || self::$_AVAILABLE_CONF['verbose']['value'] === true) {
                    self::verbose(' --- File ' . $sSource . ' has been renamed  as ' . $sDest . ' in to directory ' . $sDirDest, 'WRITING');
                }
            ...
            elseif (!file_exists($sDest)) {
                if (!copy($sSource, $sDest)) {
                    ...
                    self::verbose(' --- File ' . $sSource . ' has been copied as ' . $sDest . ' in to directory ' . $sDirDest, 'WRITING');
                }
            } //  use case - display message
            elseif ($bVerbose === true || self::$_AVAILABLE_CONF['verbose']['value'] === true) {
                self::verbose(' --- File ' . $sSource . ' already exists as ' . $sDest . ' in to directory ' . $sDirDest, 'WRITING');
            }
            ...
            if (array_key_exists($sKey, self::$_AVAILABLE_CONF)) {
                if (is_array(self::$_AVAILABLE_CONF[$sKey]['condition'])) {
                    if (in_array($mValue, self::$_AVAILABLE_CONF[$sKey]['condition'])) {
                        ...
                        throw new \Exception('Specified value for ' . $sKey . ' is not avalaible value : ' . implode(' , ', self::$_AVAILABLE_CONF[$sKey]['condition']));
                    }
    ...
    {
        if (null === self::$obj) {
            self::$obj = new fileClass($sLogFile);
        }
    ...
    /**
     * destruct() method destruct the current object
     */
