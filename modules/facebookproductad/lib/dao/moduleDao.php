...
class moduleDao
{
     ...
     * @return mixed
     */
    public static function getProductAttribute($id_product, $attribute_group_id, $id_lang, $id_product_attribute = 0)
    {
        $query = new \DbQuery();
     ...
     * @return mixed
     */
    public static function getProductIds($iShopId, $bExportMode = false, $bCountMode = false, $iFloor = null, $iStep = null, $bExportCombination = false, $bExcludedProduct = false)
    {
        $sQuery = 'SELECT '
    ...
    public static function getProductCombination($id_shop, $id_product, $has_exclusion)
    {
        ...
        if (!empty(\Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE'))) {
            $idShopGroup = \FacebookProductAd::$oContext->shop->id_shop_group;
            $shareStock = moduleTools::getGroupShopDetail('share_stock');
            if ($shareStock) {
                if ($id_shop != \FacebookProductAd::$oContext->shop->id) {
                    $request = new \DbQuery();
                    $request->select('id_shop_group');
                    $request->from('shop');
                    $request->where('id_shop = ' . (int) $id_shop);
                    $data = \Db::getInstance()->getValue($request);
                    /** @phpstan-ignore-next-line */
                    $idShopGroup = $data[0]['id_shop_group'];
                }
                $query->where('sa.id_shop_group = ' . (int) $idShopGroup);
            } else {
    ...
    public static function getShopCategories($id_shop, $id_lang, $home_cat_id = null)
    {
        ...
        if ($home_cat_id !== null) {
            $translations = is_string(\FacebookProductAd::$conf['FPA_HOME_CAT']) ? moduleTools::handleGetConfigurationData(\FacebookProductAd::$conf['FPA_HOME_CAT']) : \FacebookProductAd::$conf['FPA_HOME_CAT'];
        }
