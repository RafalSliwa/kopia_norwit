...
class Ybc_blog_defines extends Module
{
        ...
        if(!isset(self::$configs_image))
        {
            self::$configs_image = array(
                ...
                'YBC_BLOG_ENABLE_CUSTOMER_UPLOAD_AVATA' => array(
                    'label' => $this->l('Allow customers to upload avatar','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'values' => array(
                        array(
        ...
        if(!isset(self::$configs_global))
        {
            self::$configs_global = array(
                ...
                'YBC_BLOG_TEXT_READMORE' => array(
                    ...
                    'default' => 'Read more',
                    'lang'=>true,
                    'desc' => $this->l('Leave blank to disable \'\'Read more\'\' text.','ybc_blog_defines'),
                ),
                ...
                'YBC_BLOG_LABEL_TABLE_OF_CONTENT' => array(
                    'label' => $this->l('Title for table of contents section','ybc_blog_defines'),
                    'type' => 'text',
                ...
                'YBC_BLOG_DISPLAY_NUMBER_INDEX' => array(
                    'label' => $this->l('Display number','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'form_group_class'=> 'table_content',
                    'values' => array(
                ...
                'YBC_BLOG_ALLOW_COMMENT' => array(
                    ...
                    'values' => array(
                        ...
                        array(
                            ...
                            'label' => $this->l('Do not allow customers to comment','ybc_blog_defines'),
                        ),
                    ),
                    'default' => 1,
                    'tab' => 'comment',
                ...
                'YBC_BLOG_ALLOW_REPORT' => array(
                    'label' => $this->l('Allow customers to report a comment as abused','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'tab' => 'comment',
                    'form_group_class' => 'comment',
                ...
                'YBC_BLOG_ALLOW_GUEST_REPORT' => array(
                    'label' => $this->l('Allow guests to report a comment as abused','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 0,
                    'tab' => 'comment',
                    'form_group_class' => 'comment',
                ...
                'YBC_BLOG_ALLOW_RATING' => array(
                    'label' => $this->l('Allow visitors to rate a post when submitting a comment','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'tab' => 'comment',
                    'form_group_class' => 'comment',
                ...
                'YBC_BLOG_CAPTCHA_TYPE' => array(
                    'label' => $this->l('CAPTCHA type','ybc_blog_defines'),
                    'type' => 'select',
                    'options' => array(
                        'query' => array(
                            array(
                ...
                'YBC_BLOG_RTL_MODE' => array(
                    'label' => $this->l('RTL mode','ybc_blog_defines'),
                    'type' => 'select',
                    'options' => array(
                        'query' => array(
                            array(
                ...
                'YBC_BLOG_API_GPT' =>array(
                    ...
                    'validate' => (int)Tools::getValue('YBC_BLOG_ENABLED_GPT') ? 'isApiGPTKey' : 'isCleanHtml',
                    'desc' => Module::getInstanceByName('ybc_blog')->display(Module::getInstanceByName('ybc_blog')->getLocalPath(),'apichatgpt.tpl'),
                    'suffix' => '<button class="btn btn-default btn-check-api-key-gpt">'.$this->l('Check API key', 'ybc_blog_defines').'</button>',
                ),
    ...
    public function getConfigsSeo()
    {
        if(!isset(self::$configs_seo))
        {
            self::$configs_seo = array(
                ...
                'YBC_BLOG_URL_COMPACT' => array(
                    'label' => $this->l('Shortens blog post URLs','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'desc' => $this->l('Shorten long URLs for your blog posts. This is recommended for SEO. Only enable this option if your website does not have any existing .htm URLs with the same structure. This will prevent duplicate URLs.','ybc_blog_defines')."\n".sprintf($this->l('Eg. Your current blog post URL: %s will be shortened to: %s','ybc_blog_defines'),Module::getInstanceByName('ybc_blog')->getLinkExample(),Module::getInstanceByName('ybc_blog')->getLinkExample(true)),
                    'values' => array(
                        array(
                            'id' => 'active_on',
                            'value' => 1,
                ...
                'YBC_BLOG_TITLE' => array(
                    'label' => $this->l('Blog title','ybc_blog_defines'),
                    'type' => 'text',
                    'default' => $this->l('BLOG','ybc_blog_defines'),
                    'lang' => true,
                ),
                'YBC_BLOG_DESCRIPTION' => array(
                    'label' => $this->l('Blog description','ybc_blog_defines'),
                    'type' => 'textarea',
        ...
        if(!isset(self::$configs_socials))
        {
            self::$configs_socials = array(
                ...
                'YBC_BLOG_ENABLE_TWITTER_SHARE' => array(
                    'label' => $this->l('Enable X share','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'values' => array(
                        array(
    ...
    {
        $req = 'SELECT max(`'.bqSQL($primaryKey).'`) as maxid
			FROM `'._DB_PREFIX_.'ybc_blog_'.bqSQL($tbl).'` tbl';
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($req);
        return isset($row['maxid']) ? (int)$row['maxid'] : 0;
    }
    public static function changeStatus($tbl, $field, $id , $status)
    {
        $req = "UPDATE `"._DB_PREFIX_."ybc_blog_".bqSQL($tbl)."` SET `".bqSQL($field)."`=".(int)$status." WHERE id_".bqSQL($tbl)."=".(int)$id;
    ...
    public static function duplicateRowsFromDefaultShopLang($tableName, $shopId,$identifier)
    {
        ...
        $sql = 'INSERT IGNORE INTO `' . bqSQL($tableName) . '` (' . implode(',', array_map('bqSQL',$fields)) . ') (SELECT ';
    ...
    {
        $tbls = array('post', 'post_lang','post_shop','post_category', 'category','category_shop', 'category_lang', 'comment', 'gallery','gallery_shop', 'gallery_lang', 'tag', 'slide', 'slide_shop','slide_lang','employee','employee_lang','log_view','log_like','post_related_categories','reply','polls','email_template','chatgpt_message','post_product');
        foreach($tbls as $tbl)
        {
            Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ybc_blog_'.bqSQL($tbl).'`');
        }
    ...
    public static function getEmployeeByEmail($email)
    {
        ...
        $id_employee = (int)Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($sql);
        if($id_employee)
        {
