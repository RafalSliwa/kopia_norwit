...
class Ybc_blogReportModuleFrontController extends ModuleFrontController
{
	...
	{ 	  
         ...
         if(!$context->cookie->reported_comments || !Validate::isJson($context->cookie->reported_comments))
            $reportedComments = array();
         else
            $reportedComments = json_decode($context->cookie->reported_comments,true);
         
         if(is_array($reportedComments) && !in_array($id_comment, $reportedComments))
         {
             $reportedComments[] = $id_comment;
             $context->cookie->reported_comments = json_encode($reportedComments);
        ...
        if(Configuration::get('YBC_BLOG_ALERT_EMAILS'))
        {
            ...
            if($emails)
            {
                ...
                foreach($emails as $email)
                {
                    ...
                    {
                        if(($employee = Ybc_blog_defines::getEmployeeByEmail($email)) && ($lang = new Language($employee->id_lang)) && $lang->active)
