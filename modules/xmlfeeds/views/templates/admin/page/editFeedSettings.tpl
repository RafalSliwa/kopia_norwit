                ...
                <td>
                    {if !empty($linkZip)}
                        ...
                        {if empty($isZipFileExists)}
                            <div class="alert-small-blmod blmod_mb20" style="padding: 10px 10px;">{$compressorName|escape:'htmlall':'UTF-8'} {l s='file is emtpy. You need to generate it, execute cron job or click on "Open XML" button.' mod='xmlfeeds'}</div>
                        {/if}
                    {/if}
                    {if empty($s.use_cron)}
                        <div id="url_file_box">
                    ...
                    {if !empty($s.use_cron)}
                        <div id="cron_file_box">
                            <p>
                                <input id="feed_url" style="margin-bottom: 10px;" type="text" value="{$cronXmlFile|escape:'htmlall':'UTF-8'}" />
                            ...
                            {if empty($s.last_cron_date) || $s.last_cron_date == '-' || $s.last_cron_date == '0000-00-00 00:00:00'}
                                <div class="alert-small-blmod blmod_mt10 blmod_mb15">
                                    {l s='Cron is enabled but has not been launched yet.' mod='xmlfeeds'}<br>
                                    ...
                                    {l s='P.S. Cron is only useful when you have a large number of products, and the server is very slow. Our module can update the feed (products, orders, etc.) automatically without requiring a cron job.' mod='xmlfeeds'}<br>
                                </div>
                            {else}
                                <a id="feed_url_open" class="btn btn-info" href="{$cronXmlFile|escape:'htmlall':'UTF-8'}" target="_blank">{l s='Open XML' mod='xmlfeeds'}</a>
                                <a id="feed_url_download" class="btn btn-info blmod_ml10i" href="{$cronXmlFile|escape:'htmlall':'UTF-8'}" download>{l s='Download XML' mod='xmlfeeds'}</a>
                                {if !empty($linkZip)}
                                    <a class="btn btn-info blmod_ml10i" href="{$linkZip|escape:'htmlall':'UTF-8'}" target="_blank">{l s='Download ' mod='xmlfeeds'}{$compressorName|escape:'htmlall':'UTF-8'}</a>
                                {/if}
                                <div id="feed_url_copy" class="feed_url_copy_action btn btn-info blmod_ml10i">{l s='Copy URL' mod='xmlfeeds'}</div>
                            ...
                            <div id="cron_install_instruction" style="display: none;">
                                ...
                                <p><b>{l s='1st Option: Set up cron from a server command line (shell) or admin panel' mod='xmlfeeds'}<span class="recommended-box">{l s='Recommended' mod='xmlfeeds'}</span></b></p>
                                ...
                                <p style="margin-top: 15px;"><b>{l s='2nd Option: Run cron from URL or a remote server' mod='xmlfeeds'}</b></p>
                                ...
                                <p style="margin-top: 15px;"><b>{l s='3rd Option: Set up cron with cPanel, Plesk, DirectAdmin or another server management tool' mod='xmlfeeds'}</b></p>
                            </div>
                        </div>
                    {/if}
                </td>
        ...
        <div id="product-feed-settings" class="row">
            ...
            <div id="option_box_3" class="option-box">
                ...
                <div id="option-box-content_3" class="option-box-content" style="display: none;">
                    {include file="{$tpl_dir}/views/templates/admin/editFeedTabs/xmlStructure.tpl" s=$s page=$page moduleImgPath=$moduleImgPath groups=$groups productListWithXmlTags=$productListWithXmlTags attributeGroupList=$attributeGroupList}
                </div>
            </div>
            ...
            {if $s.feed_type eq 2}
                <div id="category-feed-settings" class="row option-box-content">
                    ...
                    {include file="{$tpl_dir}/views/templates/admin/editFeedTabs/xmlStructure.tpl" s=$s page=$page moduleImgPath=$moduleImgPath groups=$groups attributeGroupList=$attributeGroupList}
            ...
            {if $s.feed_type eq 3}
                <div id="category-feed-settings" class="row option-box-content order-feed-settings">
                    {include file="{$tpl_dir}/views/templates/admin/editFeedTabs/general.tpl" s=$s page=$page moduleImgPath=$moduleImgPath name=$name countries=$countries productSettingsPackagesList=$productSettingsPackagesList groups=$groups}
                    {include file="{$tpl_dir}/views/templates/admin/editFeedTabs/price.tpl" s=$s page=$page moduleImgPath=$moduleImgPath priceFromList=$priceFromList currencyList=$currencyList carriersAll=$carriersAll}
                    {include file="{$tpl_dir}/views/templates/admin/editFeedTabs/fields.tpl" s=$s page=$page moduleImgPath=$moduleImgPath}
                    {include file="{$tpl_dir}/views/templates/admin/editFeedTabs/xmlStructure.tpl" s=$s page=$page moduleImgPath=$moduleImgPath groups=$groups attributeGroupList=$attributeGroupList}
