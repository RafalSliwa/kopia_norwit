...
<table border="0" width="100%" cellpadding="3" cellspacing="0">
    ...
    <tr>
        <td class="settings-column-name">{l s='Hide sale price when there is no discount' mod='xmlfeeds'}</td>
        <td>
            <label>
                {include file="{$tpl_dir}/views/templates/admin/helper/status.tpl" id='hide_sale_price' name='hide_sale_price' status=$s.hide_sale_price}
            </label>
            <div class="cb"></div>
            <div class="bl_comments">[{l s='Display sale price only when product / combination has a discount' mod='xmlfeeds'}]</div>
        </td>
    </tr>
    <tr>
    ...
    <tr class="only-product">
        <td class="settings-column-name">{l s='Carrier' mod='xmlfeeds'}</td>
        <td>
            <select name="shipping_price_mode">
                <option value="0" {if empty($s.shipping_price_mode)} selected{/if}>{l s='Default carrier' mod='xmlfeeds'}</option>
                <option value="99999" {if $s.shipping_price_mode eq 99999} selected{/if}>{l s='Best price by country' mod='xmlfeeds'}</option>
                {foreach $carriersAll as $c}
                    <option value="{$c.id_reference|escape:'htmlall':'UTF-8'}" {if $s.shipping_price_mode eq $c.id_reference} selected{/if}>{$c.name|escape:'htmlall':'UTF-8'} {if empty($c.active)}{l s='(Inactive)' mod='xmlfeeds'}{/if}</option>
                {/foreach}
            </select>
        </td>
    </tr>
    <tr>
        <td class="settings-column-name">{l s='Shipping price by minimum quantity for sale' mod='xmlfeeds'}</td>
        <td>
            <label>
                {include file="{$tpl_dir}/views/templates/admin/helper/status.tpl" id='shipping_min_quantity' name='shipping_min_quantity' status=$s.shipping_min_quantity}
            </label>
        </td>
    </tr>
    <tr class="only-product">
    ...
    <tr class="only-product">
        <td class="settings-column-name">{l s='Customer group discount' mod='xmlfeeds'}</td>
        <td>
            <select name="customer_group_id">
                <option value="0"{if empty($s.customer_group_id)} selected{/if}>{l s='None' mod='xmlfeeds'}</option>
                {foreach $customerGroups as $g}
                    <option value="{$g.id_group|escape:'htmlall':'UTF-8'}"{if $s.customer_group_id == $g.id_group} selected{/if}>{$g.name|escape:'htmlall':'UTF-8'}</option>
                {/foreach}
            </select>
        </td>
    </tr>
    <tr class="only-product">
        <td class="settings-column-name">{l s='Apply tax by country' mod='xmlfeeds'}</td>
        <td>
            <select name="tax_country_id">
                <option value="0"{if empty($s.tax_country_id)} selected{/if}>{l s='Default' mod='xmlfeeds'}</option>
                {foreach $countries as $c}
                    <option value="{$c.id_country|escape:'htmlall':'UTF-8'}"{if $c.id_country == $s.tax_country_id} selected{/if}>{$c.name|escape:'htmlall':'UTF-8'}</option>
                {/foreach}
            </select>
        </td>
    </tr>
    <tr class="only-product">
        <td class="settings-column-name">{l s='Use specific fixed price' mod='xmlfeeds'}</td>
        <td>
            <label>
                {include file="{$tpl_dir}/views/templates/admin/helper/status.tpl" id='is_use_specific_fixed_price' name='is_use_specific_fixed_price' status=$s.is_use_specific_fixed_price}
            </label>
        </td>
    </tr>
</table>