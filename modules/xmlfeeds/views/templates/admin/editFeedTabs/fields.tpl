...
<table border="0" width="100%" cellpadding="3" cellspacing="0">
    ...
    <tr class="only-product">
        <td class="settings-column-name">{l s='Label when is not available' mod='xmlfeeds'}</td>
        <td>
            <input type="text" name="label_on_demand_stock_text" value="{$s.label_on_demand_stock_text|escape:'htmlall':'UTF-8'}" size="6">
            <div class="bl_comments">{l s='[Replace default availability label when delivery is not possible]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    <tr class="only-product">
        ...
        <td>
            ...
            <div class="gender_field_category" style="display: none; margin-top: 20px;">
                ...
                <table cellspacing="0" cellpadding="0" class="table blmod-table-light table-no-space" id="radio_div" style="">
                    ...
                    {if !empty($categoriesTreeGender)}
                        {foreach $categoriesTreeGender as $c}
                            <tr class="">
                                <td style="">
                                ...
                                <td>
                                    ...
                                    <div style="float: left;">
                                        <label style="line-height: 26px;" for="categoryWithoutBox_{$c.id|escape:'htmlall':'UTF-8'}" class="t">
                                            {$c.name|escape:'htmlall':'UTF-8'}
                                        </label>
                                    </div>
                                    <div style="clear: both;"></div>
                                    <div>
                                        <input type="text" placeholder="Enter gender" name="gender_category[{$c.id|escape:'htmlall':'UTF-8'}]" value="{$c.genderValue|escape:'htmlall':'UTF-8'}">
                                    </div>
                                    <div style="clear: both;"></div>
                                </td>
                            </tr>
    ...
    <tr class="only-product">
        <td class="settings-column-name">{l s='Merge short to long description' mod='xmlfeeds'}</td>
        <td>
            <label>
                {include file="{$tpl_dir}/views/templates/admin/helper/status.tpl" id='merge_descriptions' name='merge_descriptions' status=$s.merge_descriptions}
            </label>
        </td>
    </tr>
    <tr class="only-product">
        <td class="settings-column-name">{l s='MPN value from attribute' mod='xmlfeeds'}</td>
        <td>
            <label>
                {include file="{$tpl_dir}/views/templates/admin/helper/status.tpl" id='mpn_from_attribute' name='mpn_from_attribute' status=$s.mpn_from_attribute}
            </label>
        </td>
    </tr>
    <tr class="only-product">
        <td class="settings-column-name">{l s='Exclude empty attributes' mod='xmlfeeds'}</td>
        <td>
            <label>
                {include file="{$tpl_dir}/views/templates/admin/helper/status.tpl" id='is_skip_empty_attribute' name='is_skip_empty_attribute' status=$s.is_skip_empty_attribute}
            </label>
        </td>
    </tr>
    <tr class="only-product">
        <td class="settings-column-name">{l s='Display product ID when connected field empty' mod='xmlfeeds'}</td>
        <td>
            <input type="text" name="connected_value_prefix" value="{$s.connected_value_prefix|escape:'htmlall':'UTF-8'}" size="6" placeholder="Prefix">
            <div class="bl_comments">{l s='[Result when connected value empty: PrefixProductId (ex: P-123)]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    <tr class="only-product">
        <td class="settings-column-name">{l s='Product ID from combination ID separator' mod='xmlfeeds'}</td>
        <td>
            <input type="text" name="combination_id_separator" value="{$s.combination_id_separator|escape:'htmlall':'UTF-8'}" size="6" placeholder="Default: -">
        </td>
    </tr>
</table>