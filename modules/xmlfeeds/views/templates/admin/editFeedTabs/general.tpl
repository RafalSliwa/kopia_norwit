...
<table border="0" width="100%" cellpadding="3" cellspacing="0">
    ...
    <tr>
        ...
        <td>
            ...
            {if !empty($s.feed_mode) && $s.feed_mode != "rpro"}<img class="feed_type_id" alt="Feed type" title="Feed type" src="../modules/{$name|escape:'htmlall':'UTF-8'}/views/img/type_{$s.feed_mode|escape:'htmlall':'UTF-8'}.png" />{/if}
            {if $s.feed_mode == "rpro"}<img style="margin-left: 2px; padding-right: 0; vertical-align: middle!important;" alt="Feed type" src="../modules/{$name|escape:'htmlall':'UTF-8'}/views/img/readypro-logo-ls.png" />{/if}
        </td>
    </tr>
    <tr>
    ...
    <tr class="cron-file-settings-box"{if empty($s.use_cron)} style="display: none;"{/if}>
        <td class="settings-column-name">{l s='Cron file name' mod='xmlfeeds'}</td>
        <td>
            <input style="width: 337px;" type="text" name="cron_file_name" value="{$s.cron_file_name|escape:'htmlall':'UTF-8'}">.xml
            <div class="clear_block"></div>
            <div class="bl_comments">{l s='[Can be empty, in this case the module will create automatically]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    <tr class="cron-file-settings-box"{if empty($s.use_cron)} style="display: none;"{/if}>
        <td class="settings-column-name">{l s='Generate in chunks' mod='xmlfeeds'}</td>
        <td>
            <input style="width: 130px;" type="text" name="cron_chunks_limit" placeholder="{l s='Products per chunk' mod='xmlfeeds'}" value="{$s.cron_chunks_limit|escape:'htmlall':'UTF-8'}">
            <div class="clear_block"></div>
            <div class="bl_comments">{l s='[Can be empty, it is recommended to generate in chunks when you have a lot of products or the server is slow. Specify how many products to generate at once. Example: 1000]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    ...
    <tr class="only-product">
        ...
        <td>
            ...
            <div class="clear_block"></div>
            <div class="bl_comments">{l s='[Divide the feed into multiple parts based on the number of products]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    {if empty($s.use_cron)}
        <tr>
    ...
    <tr class="only-product">
        <td class="settings-column-name">{l s='Products, order by' mod='xmlfeeds'}</td>
        <td>
            <input type="text" name="order_by_column" value="{$s.order_by_column|escape:'htmlall':'UTF-8'}">
            <div class="bl_comments">{l s='[SQL query, example: PREFIX_product.id_product ASC]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    {if $s.feed_mode == 'vi'}
        <tr>
            <td class="settings-column-name special-feed-row">{l s='Vivino, bottle size' mod='xmlfeeds'}</td>
            <td>
        ...
        <tr>
            <td class="settings-column-name special-feed-row">{l s='Vivino, lot size' mod='xmlfeeds'}</td>
            <td>
    ...
    {if $s.feed_mode == 'spa'}
        <tr>
            <td class="settings-column-name special-feed-row">{l s='Spartoo, size' mod='xmlfeeds'}</td>
            <td>
    ...
    {if $s.feed_mode == 's' || $s.feed_mode == 'bp'}
        <tr>
            <td class="settings-column-name special-feed-row">{l s='Skroutz, Analytics ID' mod='xmlfeeds'}</td>
            <td>
        ...
        <tr>
            <td class="settings-column-name special-feed-row">{l s='Skroutz, Display variants' mod='xmlfeeds'}</td>
            <td>
                <label for="skroutz_variant_size">
                    {include file="{$tpl_dir}/views/templates/admin/helper/status.tpl" id='skroutz_variant_size' name='skroutz_variant_size' status=$s.skroutz_variant_size}
                </label>
                <div class="clear_block"></div>
                <div class="bl_comments">{l s='[Display product combinations as Skroutz variants. With this feature you need to select and "Skroutz, Merge combinations by attributes"]' mod='xmlfeeds'}</div>
            </td>
        </tr>
        {if !empty($groups)}
            <tr class="only-product">
                <td class="settings-column-name special-feed-row">{l s='Skroutz, Merge combinations by attributes' mod='xmlfeeds'}</td>
                <td>
                    <script type="text/javascript">
                        $(document).ready(function(){
                            boxToggle("merge_attributes_by_group_list");
                        });
                    </script>
                    <label for="merge_attributes_by_group_list_status">
                    ...
                    <span class="merge_attributes_by_group_list_button" style="cursor: pointer; color: #268CCD; margin-left: 10px;">{l s='[Show/Hide attribute groups]' mod='xmlfeeds'}</span>
                    <div class="merge_attributes_by_group_list" style="display: none; margin-top:10px;">{$attributeGroupList}</div>
                    <div class="bl_comments">{l s='[Especially useful for colors and sizes variants. Most of times need to select color]' mod='xmlfeeds'}</div>
                </td>
            </tr>
        {/if}
    {/if}
    {if $s.feed_mode == 'wor'}
        <tr>
            <td class="settings-column-name special-feed-row">{l s='Worten, ship from country' mod='xmlfeeds'}</td>
            <td>
