...
<table border="0" width="100%" cellpadding="3" cellspacing="0">
    ...
    <tr class="only-product">
        <td class="settings-column-name">{l s='Add all images' mod='xmlfeeds'}</td>
        <td>
    ...
    <tr id="additional-image-name-box"{if empty($s.all_images)} style="display: none;"{/if}>
        <td class="settings-column-name">{l s='Additional image name' mod='xmlfeeds'}</td>
        <td>
            <input type="text" name="additional_image_name" value="{$s.additional_image_name|escape:'htmlall':'UTF-8'}">
            <div class="cb"></div>
            <div class="bl_comments">{l s='[Enter a XML tag name if you want to display additional images in a separate XML element.]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    <tr>
    ...
    <tr class="only-product">
        <td class="settings-column-name">{l s='Product limit' mod='xmlfeeds'}</td>
        <td>
            <input type="text" name="item_limit" value="{$s.item_limit|escape:'htmlall':'UTF-8'}" placeholder="Default: unlimited">
            <div class="bl_comments">{l s='[Max number of products that can be displayed. Empty value - unlimited.]' mod='xmlfeeds'}</div>
        </td>
    </tr>
    <tr class="only-product">
        <td class="settings-column-name">{l s='Attribute, feature structure' mod='xmlfeeds'}</td>
        <td>
            <select name="attribute_structure_id">
                <option value="0">{l s='Default' mod='xmlfeeds'}</option>
                {foreach $attributeStructures as $id => $v}
                    <option value="{$id|escape:'htmlall':'UTF-8'}" {if $s.attribute_structure_id == $id}selected{/if}>{$v|escape:'htmlall':'UTF-8'}</option>
                {/foreach}
            </select>
            <div class="bl_comments">
                {l s='NAME - the name of attribute,feature; VALUE - the value of attribute,feature' mod='xmlfeeds'}
            </div>
        </td>
    </tr>
</table>