...
class OrderFieldsAdmin extends Xmlfeeds
{
    public function getFieldSettings($page, $moduleImgPath)
    {
        ...
        $lang_array = [];
        $lang_array[] = ['name' => 0, 'title' => 'Default', 'table' => 'lang', 'only_checkbox' => 1,];

        if (!empty($languages)) {
            foreach ($languages as $lan) {
                $lang_array[] = ['name' => $lan['id_lang'], 'title' => $lan['name'], 'table' => 'lang', 'only_checkbox' => 1,];
            }

            $langBlock = $this->printBlock('Languages', $lang_array, 1, 1);
        }
