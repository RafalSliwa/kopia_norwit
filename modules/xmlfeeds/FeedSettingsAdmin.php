...
class FeedSettingsAdmin extends Xmlfeeds
{
    ...
    public function manufacturersList($active = '')
    {
    ...
    public function getAttributeGroupList($activeGroupOld, $activeGroups)
    {
        $groups = AttributeGroupCore::getAttributesGroups($this->langId);

        $activeGroups = !empty($activeGroups) ? $activeGroups : [];

        if (!empty($activeGroupOld)) {
            $activeGroups[] = $activeGroupOld;
        }

        $this->smarty->assign([
            'groups' => $groups,
            'activeList' => $activeGroups,
        ]);

        return $this->displaySmarty('views/templates/admin/element/attributeGroupList.tpl');
    }

    public function supplierList($active = '')
    {
        $class = 'SupplierCore';
    ...
    {
        $states = Db::getInstance()->ExecuteS('SELECT sl.id_order_state, sl.name, s.color
    ...
    {
        $paymentModules = Db::getInstance()->ExecuteS('SELECT DISTINCT m.`name`
        ...
        $paymentModulesFromOrders = Db::getInstance()->ExecuteS('SELECT o.`module` AS `name`, o.`payment` AS displayName
    ...
    public function getFilterAttributesHtml($s, $isWithout = false)
    {
        ...
        $groups = AttributeGroupCore::getAttributesGroups($this->langId);
        $onlyWithAttributesActive = !empty($s['only_with_attributes']) ? explode(',', $s['only_with_attributes']) : [];
        $onlyWithoutAttributesActive = !empty($s['only_without_attributes']) ? explode(',', $s['only_without_attributes']) : [];
