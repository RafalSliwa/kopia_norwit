...
{
    const CSS_VERSION = 'v32';
    const DATABASE_VERSION = 'v3';
    ...
    public function install()
    {
        ...
        if (!class_exists('SystemCustomer', false)) {
            include_once(dirname(__FILE__) . '/SystemCustomer.php');
        }
        ...
        $systemCustomer = new SystemCustomer();
        $systemCustomer->addIfNotExists();
    ...
    public function loadModule()
    {
        ...
        include_once(dirname(__FILE__).'/ConfigurationBlx.php');
        include_once(dirname(__FILE__).'/UserPermissionsBlx.php');
    ...
    public function getContent()
    {
        ...
        $this->validateAndInstallMandatoryDatabaseTables();
        ...
        $xmlExportSettings->isExportAction($this->version, $this->nameFull);
    ...
    {
        $userPermissionsBlx = new UserPermissionsBlx();
        ...
        $isLoginAction = Tools::getValue('login_admin_action');

        if (!empty($isLoginAction)) {
            $password = Tools::getValue('admin_password');

            if ($userPermissionsBlx->isPasswordValid($password)) {
                $userPermissionsBlx->addPasswordToCookie($contextCookie, $userPermissionsBlx->getEncryptedPassword($password));
            } else {
                $this->smarty->assign([
        ...
        if (!$userPermissionsBlx->isAllowViewPage($contextCookie)) {
            $this->loginPage();
            $currentPage['type'] = 'login_page';
    ...
    public function loginPage()
    {
        ...
        $this->templates .= $this->displaySmarty('views/templates/admin/page/login.tpl');
    }
    ...
    public function aboutPage()
    {
        ...
        $isUpdatePassword = Tools::getValue('update_admin_password');

        if (!empty($isUpdatePassword)) {
            $this->notification->addConf('Password successfully updated');
            $userPermissionsBlx->updatePassword(Tools::getValue('admin_password'));
        }
        ...
        if (!empty($isDbUpgrade)) {
            ...
            $this->notification->addConf($this->l('Module database updated successfully'));
        }
        ...
        $this->smarty->assign([
            ...
            'currentPassword' => $userPermissionsBlx->getPassword(),
            'fullAdminUrl' => $this->fullAdminUrl.'&about_page=1',
        ]);
    ...
    public function checkGetDefaultFeed()
    {
        $feed = Db::getInstance()->getRow('SELECT `id`
			...
			ORDER BY feed_type ASC, `id` DESC');
    ...
    public function addAffiliatePrice($full_address_no_t, $token)
    {
        ...
        $this->smarty->assign([
            ...
            'prices' => $prices,
            'token' => $token,
            'categoriesTree' => $this->categoryTreeGenerator->categoriesTree(false, false, true),
            'full_address_no_t' => $full_address_no_t,
    ...
    public function assignGoogleCategoriesPage()
    {
        ...
        $useCategoryId = Db::getInstance()->getValue('SELECT c.use_category_id
			...
			WHERE c.id = '.pSQL($type));

        $this->smarty->assign([
            'requestUri' => $_SERVER['REQUEST_URI'],
            'categoriesTree' => $this->categoryTreeGenerator->categoriesTree(false,false, false, true),
            'type' => $type,
    ...
    public function updateGoogleCat($categories = array())
    {
        ...
        $useCategoryId = Tools::getValue('use_category_id');
        ...
        Db::getInstance()->Execute('UPDATE ' . _DB_PREFIX_ . 'blmod_xml_category_map 
                SET use_category_id = "' . (int)$useCategoryId . '" WHERE id = ' . pSQL($type));
    ...
    public function addNewFeed($feed_type = 1, $full_address_no_t = '', $token = 0)
    {
        ...
        if (!empty($POST['add_new_feed_insert'])) {
            ...
            $isReadyPro = Tools::getValue('is_ready_pro');
            ...
            if (!empty($POST['name'])) {
                ...
                if ($POST['feed_type'] == 1) {
                    ...
                    $xmlFeedInstall->installDefaultFeedOrderSettings($new_id, $isReadyPro);
                } else {
        ...
        $this->smarty->assign([
            ...
            'moduleImgPath' => $this->moduleImgPath,
            'searchTypeControllerUrl' => $link->getModuleLink($this->name, 'typeSearch'),
        ]);
    ...
    public function feedsSettings($page)
    {
        ...
        $s['merge_attributes_parent_id'] = isset($s['merge_attributes_parent_id']) ? $s['merge_attributes_parent_id'] : [];
        ...
        $cronFileName = !empty($s['cron_file_name']) ? $s['cron_file_name'].'.xml' : 'feed_'.$page.'.xml';
        ...
        $this->smarty->assign([
            ...
            'cronCommand' => (!empty($s['cron_chunks_limit']) ? '*/12 *' : '19 */2').' * * * /usr/bin/php -q '._PS_ROOT_DIR_.'/modules/xmlfeeds/api/xml.php '.$page,
            'cronXmlFile' => $this->getShopProtocol().$_SERVER['HTTP_HOST'].__PS_BASE_URI__.'modules/xmlfeeds/xml_files/'.$cronFileName,
            'page' => $page,
            'fullAdminUrl' => $this->fullAdminUrl,
            ...
            'attributeGroupList' => $feedSettingsAdmin->getAttributeGroupList($s['merge_attributes_parent'], $s['merge_attributes_parent_id']),
            'categoriesTree' => $this->categoryTreeGenerator->categoriesTree($s['cat_list']),
            'categoriesTreeL' => $this->categoryTreeGenerator->categoriesTree($s['cat_without_list']),
            ...
            'categoriesTreeGender' => $this->categoryTreeGenerator->categoriesTree(false, false, true),
            ...
            'customerGroups' => $group->getGroups($this->shopLang),
            'carriersAll' => Carrier::getCarriers($this->shopLang, false, false, false, null, Carrier::ALL_CARRIERS),
            'attributeStructures' => [
                1 => '<NAME>VALUE</NAME>',
                2 => '<atribute_name>NAME</atribute_name><attribute_value>VALUE</attribute_value>',
                3 => '<param name="NAME">VALUE</param>',
                4 => '<PARAM><PARAM_NAME>NAME</PARAM_NAME><VAL>VALUE</VAL></PARAM>',
                5 => '<PARAM><NAME>NAME</NAME><VALUE>VALUE</VALUE></PARAM>',
                6 => '<attribute><code>NAME</code><value>VALUE</value></attribute>',
            ],
        ]);
    ...
    public function printBlock($block_name = false, $info = [], $only_checkbox = false, $onlyRadio = false)
    {
        if (empty($info)) {
            return false;
        }
        ...
        $editPriceFields = [
            ...
            'price_sale_with_min_qty_blmod+bl_extra',
        ];
        ...
        foreach ($info as $y => $i) {
            ...
            $fields[$y]['status_value_checkbox'] = isset($this->tags_info['order+'.$i['table'].'+status']) ? $this->tags_info['order+'.$i['table'].'+status'] : false;
            ...
            $fields[$y]['name'] = $i['name'];
            $fields[$y]['table'] = $i['table'];
    ...
    public function productsXmlSettings($page_id = false)
    {
        ...
        $lang_array = array();
        $page_id = (int)$page_id;
        $iSMinqcValid = XmlFeedsTools::iSMinqcValid();
        ...
        $productBasicInformationFields = array(
            ...
            array('name' => 'all_features', 'title' => 'all_features', 'table' => 'bl_extra'),
            array('name' => 'is_default_combination', 'title' => 'is_default_combination', 'table' => 'bl_extra'),
        );

        if (version_compare(_PS_VERSION_, '1.7.8', '>=')) {
            $productBasicInformationFields[] = ['name' => 'product_type', 'title' => 'product_type', 'table' => 'product',];
        }
        ...
        $pricesRows = [
            ...
            array('name' => 'price_sale_with_min_qty_blmod', 'title' => 'sale_price_with_min._qty', 'table' => 'bl_extra', 'tagName' => 'sale_price_with_qty',),
            ...
            array('name' => 'id_tax_rules_group', 'title' => 'id_tax_rules_group', 'table' => 'product'),
        ];

        if ($iSMinqcValid) {
            $pricesRows[] = ['name' => 'price_of_pallet_minqc_blmod', 'title' => 'price_of_a_pallet', 'table' => 'bl_extra',];
        }
        ...
        if ($iSMinqcValid) {
            $quantitiesRows[] = ['name' => 'lot_size_minqc_blmod', 'title' => 'lot_size', 'table' => 'bl_extra',];
        }
        ...
        $html .= $this->printBlock(
            'Categories',
            array(
                array('name' => 'id_category_default', 'title' => 'main_category_id', 'table' => 'product'),
                array('name' => 'id_category_all', 'title' => 'ids_of_all_categories', 'table' => 'bl_extra'),
                array('name' => 'name', 'title' => 'main_category_name', 'table' => 'category_lang'),
                array('name' => 'main_category_name_2', 'title' => 'main_category_name_2', 'table' => 'bl_extra'),
    ...
    public function updateFeedsS(
        ...
        $customerGroupId = Tools::getValue('customer_group_id', 0);
        ...
        if (!empty($customerGroupId)) {
            $systemCustomer->updateGroup($customerGroupId);
        }

        $error = Db::getInstance()->getMsgError();

        if (!empty($id)) {
            $cronManager->resetChunkFile($id);
            $this->deleteCache($id);
        }
    ...
    public function updateFields($type)
    {
        ...
        $this->feedMode = Db::getInstance()->getValue('SELECT f.feed_mode FROM '._DB_PREFIX_.'blmod_xml_feeds f WHERE f.id = '.(int)$category);
    ...
    public function onSpecial($v, $fieldName = '', $isLowerCase = false)
    {
        ...
        if (in_array($this->feedMode, ['man', 'ro', 'x', 'gla', 'u', 'naj', 'zbo', 'tov', 'alz',])) {
            return $v;
        }
    ...
    protected function validateFeedSettingsUpdate($POST)
    {
        if ((!empty($POST['update_feeds_s']) || !empty($POST['settings_cat']) || !empty($POST['settings_prod'])) && !empty($POST['feeds_name'])) {
            ...
            $filterUpdatedBeforeDays = Tools::getValue('filter_updated_before_days', 0);
            $shippingCountriesStatus = Tools::getValue('shipping_countries_status', 0);
            $shippingCountries = Tools::getValue('shipping_countries');
            $mergeAttributesByGroup = Tools::getValue('merge_attributes_by_group');
            $mergeAttributesParent = Tools::getValue('merge_attributes_parent');
            $mergeAttributesParents = Tools::getValue('merge_attributes_parent_id');
            $itemLimit = Tools::getValue('item_limit');
            $cronChunksLimit = Tools::getValue('cron_chunks_limit');
            $errors = [];
            ...
            if (!empty($filterUpdatedBeforeDays) && !Validate::isInt($filterUpdatedBeforeDays)) {
                $errors[] = $this->l('Filter "Updated for the last XX days" value must a number or empty');
            }
            ...
            if (!empty($mergeAttributesByGroup) && empty($mergeAttributesParent) && empty($mergeAttributesParents)) {
                $errors[] = $this->l('"Merge attributes by group" is enabled, but no attribute is specified');
            }

            if (!empty($itemLimit) && !Validate::isInt($itemLimit)) {
                $errors[] = $this->l('"Product limit" value must be integer (example: 10) or empty');
            }

            if (!empty($cronChunksLimit) && !Validate::isInt($cronChunksLimit)) {
                $errors[] = $this->l('"Generate in chunks" value must be integer (example: 2000) or empty');
            }
    ...
    protected function validateAndInstallMandatoryDatabaseTables()
    {
        $xmlFeedInstall = new XmlFeedInstall();

        $configurationTable = Db::getInstance()->executeS('SHOW TABLES LIKE "'._DB_PREFIX_ . 'blmod_xml_configuration"');

        if (empty($configurationTable)) {
            $xmlFeedInstall->upgrade3128();
        }
    }
