...
require_once(_PS_MODULE_DIR_.'/xmlfeeds/api/OrderXmlApiReadyPro.php');
...
require_once(_PS_MODULE_DIR_.'/xmlfeeds/SystemCustomer.php');
require_once(_PS_MODULE_DIR_.'/xmlfeeds/CronManager.php');
require_once(_PS_MODULE_DIR_.'/xmlfeeds/ConfigurationBlx.php');
...
$permissions['merge_attributes_parent_id'] = !empty($permissions['merge_attributes_parent_id']) ? $permissions['merge_attributes_parent_id'] : [];
$cronFileName = !empty($permissions['cron_file_name']) ? $permissions['cron_file_name'].'.xml' : 'feed_'.$id.'.xml';

if (!empty($permissions['merge_attributes_parent'])) {
    $permissions['merge_attributes_parent_id'][] = $permissions['merge_attributes_parent'];
}
...
if (!empty($permissions['tax_country_id'])) {
    $context->country = new Country($permissions['tax_country_id']);
}
...
if (!empty($permissions['protect_by_ip']) && !$useCron) {
    ...
    if (!empty($ipList[0]) && !in_array(get_ip(), $ipList)) {
...
if ($feed_type == 1) {
    ...
    if ($permissions['feed_mode'] == 'rpro') {
        $orderXmlApi = new OrderXmlApiReadyPro();
    }
...
if ($useCron) {
    $file_url = _PS_ROOT_DIR_.'/modules/xmlfeeds/xml_files/'.$cronFileName;

    if (!empty($permissions['cron_chunks_limit'])) {
        $currentChunkNo = $cronManager->getCurrentChunkNo($id);
        $file_urlOrg = $file_url;
        $file_url = _PS_ROOT_DIR_.'/modules/xmlfeeds/xml_files/chunk_'.$cronFileName;
        file_put_contents($file_url, ($currentChunkNo == 1 ? $mainHeader.$permissions['header_information'] : '').$xml.($currentChunkNo == 0 ? $permissions['footer_information'] : ''), FILE_APPEND | LOCK_EX);

        AccessLog::save($id, 'chunk_'.$currentChunkNo, $sessionId);

        if ($currentChunkNo == 0) {
            rename($file_url, $file_urlOrg);
            $compressor->compress($cronFileName);
            AccessLog::save($id, 'end_file_cron', $sessionId);
        }
    } else {
        $xml = $permissions['header_information'] . $xml . $permissions['footer_information'];
        file_put_contents($file_url, $mainHeader . $xml);
        $compressor->compress($cronFileName);
    }
