...
class AccessLog
{
    ...
    {
        Db::getInstance()->Execute('INSERT INTO '._DB_PREFIX_.'blmod_xml_access_log
            (`feed_id`, `is_cron`, `action`, session_id, get_param, argv_param, created_at)
            VALUES
            ("'.(int)$feedId.'", "'.(int)$isCron.'", "'.pSQL($action).'", "'.pSQL($sessionId).'", "'.pSQL(json_encode($getParam)).'", "'.pSQL(json_encode($argParam)).'", "'.pSQL(date('Y-m-d H:i:s')).'")
        ');
    }
