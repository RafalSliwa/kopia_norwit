...
class ToolsModuleSMP
{
                ...
                if (array_key_exists('lang', $definition_fields[$field]) && $definition_fields[$field]['lang']) {
                    foreach ($languages as $lang) {
                        ...
                        if (!empty($object_field[$lang['id_lang']]) && !forward_static_call_array(
                            ['Validate', $definition_fields[$field]['validate']],
    ...
    public static function registerSmartyFunctions()
    {
        ...
        if (!array_key_exists('no_escape', $smarty->registered_plugins['modifier'])) {
            smartyRegisterFunction($smarty, 'modifier', 'no_escape', [__CLASS__, 'noEscape']);
        }
        if (!array_key_exists('get_image_lang_map', $smarty->registered_plugins['function'])) {
            smartyRegisterFunction($smarty, 'function', 'get_image_lang_map', [__CLASS__, 'getImageLangMap']);
        }
        if (class_exists('TransModSMP')) {
            if (!array_key_exists('ld', $smarty->registered_plugins['modifier'])) {
                smartyRegisterFunction($smarty, 'modifier', 'ld', [TransModSMP::getInstance(), 'ld']);
            }
        }
    }
    ...
    public static $memory_units = ['B', 'KB', 'MB', 'GB', 'TB', 'PB'];
    /**
     * @param $size
     * @return bool
     */
