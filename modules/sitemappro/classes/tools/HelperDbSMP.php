...
class HelperDbSMP
{
    ...
    public function __construct($class_name = null)
    {
        if (!is_null($class_name)) {
            ...
            if (isset($this->definition['multilang_shop']) && $this->definition['multilang_shop']
                ...
                $this->definition['fields']['id_shop'] = [
    ...
    public function installDb()
    {
        ...
        $sql['shop'] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . bqSQL($this->definition['table']) . '_shop`
            (%fields%) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8;';
        $fields = [
            'default' => [$this->definition['primary'] => '`' . bqSQL($this->definition['primary']) . '` int(10) signed NOT NULL AUTO_INCREMENT'],
            'lang' => [$this->definition['primary'] => '`' . bqSQL($this->definition['primary']) . '` int(10) signed NOT NULL', '`id_lang` int(10) signed NOT NULL'],
            'shop' => [$this->definition['primary'] => '`' . bqSQL($this->definition['primary']) . '` int(10) signed NOT NULL', '`id_shop` int(11) signed NOT NULL'],
        ];
    ...
    public function updateObjectField($field, $id, $value)
    {
        ...
        $multi_lang_shop = (
            array_key_exists('multilang_shop', $definition) && $definition['multilang_shop'] ? true : false);
     ...
     */
    public function installManyToOne($entity, $definition_fields = [])
    {
        $fields = [];
        ...
        if ($entity instanceof ObjectModel) {
            ...
            $fields[$def['primary']] = ['type' => ObjectModel::TYPE_INT, 'validate' => 'isUnsignedInt'];
    ...
    public function upgradeManyToOne($entity, $definition_fields = [])
    {
        $fields = [];
        if ($entity instanceof ObjectModel) {
            $def = $this->getDefinition(get_class($entity));
    ...
    public function upgradeTable($table, $definition_fields = [])
    {
    ...
    public function upgrade()
    {
        $tables = [
            $this->definition['table'] => [
                ...
                'type' => 'default',
                'excl_columns' => [$this->definition['primary']],
            ],
            $this->definition['table'] . '_lang' => [
                'fields' => $this->getLangFieldsFromDatabase($this->definition['table']),
                'type' => 'lang',
                'excl_columns' => [$this->definition['primary'], 'id_lang'],
            ],
            $this->definition['table'] . '_shop' => [
                'fields' => $this->getShopFieldsFromDatabase($this->definition['table']),
                'type' => 'shop',
                'excl_columns' => [$this->definition['primary'], 'id_shop'],
            ],
        ];
