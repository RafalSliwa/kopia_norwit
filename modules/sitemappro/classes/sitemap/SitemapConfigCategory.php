...
class SitemapConfigCategory extends SitemapConfig
{
    ...
    protected static function getItemsSql($id_lang = null)
    {
        ...
        $sql->leftJoin('category_lang', 'cl', 'c.`id_category` = cl.`id_category` ' . (!is_null($id_lang) ? 'AND cl.`id_lang` = ' . (int) $id_lang : ''));
        $sql->leftJoin('category_shop', 'cs', 'c.`id_category` = cs.`id_category`');
        $sql->where('cl.`id_shop` = ' . (int) Shop::getContextShopID()
            . ' AND c.`active` = 1'
    ...
    public static function getItems($id_lang = null, $include_link = false, $with_image = false)
    {
        ...
        foreach ($items as &$item) {
            ...
            if ($with_image) {
                ...
                // ps16
                if (version_compare(_PS_VERSION_, '1.6.24', '<')) {
                     $thumbs = glob(_PS_IMG_DIR_ . 'c/' . $item['id_category'] . '-medium_default.jpg');
                } else {
                    $thumbs = glob(_PS_IMG_DIR_.'c/'.$item['id_category'].ImageType::getFormattedName('medium').'.jpg');
                }
