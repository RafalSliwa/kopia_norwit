...
class SitemapConfig extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'priority' => ['type' => 'DECIMAL(2,1)'],
            'type_object' => ['type' => self::TYPE_STRING, 'validate' => 'isString'],
            'changefreq' => ['type' => self::TYPE_STRING, 'validate' => 'isString'],
            'is_export' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
            'default_category' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
        ],
    ];
    ...
    protected static function getConfigSql()
    {
        ...
        $sql->where('a.`id_shop` = ' . (int) Context::getContext()->shop->id . ' AND a.`type_object` = "' . pSQL(static::getType()) . '"');
        return $sql;
    }
    ...
    public static function getSitemapFilename(
        ...
        $file_name = 'sitemap' . $lang_prefix . $shop_prefix . ($with_image ? '-with-images' : '') . $name_text
    ...
    public static function getFullLinkSitemap(
        ...
        return _PS_ROOT_DIR_ . '/' . self::getSitemapFilename($id_lang, $without_index, $with_image, $with_link, $only);
    }
    ...
    public static function getLinkSitemap(
        ...
        return ToolsSMP::getShopDomainWithBase()
            . self::getSitemapFilename($id_lang, $without_index, $with_image, $with_link, $only);
    }
    ...
    public static function getDefaultSettings($type = null)
    {
        ...
        if (is_null($type)) {
            ...
            $default_category = (ConfSMP::hasConf($type . '_default_category', null, $id_shop) ? ConfSMP::getConf($type . '_default_category', null, $id_shop) : self::$default_params['default_category']);
    ...
    public static function setDefaultSettings($settings, $type = null)
    {
        ...
        if (is_null($type)) {
            ...
            ConfSMP::setConf(
                $type . '_is_export',
                isset($settings['is_export']) ? $settings['is_export'] : 0,
                false,
                null,
                $id_shop
            );
            ConfSMP::setConf(
                $type . '_is_changefreq',
                isset($settings['is_changefreq']) ? $settings['is_changefreq'] : 0,
                false,
                null,
                $id_shop
            );
            ConfSMP::setConf(
                $type . '_is_on_default_setting_category',
                isset($settings['is_on_default_setting_category']) ? $settings['is_on_default_setting_category'] : 0,
                false,
                null,
                $id_shop
            );
            ConfSMP::setConf(
                $type . '_is_on_default_setting_export_product',
                isset($settings['is_on_default_setting_export_product']) ? $settings['is_on_default_setting_export_product'] : 0,
                false,
                null,
                $id_shop
            );
            ConfSMP::setConf(
                $type . '_priority',
                isset($settings['priority']) ? $settings['priority'] : self::$default_params['priority'],
                false,
                null,
                $id_shop
            );
            ConfSMP::setConf(
                $type . '_changefreq',
                isset($settings['changefreq']) ? $settings['changefreq'] : self::$default_params['changefreq'],
                false,
                null,
                $id_shop
            );
            ConfSMP::setConf(
                ...
                isset($settings['default_category']) ? $settings['default_category'] : self::$default_params['default_category'],
                false,
                null,
                $id_shop
            );
