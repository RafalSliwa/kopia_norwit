...
class SitemapBuilder
{
    ...
    public function generate($id_lang = null, $with_image = false, $with_link = false, $only = 0)
    {
        ...
        if ($only == 0 || $only == 2) {
            foreach (SitemapConfigCategory::getItems($id_lang, $with_link, $with_image) as $category) {
                if (in_array($category['id_category'], [1, 2])) {
        ...
        $include_id_attr = (int) ConfSMP::getConf('INCLUDE_ID_IN_ATTRIBUTE');

        if ((strpos($sitemap->filename, 'sitemap-with-alternate-links') !== false)
            || (strpos($sitemap->filename, 'sitemap-with-images-with-alternate-links') !== false) || $sitemap->filename == 'sitemap') {
        ...
        if ($only == 0 || $only == 1) {
            foreach ($lang_mass as $item) {
                ...
                foreach (SitemapConfigProduct::getItems($id_lang, $include_id_attr, $with_image) as $product) {
                    ...
                    if (ConfSMP::getConf('EXPORT_COMBINATION') == 1) {
                        ...
                        if ($combinations != []) {
                            foreach ($combinations as $combination) {
                                $anchor = $this->getAnchor(
                                    ...
                                    (bool) ConfSMP::getConf('INCLUDE_ID_IN_ATTRIBUTE')
                                );
                                ...
                                foreach ($product['links'] as $l) {
                                    ...
                                    $links[$iso_code] = $this->link->getProductLink($l['id_product'], $l['link_rewrite'], null, null, $l['id_lang'], null, $temp) . $anchor;
                                }
                                if ((bool) ConfSMP::getConf('INCLUDE_ID_IN_ATTRIBUTE')) {
                                ...
                                foreach ($product['images'] as $image) {
                                    ...
                                    if (ConfSMP::getConf('ALLOW_IMAGE_CAPTION_ATTR')) {
                                        $image['caption'] = strip_tags(
                                            html_entity_decode($product['description_short'])
                                        );
                                    }
                                ...
                                if ($product_default_settings['is_on_default_setting_category']) {
                                    $default = isset($categories[$product['id_category_default']]) ? $categories[$product['id_category_default']] : (isset($categories[$product['id_category']]) ? $categories[$product['id_category']] : [
                                        'priority' => 0.5,
                                        'changefreq' => 'always',
                                    ]);
                                } else {
                    ...
                    } elseif (ConfSMP::getConf('EXPORT_COMBINATION') == 0 && ConfSMP::getConf('EXPORT_COMBINATION_DEF') == 1) {
                        $combinations = Product::getProductAttributesIds($product['id_product']);
                        if ($combinations != []) {
                            $combination_def = (int) Product::getDefaultAttribute($product['id_product']);
                            $anchor = $this->getAnchor(
                                $product['id_product'],
                                $combination_def,
                                (bool) ConfSMP::getConf('INCLUDE_ID_IN_ATTRIBUTE')
                            ...
                            $anchor = '';
                            $link = $this->link->getProductLink($product['id_product'], $product['link_rewrite'], null, null, $product['id_lang'], null, null);
                            $links = [];
                            foreach ($product['links'] as $l) {
                                $iso_code = Language::getIsoById($l['id_lang']);
                                if (!$iso_code) {
        ...
        $sitemap->createSitemapIndex(ToolsSMP::getShopDomainWithBase(), 'Today', $only);
    }
