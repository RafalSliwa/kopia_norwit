...
class SitemapConfigCms extends SitemapConfig
{
    ...
    protected static function getItemsSql($id_lang = null)
    {
        ...
        $sql->where(version_compare(_PS_VERSION_, '1.6.0.9', '>') ? 'l.id_shop = a.`id_shop`' : '');
        $sql->where('l.`id_lang` IN(' . implode(',', array_map('intval', ToolsModuleSMP::getLanguageIds())) . ')');
    ...
    public static function getLinks($id_cms)
    {
        ...
        $sql->innerJoin(
            ...
            'l',
            'c.id_cms = l.id_cms
            ' . (version_compare(_PS_VERSION_, '1.6', '>=') ? ' AND l.id_shop = ' . (int) Shop::getContextShopID() : '')
        );
        $sql->leftJoin(
            'lang',
            'lang',
