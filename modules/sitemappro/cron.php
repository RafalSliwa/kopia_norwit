...
foreach ($shops as $shop_id) {
    ...
    $_SERVER['HTTP_X_FORWARDED_HOST'] = Tools::usingSecureMode() ? Tools::getShopDomainSSL() : Tools::getShopDomain();

    try {
        SitemapBuilder::getInstance()->generate($id_lang, $with_image, $with_link, $only);
        if (Tools::isSubmit('ajax') && !ToolsModuleSMP::isPHPCLI()) {
            ...
            exit(json_encode([
                ...
                'message' => ($exists_sitemap_link ? $t->l('Sitemap generate successfully!', __FILE__) : $t->l('Sitemap generate with error!', __FILE__)),
            ]));
        }
    } catch (Exception $e) {
