...
class AdminSiteMapProController extends ModuleAdminControllerSMP
{
    ...
    public function getSelect2Products($categories = [], $ids_products = [], $search = null)
    {
    ...
    public function ajaxProcessSearchManufacturer()
    {
        ...
        $result = Db::getInstance()->executeS(
            ...
            ' . (is_array($excl_ids) && count($excl_ids) ? 'and m.`id_manufacturer` NOT IN(' . implode(',', array_map('intval', $excl_ids)) . ')' : '') . '
            HAVING ' . ToolsModuleSMP::buildSQLSearchWhereFromQuery($query, false, '`text`'));
    ...
    public function ajaxProcessSearchSupplier()
    {
        ...
        $result = Db::getInstance()->executeS(
            ...
            ' . (is_array($excl_ids) && count($excl_ids) ? 'and s.`id_supplier` NOT IN(' . implode(',', array_map('intval', $excl_ids)) . ')' : '') . '
            HAVING ' . ToolsModuleSMP::buildSQLSearchWhereFromQuery($query, false, '`text`'));
    ...
    public function ajaxProcessSaveConf()
    {
        ...
        $excluded_products = (isset($data['sitemap_excluded']) ? $data['sitemap_excluded'] : []);
        $user_links = (isset($data['user_links']) ? $data['user_links'] : []);
        $default_settings = (isset($data['default_settings']) ? $data['default_settings'] : []);
        ...
        if (is_array($data)
            ...
            array_unique($all_cms_setting, SORT_REGULAR);
            ...
            foreach ($merge_cms as $item) {
                if (in_array($item['type_object'], ['category', 'cms', 'meta', 'manufacturer', 'supplier']) && !isset($item['is_export'])) {
                    $item['is_export'] = 0;
                }
    ...
    public static function getCMS($idLang = null, $limit = 25, $page = 1)
    {
       ...
       INNER JOIN `' . _DB_PREFIX_ . 'cms_lang` l ON c.id_cms = l.id_cms AND l.id_shop = ' . Context::getcontext()->shop->id . ' AND l.id_lang = ' . $idLang . '
    ...
    {
        $sql = 'SELECT count(*) FROM `' . _DB_PREFIX_ . 'cms` WHERE active = 1';
