        ...
        if (Tools::getIsset('submitBulkduplicateEmailTemplateets_abancart_email_template')) {
            ...
            if ($temps && is_array($temps)) {
                ...
                if ($temps) {
                    ...
                    if ($success) {
                        $this->context->cookie->msg_bulk_email_temp_success = sprintf($this->module->l('Duplicated %s email(s) template successfully', 'AdminEtsACEmailTemplateController'), $count);
                    }
                }

            }
            if (!$temps) {
                $this->context->cookie->msg_bulk_email_temp_error = $this->module->l('No email template selected to duplicate', 'AdminEtsACEmailTemplateController');
            } else
                $this->context->cookie->msg_bulk_email_temp_error = $this->module->l('Duplicate email template failed', 'AdminEtsACEmailTemplateController');
