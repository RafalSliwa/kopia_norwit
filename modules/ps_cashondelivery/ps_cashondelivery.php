...
class Ps_Cashondelivery extends PaymentModule
{
    ...
    public function hookPaymentOptions(array $params)
    {
        ...
        // Check maximum amount limit for Cash on Delivery (15000 PLN)
        $maxCodAmount = 15000;
        $cartTotal = $cart->getOrderTotal(true, Cart::BOTH);
        
        // DEBUG - Remove after testing
        error_log("COD DEBUG: Cart total = " . $cartTotal . " PLN, Max allowed = " . $maxCodAmount . " PLN");
        ...
        $templateVars = [
            'cod_max_amount' => number_format($maxCodAmount, 0, '', ' ')
        ];
        
        if ($cartTotal > $maxCodAmount) {
            // Display disabled option with error message
            $templateVars['cod_disabled'] = true;
            $templateVars['cart_total'] = number_format($cartTotal, 2, ',', ' ');
            
            // Simple text without HTML tags
            $cashOnDeliveryOption->setCallToActionText(
                $this->trans('Pay by Cash on Delivery', [], 'Modules.Cashondelivery.Shop') . 
                ' - NIEDOSTĘPNE (przekroczono limit 15 000 zł)'
            );
            
            // No action - option will be disabled
            $cashOnDeliveryOption->setAction('');
        } else {
            // Normal flow - option enabled
            $templateVars['cod_disabled'] = false;
        ...
        // Assign variables and fetch template
        $this->context->smarty->assign($templateVars);
        
        $additionalInfo = $this->context->smarty->fetch(
            'module:ps_cashondelivery/views/templates/hook/paymentOptions-additionalInformation.tpl'
        );
        
        $cashOnDeliveryOption->setAdditionalInformation($additionalInfo);
