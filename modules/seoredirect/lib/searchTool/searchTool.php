...
class searchToolseoredirect extends seoredirect
{
    public $addon;
    public $name;
    public $tab;
    ...
    public function productsFound($type, $id)
    {
        if ($type == 'attribute') {
            ...
            if (isset($result[0]['id_product'])) {
                return ' | ' . Context::getContext()->controller->module->getTranslator()->trans('Products found:', [], 'Modules.Seoredirect.seoredirect') . ' ' . count($result);
            } else {
                return '';
            }
        }
    ...
    public function searchTool($type, $resultInput, $replacementType = 'replace', $returnBox = false, $object = false, $combinations = false, $combination_input = '', $selected_combination = false, $showCounter = false)
    {
        ...
        return '<input style="width: 80px; font-size: 10px; margin: 0px; height: 17px;" type="text" placeholder="' . Context::getContext()->controller->module->getTranslator()->trans('Search', [], 'Modules.Seoredirect.seoredirect') . '" id="searchTool_' . $type . '" data-replacementtype="' . $replacementType . '" data-resultinput="' . $resultInput . '" data-type="' . $type . '" data-combinations="' . (int)$combinations . '" data-combinations-class="' . $combination_input . '" data-toggle="tooltip" data-showCounter="' . (bool)$showCounter . '" data-original-title="' . Context::getContext()->controller->module->getTranslator()->trans('Search for name of: ', [], 'Modules.Seoredirect.seoredirect') . $type . '" class="label-tooltip searchToolInput searchTool_' . $type . '"/>';
    }
    ...
    public function returnCombinations($id = 0, $class = 'class', $preselected = false)
    {
        ...
        if (count($combinations) > 0) {
            foreach ($combinations AS $key => $combination) {
                ...
                $combinations_array[$combination['id_product_attribute']] = "<div class='form-control-static margin-form '><div class='btn btn-default " . $class . "Buttons " . (($preselected != false) ? (($preselected == $combination['id_product_attribute']) ? 'active' : '') : '') . "' onclick=\"$('." . $class . "Buttons').removeClass('active'); $(this).addClass('active'); $('#" . $class . "').val(" . $combination['id_product_attribute'] . ")\" >" . Context::getContext()->controller->module->getTranslator()->trans('select', [], 'Modules.Seoredirect.seoredirect') . "</div> " . $combination_name . " " . ($combination['reference'] != '' ? '(' . $combination['reference'] . ')' : '') . "</div>";
            }
            return '<div class="panel ' . $class . 'selectedCombinations" style="margin-top:20px"><h3>' . Context::getContext()->controller->module->getTranslator()->trans('Select combination', [], 'Modules.Seoredirect.seoredirect') . '</h3>' . implode('', $combinations_array) . '</div>';
        }
    }
}